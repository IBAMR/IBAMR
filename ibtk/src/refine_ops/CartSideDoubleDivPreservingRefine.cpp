...
CartSideDoubleDivPreservingRefine::CartSideDoubleDivPreservingRefine(const int u_dst_idx,
                                                                     ...
                                                                     RefinePatchStrategyNd* const phys_bdry_op)
...
void
CartSideDoubleDivPreservingRefine::setPhysicalBoundaryConditions(PatchNd& patch,
                                                                 const double fill_time,
...
void
CartSideDoubleDivPreservingRefine::preprocessRefine(PatchNd& /*fine*/,
...
void
CartSideDoubleDivPreservingRefine::postprocessRefine(PatchNd& fine,
                                                     const PatchNd& coarse,
                                                     const BoxNd& unrestricted_fine_box,
                                                     const IntVectorNd& ratio)
{
    ...
    // corrections.
    const BoxNd fine_box = unrestricted_fine_box * BoxNd::grow(fine.getBox(), 2);
    ...
    SAMRAIPointer<SideDataNd<double> > fdata = fine.getPatchData(d_u_dst_idx);
#if !defined(NDEBUG)
    TBOX_ASSERT(fdata);
#endif
    ...
    SAMRAIPointer<SideDataNd<double> > cdata = coarse.getPatchData(d_u_dst_idx);
#if !defined(NDEBUG)
    TBOX_ASSERT(cdata);
    ...
    if (ratio == IntVectorNd(2))
    {
        ...
        SAMRAIPointer<SideDataNd<double> > u_src_data = fine.getPatchData(d_u_src_idx);
        SAMRAIPointer<SideDataNd<double> > indicator_data = fine.getPatchData(d_indicator_idx);
        ...
        if (u_src_data && indicator_data)
        {
            ...
            {
                for (BoxNd::Iterator b(SideGeometryNd::toSideBox(fine_box, axis)); b; b++)
                {
                    const hier::IndexNd& i = b();
                    const SideIndexNd i_s(i, axis, 0);
        ...
        if (indicator_data)
        {
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                for (BoxNd::Iterator b(SideGeometryNd::toSideBox(fine_box, axis)); b; b++)
                {
                    ...
                    if (!(std::abs((*indicator_data)(i_s)-1.0) < 1.0e-12))
                    {
                        const hier::IndexNd i_coarse_lower = IndexUtilities::coarsen(i, ratio);
                        const hier::IndexNd i_lower = IndexUtilities::refine(i_coarse_lower, ratio);
                        if (i(axis) == i_lower(axis)) continue;

                        hier::IndexNd i_coarse_upper = i_coarse_lower;
                        i_coarse_upper(axis) += 1;
                        const hier::IndexNd i_upper = IndexUtilities::refine(i_coarse_upper, ratio);

                        const double w1 =
                            static_cast<double>(i(axis) - i_lower(axis)) / static_cast<double>(ratio(axis));
                        const double w0 = 1.0 - w1;

                        const SideIndexNd i_s_lower(i_lower, axis, 0);
                        const SideIndexNd i_s_upper(i_upper, axis, 0);
                        for (int depth = 0; depth < fdata_depth; ++depth)
                        {
        ...
        const BoxNd correction_box = BoxNd::refine(BoxNd::coarsen(fine_box, 2), 2);
#if !defined(NDEBUG)
    ...
    else
    {
        ...
        const BoxNd intermediate_patch_box = BoxNd::refine(coarse.getBox(), 2);
        PatchNd intermediate(intermediate_patch_box, coarse.getPatchDescriptor());
        intermediate.allocatePatchData(d_u_dst_idx);

        // Setup a patch geometry object for the intermediate patch.
        SAMRAIPointer<CartesianPatchGeometryNd> pgeom_coarse = coarse.getPatchGeometry();
        const IntVectorNd& ratio_to_level_zero_coarse = pgeom_coarse->getRatio();
        ...
        const IntVectorNd ratio_to_level_zero_intermediate = ratio_to_level_zero_coarse * 2;
        ...
        intermediate.setPatchGeometry(new CartesianPatchGeometryNd(ratio_to_level_zero_intermediate,
        ...
        const BoxNd intermediate_box = BoxNd::grow(BoxNd::coarsen(fine_box, ratio / 2), 2);
        ...
        if (fine.checkAllocated(d_u_src_idx) && fine.checkAllocated(d_indicator_idx))
        {
            ...
            SAMRAIPointer<SideDataNd<double> > u_src_idata = intermediate.getPatchData(d_u_src_idx);
            SAMRAIPointer<SideDataNd<double> > indicator_idata = intermediate.getPatchData(d_indicator_idx);
            u_src_idata->fillAll(std::numeric_limits<double>::quiet_NaN());
            indicator_idata->fillAll(-1.0);
#if !defined(NDEBUG)
            SAMRAIPointer<SideDataNd<double> > u_src_fdata = fine.getPatchData(d_u_src_idx);
            SAMRAIPointer<SideDataNd<double> > indicator_fdata = fine.getPatchData(d_indicator_idx);
            TBOX_ASSERT(u_src_fdata->getGhostBox().contains(BoxNd::refine(intermediate_box, ratio / 2)));
            TBOX_ASSERT(indicator_fdata->getGhostBox().contains(BoxNd::refine(intermediate_box, ratio / 2)));
#endif
