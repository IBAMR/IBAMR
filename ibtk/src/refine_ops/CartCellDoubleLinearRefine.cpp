                                   ...
                                   const IntVector<NDIM>& ratio) const
{
    ...
    {
        TBOX_ERROR("CartCellDoubleLinearRefine::refine(): data require uniform ghost widths and matching depths.\n");
    }
    for (int axis = 0; axis < NDIM; ++axis)
    {
        if (ratio(axis) <= 0)
        {
            TBOX_ERROR("CartCellDoubleLinearRefine::refine(): refinement ratios must be positive.\n");
        }
    }
