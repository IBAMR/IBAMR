...
void
CartCellDoubleBoundsPreservingConservativeLinearRefine::refine(PatchNd& fine,
                                                               ...
                                                               const IntVectorNd& ratio) const
{
    ...
    BoxNd correction_box = BoxNd::refine(BoxNd::coarsen(fine_box, ratio), ratio);
    ...
    const BoxNd coarse_correction_box = BoxNd::coarsen(correction_box, ratio);

    BoxListNd uncorrected_boxes(fine_box);
    ...
    // box.
    for (BoxListNd::Iterator b(uncorrected_boxes); b; b++)
    {
    ...
    for (int depth = 0; depth < data_depth; ++depth)
    {
        ...
        {
            const hier::IndexNd& i_crse = b();
            const hier::IndexNd i_fine = i_crse * ratio;
            ...
            for (BoxNd::Iterator b(stencil_box_crse); b; b++)
            {
            ...
            // discrepancy.
            BoxNd stencil_box_fine(i_fine, i_fine);
            stencil_box_fine.growUpper(ratio - IntVectorNd(1));
            double Delta = 0.0;
            for (BoxNd::Iterator b(stencil_box_fine); b; b++)
            {
