                                                               ...
                                                               const int finest_ln) const
{
         ...
         ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            {
                Pointer<SAMRAISideData<double>> u_out_data = patch->getPatchData(u_out_data_idx);
                Pointer<SAMRAISideData<double>> u_in_data = patch->getPatchData(u_in_data_idx);
...
void
StaggeredPhysicalBoundaryHelper::copyDataAtDirichletBoundaries(Pointer<SAMRAISideData<double>> u_out_data,
                                                               Pointer<SAMRAISideData<double>> u_in_data,
                                                               Pointer<SAMRAIPatch> patch) const
{
    ...
    const std::vector<Pointer<SAMRAIArrayData<bool>>>& dirichlet_bdry_locs =
                                                      ...
                                                      const int finest_ln) const
{
         ...
         ++ln)
    {
        ...
        {
            Pointer<SAMRAIPatch> patch = level->getPatch(p());
            Pointer<SAMRAISideData<int>> mask_data = patch->getPatchData(mask_data_idx);
            if (patch->getPatchGeometry()->getTouchesRegularBoundary())
            {
...
void
StaggeredPhysicalBoundaryHelper::setupMaskingFunction(Pointer<SAMRAISideData<int>> mask_data,
                                                      Pointer<SAMRAIPatch> patch) const
{
    mask_data->fillAll(0);
    if (!patch->getPatchGeometry()->getTouchesRegularBoundary()) return;
...
bool
StaggeredPhysicalBoundaryHelper::patchTouchesDirichletBoundary(Pointer<SAMRAIPatch> patch) const
{
...
bool
StaggeredPhysicalBoundaryHelper::patchTouchesDirichletBoundaryAxis(Pointer<SAMRAIPatch> patch,
                                                 ...
                                                 const Pointer<SAMRAIPatchHierarchy> hierarchy)
{
    ...
    for (int ln = 0; ln <= finest_hier_level; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAIPatch> patch = level->getPatch(patch_num);
            ...
            if (pgeom->getTouchesRegularBoundary())
            {
                ...
                const int n_physical_codim1_boxes = physical_codim1_boxes.size();
                std::vector<Pointer<SAMRAIArrayData<bool>>>& dirichlet_bdry_locs = d_dirichlet_bdry_locs[ln][patch_num];
                ...
                for (int n = 0; n < n_physical_codim1_boxes; ++n)
                {
                    ...
                    Pointer<SAMRAIArrayData<double>> acoef_data = new SAMRAIArrayData<double>(bc_coef_box, 1);
                    Pointer<SAMRAIArrayData<double>> bcoef_data = new SAMRAIArrayData<double>(bc_coef_box, 1);
                    Pointer<SAMRAIArrayData<double>> gcoef_data;
                    u_bc_coefs[bdry_normal_axis]->setBcCoefs(acoef_data,
                                                             bcoef_data,
                                                             gcoef_data,
                                                             Pointer<SAMRAIVariable>(),
                                                             *patch,
                                                             trimmed_bdry_box,
                                                             fill_time);
