...
void
CartSideRobinPhysBdryOp::setPhysicalBoundaryConditions(PatchNd& patch,
...
void
CartSideRobinPhysBdryOp::accumulateFromPhysicalBoundaryData(PatchNd& patch,
                                                            const double fill_time,
                                                            const IntVectorNd& ghost_width_to_fill)
{
    if (ghost_width_to_fill == IntVectorNd(0)) return;
...
#endif
    const IntVectorNd gcw_to_fill = IntVectorNd::min(patch_data->getGhostCellWidth(), ghost_width_to_fill);
                                                             ...
                                                             const bool adjoint_op)
{
    ...
    std::array<BoxNd, NDIM> side_box;
    for (unsigned int axis = 0; axis < NDIM; ++axis)
    {
        side_box[axis] = SideGeometryNd::toSideBox(patch_box, axis);
    }
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        for (unsigned int axis = 0; axis < NDIM; ++axis)
        {
            if (axis != bdry_normal_axis)
            {
                ...
                for (int d = 0; d < patch_data_depth; ++d)
                {
                    RobinBcCoefStrategyNd* bc_coef = d_bc_coefs[NDIM * d + axis];
                    auto const extended_bc_coef = dynamic_cast<ExtendedRobinBcCoefStrategy*>(bc_coef);
                    if (extended_bc_coef)
                    {
                        extended_bc_coef->setTargetPatchDataIndex(patch_data_idx);
...
CartSideRobinPhysBdryOp::fillGhostCellValuesCodim2(const int patch_data_idx,
                                                   const Array<BoundaryBoxNd>& physical_codim2_boxes,
                                                   const IntVectorNd& ghost_width_to_fill,
                                                   const PatchNd& patch,
                                                   const bool adjoint_op)
{
    ...
    for (int n = 0; n < n_physical_codim2_boxes; ++n)
    {
        ...
        for (int d = 0; d < patch_data_depth; ++d)
        {
            ...
            if (location_index < 4)
            {
                ...
                BoxNd side_bc_fill_box = SideGeometryNd::toSideBox(bc_fill_box, axis);
...
CartSideRobinPhysBdryOp::fillGhostCellValuesCodim3(const int patch_data_idx,
                                                   const Array<BoundaryBoxNd>& physical_codim3_boxes,
                                                   const IntVectorNd& ghost_width_to_fill,
                                                   const PatchNd& patch,
                                                   const bool adjoint_op)
{
