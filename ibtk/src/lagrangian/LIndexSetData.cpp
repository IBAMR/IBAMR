...
template <class T>
LIndexSetData<T>::LIndexSetData(BoxNd box, IntVectorNd ghosts) : LSetData<T>(std::move(box), std::move(ghosts))
{
    // intentionally blank
    return;
} // LIndexSetData
...
template <class T>
void
LIndexSetData<T>::cacheLocalIndices(SAMRAIPointer<PatchNd> patch, const IntVectorNd& periodic_shift)
{
    d_lag_indices.clear();
