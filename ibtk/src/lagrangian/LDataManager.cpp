...
void
LDataManager::setPatchHierarchy(Pointer<PatchHierarchyNd> hierarchy)
{
#if !defined(NDEBUG)
                     ...
                     const int finest_ln_in)
{
    ...
    Pointer<VariableNd> f_var;
    var_db->mapIndexToVariable(f_data_idx, f_var);
    Pointer<CellVariableNd<double> > f_cc_var = f_var;
    Pointer<EdgeVariableNd<double> > f_ec_var = f_var;
    Pointer<NodeVariableNd<double> > f_nc_var = f_var;
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            if (ec_data)
            {
                Pointer<EdgeDataNd<double> > f_ec_data = f_data;
                LEInteractor::spread(
                    f_ec_data, F_data[ln], X_data[ln], idx_data, patch, box, periodic_shift, spread_kernel_fcn);
            }
            if (nc_data)
            {
                Pointer<NodeDataNd<double> > f_nc_data = f_data;
                LEInteractor::spread(
...
LDataManager::interp(const int f_data_idx,
                     ...
                     Pointer<LData> X_data,
                     const int level_num,
                     const std::vector<Pointer<CoarsenScheduleNd> >& f_synch_scheds,
                     const std::vector<Pointer<RefineScheduleNd> >& f_ghost_fill_scheds,
                     const double fill_data_time)
{
    const int coarsest_ln = 0;
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
...
void
LDataManager::registerVisItDataWriter(Pointer<VisItDataWriterNd> visit_writer)
{
#if !defined(NDEBUG)
...
void
LDataManager::registerLoadBalancer(Pointer<LoadBalancerNd> load_balancer, int workload_idx)
{
    ...
    Pointer<VariableNd> workload_var;
...
LDataManager::endDataRedistribution(const int coarsest_ln_in, const int finest_ln_in)
{
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        Pointer<PatchLevelNd> level = d_hierarchy->getPatchLevel(level_number);
        Pointer<BoxTreeNd> box_tree = level->getBoxTree();
...
void
LDataManager::addWorkloadEstimate(Pointer<PatchHierarchyNd> hierarchy,
...
LDataManager::updateNodeCountData(const int coarsest_ln_in, const int finest_ln_in)
{
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            const BoxNd& patch_box = patch->getBox();
            const Pointer<LNodeSetData> idx_data = patch->getPatchData(d_lag_node_index_current_idx);
            Pointer<CellDataNd<double> > node_count_data = patch->getPatchData(d_node_count_idx);
...
void
LDataManager::initializeLevelData(const Pointer<BasePatchHierarchyNd> hierarchy,
                                  const int level_number,
...
void
LDataManager::resetHierarchyConfiguration(const Pointer<BasePatchHierarchyNd> hierarchy,
                                          const int coarsest_ln,
                                          const int finest_ln)
{
    ...
    {
        Pointer<PatchLevelNd> level = hierarchy->getPatchLevel(level_number);
        Pointer<PatchLevelNd> coarser_level = hierarchy->getPatchLevel(level_number - 1);
...
void
LDataManager::applyGradientDetector(const Pointer<BasePatchHierarchyNd> hierarchy,
                                    const int level_number,
                                    ...
                                    const bool /*uses_richardson_extrapolation_too*/)
{
    ...
    {
        Pointer<PatchLevelNd> level = hierarchy->getPatchLevel(level_number);
        Pointer<PatchLevelNd> finer_level = hierarchy->getPatchLevel(level_number + 1);
        const IntVectorNd& ratio = level->getRatio();
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            const Pointer<CellDataNd<double> > node_count_data = patch->getPatchData(d_node_count_idx);
      ...
      d_ghost_width(std::move(ghost_width))
{
    ...
    Pointer<RefineOperatorNd> lag_node_index_bdry_fill_op = Pointer<RefineOperatorNd>(nullptr);
    ...
    Pointer<CoarsenOperatorNd> node_count_coarsen_op = new CartesianCellDoubleWeightedAverageNd();
