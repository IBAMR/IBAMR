...
{
// Timers.
static SAMRAITimer* t_spread;
static SAMRAITimer* t_interp;
static SAMRAITimer* t_map_lagrangian_to_petsc;
static SAMRAITimer* t_map_petsc_to_lagrangian;
static SAMRAITimer* t_begin_data_redistribution;
static SAMRAITimer* t_end_data_redistribution;
static SAMRAITimer* t_update_workload_estimates;
static SAMRAITimer* t_update_node_count_data;
static SAMRAITimer* t_initialize_level_data;
static SAMRAITimer* t_reset_hierarchy_configuration;
static SAMRAITimer* t_apply_gradient_detector;
static SAMRAITimer* t_put_to_database;
static SAMRAITimer* t_begin_nonlocal_data_fill;
static SAMRAITimer* t_end_nonlocal_data_fill;
static SAMRAITimer* t_compute_node_distribution;
static SAMRAITimer* t_compute_node_offsets;
...
LDataManager::getManager(const std::string& name,
                         ...
                         const SAMRAIIntVector& min_ghost_width,
                         bool register_for_restart)
{
    ...
    {
        const SAMRAIIntVector ghost_width = SAMRAIIntVector::max(
            min_ghost_width,
            SAMRAIIntVector(std::max(LEInteractor::getMinimumGhostWidth(default_interp_kernel_fcn),
...
void
LDataManager::setPatchHierarchy(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy)
{
#if !defined(NDEBUG)
                     ...
                     const int finest_ln_in)
{
    ...
    SAMRAIPointer<SAMRAIEdgeVariable<double>> f_ec_var = f_var;
    SAMRAIPointer<SAMRAINodeVariable<double>> f_nc_var = f_var;
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        const SAMRAIIntVector& periodic_shift = grid_geom->getPeriodicShift(level->getRatio());
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            const SAMRAIBox& box = idx_data->getGhostBox();
            if (cc_data)
            {
            ...
            if (ec_data)
            {
                SAMRAIPointer<SAMRAIEdgeData<double>> f_ec_data = f_data;
                LEInteractor::spread(
                    f_ec_data, F_data[ln], X_data[ln], idx_data, patch, box, periodic_shift, spread_kernel_fcn);
            }
            if (nc_data)
            {
                SAMRAIPointer<SAMRAINodeData<double>> f_nc_data = f_data;
                LEInteractor::spread(
                     ...
                     const int finest_ln_in)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            SAMRAIPointer<SAMRAIPatchData> f_data = patch->getPatchData(f_data_idx);
            SAMRAIPointer<LNodeSetData> idx_data = patch->getPatchData(d_lag_node_index_current_idx);
            const SAMRAIBox& box = idx_data->getBox();
            if (cc_data)
            {
                SAMRAIPointer<SAMRAICellData<double>> f_cc_data = f_data;
...
void
LDataManager::registerVisItDataWriter(SAMRAIPointer<SAMRAIVisItDataWriter> visit_writer)
{
#if !defined(NDEBUG)
...
void
LDataManager::registerLoadBalancer(SAMRAIPointer<SAMRAILoadBalancer> load_balancer, int workload_idx)
{
#if !defined(NDEBUG)
    ...
    SAMRAIPointer<SAMRAIVariable> workload_var;
    SAMRAIVariableDatabase::getDatabase()->mapIndexToVariable(d_workload_idx, workload_var);
...
LDataManager::beginDataRedistribution(const int coarsest_ln_in, const int finest_ln_in)
{
    ...
    std::vector<std::map<int, SAMRAIIntVector>> periodic_offset_data(finest_ln + 1);
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        const SAMRAIIntVector& periodic_shift = d_grid_geom->getPeriodicShift();
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            for (LNodeSetData::CellIterator it(SAMRAIBox::grow(patch_box, SAMRAIIntVector(CFL_WIDTH))); it; it++)
            {
                const SAMRAIIndex& old_cell_idx = *it;
                ...
                if (old_node_set)
                {
                    for (const auto& node_idx : *old_node_set)
                    {
                        const int local_idx = node_idx->getLocalPETScIndex();
                        double* const X = &X_data[local_idx][0];
                        const SAMRAICellIndex new_cell_idx = IndexUtilities::getCellIndex(X, grid_geom, ratio);
                        if (patch_box.contains(new_cell_idx))
                        {
                            ...
                            if (!periodic_node || unregistered_periodic_node)
                            {
                                ...
                                {
                                    const SAMRAIIntVector& periodic_offset = it_offset->second;
...
LDataManager::endDataRedistribution(const int coarsest_ln_in, const int finest_ln_in)
{
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        SAMRAIPointer<SAMRAIPatchLevel> level = d_hierarchy->getPatchLevel(level_number);
        SAMRAIPointer<SAMRAIBoxTree> box_tree = level->getBoxTree();
        const SAMRAIProcessorMapping& processor_mapping = level->getProcessorMapping();
        const SAMRAIIntVector& ratio = level->getRatio();
        ...
        for (size_t k = 0; k < num_nodes; ++k)
        {
            ...
            const SAMRAICellIndex cell_idx = IndexUtilities::getCellIndex(posn, d_grid_geom, ratio);

            SAMRAIArray<int> indices;
            box_tree->findOverlapIndices(indices, SAMRAIBox(cell_idx, cell_idx));
#if !defined(NDEBUG)
        ...
        SAMRAISchedule lnode_idx_data_mover;
        std::vector<std::vector<SAMRAIPointer<SAMRAITransaction>>> transactions(
            num_procs, std::vector<SAMRAIPointer<SAMRAITransaction>>(num_procs));
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        SAMRAIPointer<std::vector<LNode>> new_local_and_ghost_nodes = new std::vector<LNode>(num_local_and_ghost_nodes);
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            const SAMRAIBox& ghost_box = idx_data->getGhostBox();
...
void
LDataManager::addWorkloadEstimate(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
...
LDataManager::updateNodeCountData(const int coarsest_ln_in, const int finest_ln_in)
{
    ...
    for (int level_number = coarsest_ln; level_number <= finest_ln; ++level_number)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            const SAMRAIBox& patch_box = patch->getBox();
            const SAMRAIPointer<LNodeSetData> idx_data = patch->getPatchData(d_lag_node_index_current_idx);
            SAMRAIPointer<SAMRAICellData<double>> node_count_data = patch->getPatchData(d_node_count_idx);
            node_count_data->fillAll(0.0);
            for (LNodeSetData::SetIterator it(*idx_data); it; it++)
            {
                const SAMRAIIndex& i = it.getIndex();
...
void
LDataManager::initializeLevelData(const SAMRAIPointer<SAMRAIBasePatchHierarchy> hierarchy,
                                  const int level_number,
                                  ...
                                  const bool allocate_data)
{
    ...
    SAMRAIBoxList boxes(level->getBoxes());
    ...
    {
        j = k + 1;
        SAMRAIBox tryme = boxes.getFirstItem();
        boxes.removeFirstItem();

        for (SAMRAIBoxList::Iterator ib(boxes); ib; ib++)
        {
...
void
LDataManager::resetHierarchyConfiguration(const SAMRAIPointer<SAMRAIBasePatchHierarchy> hierarchy,
                                          const int coarsest_ln,
                                          const int finest_ln)
{
    ...
    {
        SAMRAIPointer<SAMRAIPatchLevel> level = hierarchy->getPatchLevel(level_number);
        SAMRAIPointer<SAMRAIPatchLevel> coarser_level = hierarchy->getPatchLevel(level_number - 1);
...
void
LDataManager::applyGradientDetector(const SAMRAIPointer<SAMRAIBasePatchHierarchy> hierarchy,
                                    const int level_number,
                                    ...
                                    const bool /*uses_richardson_extrapolation_too*/)
{
    ...
    else if (hierarchy->finerLevelExists(level_number))
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            const SAMRAIPointer<SAMRAICellData<double>> node_count_data = patch->getPatchData(d_node_count_idx);
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            for (int ln = level_number + 1; ln <= d_finest_ln; ++ln)
            {
                for (const auto& bounding_box : d_displaced_strct_bounding_boxes[ln])
                {
                    ...
                    const SAMRAICellIndex bbox_lower = IndexUtilities::getCellIndex(X_lower, d_grid_geom, ratio);
                    const SAMRAICellIndex bbox_upper = IndexUtilities::getCellIndex(X_upper, d_grid_geom, ratio);
                    const SAMRAIBox tag_box(bbox_lower, bbox_upper);
...
void
LDataManager::putToDatabase(SAMRAIPointer<SAMRAIDatabase> db)
{
    ...
    for (int level_number = d_coarsest_ln; level_number <= d_finest_ln; ++level_number)
    {
        ...
        SAMRAIPointer<SAMRAIDatabase> level_db = db->putDatabase(level_db_name);
      ...
      d_ghost_width(std::move(ghost_width))
{
    ...
    SAMRAIPointer<SAMRAIRefineOperator> lag_node_index_bdry_fill_op = nullptr;
    d_lag_node_index_bdry_fill_alg = new SAMRAIRefineAlgorithm();
    ...
    d_node_count_var = new SAMRAICellVariable<double>(d_object_name + "::node_count");
    ...
    SAMRAIPointer<SAMRAICoarsenOperator> node_count_coarsen_op = new SAMRAICartesianCellDoubleWeightedAverage();
    d_node_count_coarsen_alg = new SAMRAICoarsenAlgorithm();
    d_node_count_coarsen_alg->registerCoarsen(d_node_count_idx, d_node_count_idx, node_count_coarsen_op);

    // Setup Timers.
    IBTK_DO_ONCE(t_spread = SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::spread()");
                 t_interp = SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::interp()");
                 t_map_lagrangian_to_petsc =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::mapLagrangianToPETSc()");
                 t_map_petsc_to_lagrangian =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::mapPETScToLagrangian()");
                 t_begin_data_redistribution =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::beginDataRedistribution()");
                 t_end_data_redistribution =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::endDataRedistribution()");
                 t_update_workload_estimates =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::updateWorkloadEstimates()");
                 t_update_node_count_data =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::updateNodeCountData()");
                 t_initialize_level_data =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::initializeLevelData()");
                 t_reset_hierarchy_configuration =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::resetHierarchyConfiguration()");
                 t_apply_gradient_detector =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::applyGradientDetector()");
                 t_put_to_database = SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::putToDatabase()");
                 t_begin_nonlocal_data_fill =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::beginNonlocalDataFill()");
                 t_end_nonlocal_data_fill =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::endNonlocalDataFill()");
                 t_compute_node_distribution =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::computeNodeDistribution()");
                 t_compute_node_offsets =
                     SAMRAITimerManager::getManager()->getTimer("IBTK::LDataManager::computeNodeOffsets()"););
    return;
} // LDataManager
                                      ...
                                      const int level_number)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(level); p; p++)
    {
        ...
        const SAMRAIBox& patch_box = patch->getBox();
        const SAMRAIPointer<LNodeSetData> idx_data = patch->getPatchData(d_lag_node_index_current_idx);
        SAMRAIBoxList ghost_boxes = idx_data->getGhostBox();
        ghost_boxes.removeIntersections(patch_box);
        for (SAMRAIBoxList::Iterator bl(ghost_boxes); bl; bl++)
        {
...
LDataManager::getFromRestart()
{
    ...
    {
        const std::string level_db_name = "level_" + std::to_string(level_number);
        SAMRAIPointer<SAMRAIDatabase> level_db = db->getDatabase(level_db_name);
