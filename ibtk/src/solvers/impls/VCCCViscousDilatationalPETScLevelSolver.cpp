...
VCCCViscousDilatationalPETScLevelSolver::VCCCViscousDilatationalPETScLevelSolver(std::string object_name,
                                                                                 Pointer<Database> input_db,
                                                                                 std::string default_options_prefix)
    : CCPoissonPETScLevelSolver(std::move(object_name), input_db, std::move(default_options_prefix))
{
...
VCCCViscousDilatationalPETScLevelSolver::~VCCCViscousDilatationalPETScLevelSolver()
{
...
void
VCCCViscousDilatationalPETScLevelSolver::initializeSolverStateSpecialized(const SAMRAIVectorReal<NDIM, double>& x,
                                                                          const SAMRAIVectorReal<NDIM, double>& /*b*/)
{
    ...
    IBTK_CHKERRQ(ierr);
    PETScMatUtilities::constructPatchLevelVCCCViscousDilatationalOp(d_petsc_mat,
                                                                    d_problem_spec,
                                                                    d_bc_coefs,
                                                                    d_solution_time,
                                                                    d_num_dofs_per_proc,
...
void
VCCCViscousDilatationalPETScLevelSolver::setupKSPVecs(Vec& petsc_x,
                                                      Vec& petsc_b,
                                                      ...
                                                      SAMRAIVectorReal<NDIM, double>& b)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(d_level); p; p++)
    {
        ...
        if (at_physical_bdry)
        {
            PoissonUtilities::adjustVCCCViscousDilatationalOpRHSAtPhysicalBoundary(*b_adj_data,
                                                                                   patch,
                                                                                   mu_idx,
                                                                                   lambda_idx,
                                                                                   d_bc_coefs,
        ...
        if (at_cf_bdry)
        {
            PoissonUtilities::adjustVCCCViscousDilatationalOpRHSAtCoarseFineBoundary(
                *b_adj_data, *x_data, patch, mu_idx, lambda_idx, type_1_cf_bdry, d_mu_interp_type);
        }
    }
...
void
VCCCViscousDilatationalPETScLevelSolver::setShearViscosityInterpolationType(const IBTK::VCInterpType mu_interp_type)
{
