                                                     ...
                                                     std::string default_options_prefix)
{
    ...
    d_dof_index_var = new SideVariableNd<int>(object_name + "::dof_index");
...
void
SCPoissonPETScLevelSolver::initializeSolverStateSpecialized(const SAMRAIVectorRealNd<double>& x,
                                                            const SAMRAIVectorRealNd<double>& /*b*/)
{
    // Allocate DOF index data.
    VariableDatabaseNd* var_db = VariableDatabaseNd::getDatabase();
    const int x_idx = x.getComponentDescriptorIndex(0);
    SAMRAIPointer<SideDataFactoryNd<double> > x_fac = var_db->getPatchDescriptor()->getPatchDataFactory(x_idx);
    const int depth = x_fac->getDefaultDepth();
    SAMRAIPointer<SideDataFactoryNd<int> > dof_index_fac =
...
void
SCPoissonPETScLevelSolver::copyToPETScVec(Vec& petsc_x, SAMRAIVectorRealNd<double>& x)
{
    const int x_idx = x.getComponentDescriptorIndex(0);
...
void
SCPoissonPETScLevelSolver::copyFromPETScVec(Vec& petsc_x, SAMRAIVectorRealNd<double>& x)
{
    const int x_idx = x.getComponentDescriptorIndex(0);
                                        ...
                                        SAMRAIVectorRealNd<double>& b)
{
    ...
    for (PatchLevelNd::Iterator p(d_level); p; p++)
    {
        ...
        SAMRAIPointer<SideDataNd<double> > b_adj_data = patch->getPatchData(b_adj_idx);
