...
VCCCViscousDilatationalOperator::VCCCViscousDilatationalOperator(std::string object_name, const bool homogeneous_bc)
    : CCLaplaceOperator(std::move(object_name), homogeneous_bc)
{
    ...
    // Setup Timers.
    IBTK_DO_ONCE(t_apply = TimerManager::getManager()->getTimer("IBTK::VCCCViscousDilatationalOperator::apply()");
                 t_initialize_operator_state = TimerManager::getManager()->getTimer(
                     "IBTK::VCCCViscousDilatationalOperator::initializeOperatorState()");
                 t_deallocate_operator_state = TimerManager::getManager()->getTimer(
                     "IBTK::VCCCViscousDilatationalOperator::deallocateOperatorState()"););
...
VCCCViscousDilatationalOperator::~VCCCViscousDilatationalOperator()
{
...
void
VCCCViscousDilatationalOperator::apply(SAMRAIVectorReal<NDIM, double>& x, SAMRAIVectorReal<NDIM, double>& y)
{
    ...
    for (int comp = 0; comp < d_ncomp; ++comp)
    {
        ...
        if (x_depth != NDIM || y_depth != NDIM)
        {
            TBOX_ERROR(d_object_name << "::apply()\n"
                                     << "  each vector component must have data depth == " << NDIM << std::endl);
        }
    }
#endif
    ...
    for (int comp = 0; comp < d_ncomp; ++comp)
    {
        ...
        d_hier_math_ops->vc_laplace(y_idx,
                                    ...
                                    d_solution_time,
                                    d_D_interp_type,
                                    !(vc_op_spec.d_C_is_const) ? vc_op_spec.d_C_idx : IBTK::invalid_index);

        if (vc_op_spec.d_L_idx > 0)
        {
            d_hier_math_ops->vc_dilatational(y_idx,
                                             y_cc_var,
                                             1.0,
                                             vc_op_spec.d_L_idx,
                                             Pointer<CellVariable<NDIM, double> >(nullptr),
...
void
VCCCViscousDilatationalOperator::initializeOperatorState(const SAMRAIVectorReal<NDIM, double>& in,
...
void
VCCCViscousDilatationalOperator::deallocateOperatorState()
{
...
void
VCCCViscousDilatationalOperator::setDPatchDataInterpolationType(const IBTK::VCInterpType D_interp_type)
{
