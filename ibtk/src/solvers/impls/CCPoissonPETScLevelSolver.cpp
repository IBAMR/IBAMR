                                                     ...
                                                     std::string default_options_prefix)
{
    ...
    d_dof_index_var = new SAMRAICellVariable<int>(object_name + "::dof_index");
...
void
CCPoissonPETScLevelSolver::initializeSolverStateSpecialized(const SAMRAISAMRAIVectorReal<double>& x,
                                                            const SAMRAISAMRAIVectorReal<double>& /*b*/)
{
    // Allocate DOF index data.
    SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
    const int x_idx = x.getComponentDescriptorIndex(0);
    Pointer<SAMRAICellDataFactory<double>> x_fac = var_db->getPatchDescriptor()->getPatchDataFactory(x_idx);
    const int depth = x_fac->getDefaultDepth();
    Pointer<SAMRAICellDataFactory<int>> dof_index_fac =
...
void
CCPoissonPETScLevelSolver::copyToPETScVec(Vec& petsc_x, SAMRAISAMRAIVectorReal<double>& x)
{
    const int x_idx = x.getComponentDescriptorIndex(0);
...
void
CCPoissonPETScLevelSolver::copyFromPETScVec(Vec& petsc_x, SAMRAISAMRAIVectorReal<double>& x)
{
    const int x_idx = x.getComponentDescriptorIndex(0);
                                        ...
                                        SAMRAISAMRAIVectorReal<double>& b)
{
    ...
    {
        Pointer<SAMRAIPatch> patch = d_level->getPatch(p());
        Pointer<SAMRAIPatchGeometry> pgeom = patch->getPatchGeometry();
        Pointer<SAMRAICellData<double>> x_data = patch->getPatchData(x_idx);
        Pointer<SAMRAICellData<double>> b_data = patch->getPatchData(b_idx);
        Pointer<SAMRAICellData<double>> b_adj_data = patch->getPatchData(b_adj_idx);
