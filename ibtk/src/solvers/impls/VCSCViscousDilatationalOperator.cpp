...
VCSCViscousDilatationalOperator::VCSCViscousDilatationalOperator(std::string object_name, const bool homogeneous_bc)
    : SCLaplaceOperator(std::move(object_name), homogeneous_bc)
{
    ...
    // Setup Timers.
    IBTK_DO_ONCE(t_apply = TimerManager::getManager()->getTimer("IBTK::VCSCViscousDilatationalOperator::apply()");
                 t_initialize_operator_state = TimerManager::getManager()->getTimer(
                     "IBTK::VCSCViscousDilatationalOperator::initializeOperatorState()");
                 t_deallocate_operator_state = TimerManager::getManager()->getTimer(
                     "IBTK::VCSCViscousDilatationalOperator::deallocateOperatorState()"););
...
VCSCViscousDilatationalOperator::~VCSCViscousDilatationalOperator()
{
...
void
VCSCViscousDilatationalOperator::apply(SAMRAIVectorReal<NDIM, double>& x, SAMRAIVectorReal<NDIM, double>& y)
{
    ...
    for (int comp = 0; comp < d_ncomp; ++comp)
    {
        ...
        d_hier_math_ops->vc_laplace(y_idx,
                                    ...
                                    d_solution_time,
                                    d_D_interp_type,
                                    !(vc_op_spec.d_C_is_const) ? vc_op_spec.d_C_idx : -1);
...
void
VCSCViscousDilatationalOperator::initializeOperatorState(const SAMRAIVectorReal<NDIM, double>& in,
...
void
VCSCViscousDilatationalOperator::deallocateOperatorState()
{
...
void
VCSCViscousDilatationalOperator::setDPatchDataInterpolationType(const IBTK::VCInterpType D_interp_type)
{
