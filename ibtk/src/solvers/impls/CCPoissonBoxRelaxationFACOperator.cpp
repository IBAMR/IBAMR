...
static SAMRAITimer* t_solve_coarsest_level;
static SAMRAITimer* t_compute_residual;
...
CCPoissonBoxRelaxationFACOperator::CCPoissonBoxRelaxationFACOperator(const std::string& object_name,
    ...
    : PoissonFACPreconditionerStrategy(
          object_name,
          new SAMRAICellVariable<double>(object_name + "::cell_scratch", DEFAULT_DATA_DEPTH),
          CELLG,
          input_db,
          std::move(default_options_prefix))
{
    ...
    d_coarse_solver_db = new SAMRAIMemoryDatabase(object_name + "::coarse_solver_db");
    ...
    IBTK_DO_ONCE(
        t_smooth_error =
            SAMRAITimerManager::getManager()->getTimer("IBTK::CCPoissonBoxRelaxationFACOperator::smoothError()");
        t_solve_coarsest_level =
            SAMRAITimerManager::getManager()->getTimer("IBTK::CCPoissonBoxRelaxationFACOperator::solveCoarsestLevel()");
        t_compute_residual =
            SAMRAITimerManager::getManager()->getTimer("IBTK::CCPoissonBoxRelaxationFACOperator::computeResidual()"););
    return;
} // CCPoissonBoxRelaxationFACOperator
...
void
CCPoissonBoxRelaxationFACOperator::smoothError(SAMRAISAMRAIVectorReal<double>& error,
                                               ...
                                               bool /*performing_post_sweeps*/)
{
    ...
    if (level_num > d_coarsest_ln && num_sweeps > 1)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++, ++patch_counter)
        {
            ...
            scratch_data->getArrayData().copy(
                error_data->getArrayData(), d_patch_bc_box_overlap[level_num][patch_counter], SAMRAIIntVector(0));
        }
    }
    ...
    for (int isweep = 0; isweep < num_sweeps; ++isweep)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++, ++patch_counter)
        {
            SAMRAIPointer<SAMRAIPatch> patch = level->getPatch(p());
            SAMRAIPointer<SAMRAICellData<double> > error_data = error.getComponentPatchData(0, *patch);
            SAMRAIPointer<SAMRAICellData<double> > residual_data = residual.getComponentPatchData(0, *patch);
#if !defined(NDEBUG)
            const SAMRAIBox& ghost_box = error_data->getGhostBox();
            ...
            {
                const std::map<int, SAMRAIBox> neighbor_overlap = d_patch_neighbor_overlap[level_num][patch_counter];
                for (const auto& pair : neighbor_overlap)
                {
                    ...
                    SAMRAIPointer<SAMRAICellData<double> > src_error_data = error.getComponentPatchData(0, *src_patch);
                    error_data->getArrayData().copy(src_error_data->getArrayData(), overlap, SAMRAIIntVector(0));
                }
            }
...
bool
CCPoissonBoxRelaxationFACOperator::solveCoarsestLevel(SAMRAISAMRAIVectorReal<double>& error,
                                                      const SAMRAISAMRAIVectorReal<double>& residual,
                                                      int coarsest_ln)
{
...
void
CCPoissonBoxRelaxationFACOperator::computeResidual(SAMRAISAMRAIVectorReal<double>& residual,
                                                   ...
                                                   int finest_level_num)
{
    ...
    const SAMRAIPointer<SAMRAICellVariable<double> > rhs_var = rhs.getComponentVariable(0);

    // Fill ghost-cell values.
    using InterpolationTransactionComponent = HierarchyGhostCellInterpolation::InterpolationTransactionComponent;
    SAMRAIPointer<CellNoCornersFillPattern> fill_pattern =
        new CellNoCornersFillPattern(CELLG, /*overwrite_interior*/ false);
    ...
    SAMRAIHierarchyCellDataOpsReal<double> hier_cc_data_ops(d_hierarchy, coarsest_level_num, finest_level_num);
...
void
CCPoissonBoxRelaxationFACOperator::initializeOperatorStateSpecialized(const SAMRAISAMRAIVectorReal<double>& solution,
                                                                      const SAMRAISAMRAIVectorReal<double>& rhs,
                                                                      ...
                                                                      const int finest_reset_ln)
{
    ...
    for (int ln = coarsest_reset_ln; ln <= finest_reset_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++, ++patch_counter)
        {
            ...
            const SAMRAIBox& ghost_box = SAMRAIBox::grow(patch_box, d_gcw);
    ...
    for (int ln = coarsest_reset_ln; ln <= finest_reset_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++, ++patch_counter)
        {
            ...
            d_patch_bc_box_overlap[ln][patch_counter] = SAMRAIBoxList(ghost_box);
    ...
    for (int ln = coarsest_reset_ln; ln <= finest_reset_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p1(level); p1; p1++, ++patch_counter1)
        {
            d_patch_neighbor_overlap[ln][patch_counter1].clear();
            SAMRAIPointer<SAMRAIPatch> dst_patch = level->getPatch(p1());
            const SAMRAIBox& dst_patch_box = dst_patch->getBox();
            const SAMRAIBox& dst_ghost_box = SAMRAIBox::grow(dst_patch_box, 1);
            int patch_counter2 = 0;
            for (SAMRAIPatchLevel::Iterator p2(level); patch_counter2 < patch_counter1; p2++, ++patch_counter2)
            {
                SAMRAIPointer<SAMRAIPatch> src_patch = level->getPatch(p2());
                const SAMRAIBox& src_patch_box = src_patch->getBox();
                const SAMRAIBox overlap = dst_ghost_box * src_patch_box;
                if (!overlap.empty())
                {
                                                             ...
                                                             const SAMRAIIntVector& ghost_cell_width)
{
    ...
    {
        C_data = new SAMRAICellData<double>(patch_box, 1, no_ghosts);
    ...
    {
        D_data = new SAMRAISideData<double>(patch_box, 1, no_ghosts);
...
void
CCPoissonBoxRelaxationFACOperator::buildPatchLaplaceOperator_aligned(
    Mat& A,
    const SAMRAIPointer<SAMRAICellData<double> > C_data,
    const SAMRAIPointer<SAMRAISideData<double> > D_data,
    const SAMRAIPointer<SAMRAIPatch> patch,
    const SAMRAIIntVector& ghost_cell_width)
{
    int ierr;
    ...
    const SAMRAIBox& patch_box = patch->getBox();
    const SAMRAIBox& ghost_box = SAMRAIBox::grow(patch_box, ghost_cell_width);
    const int size = ghost_box.size();

    static const int stencil_sz = 2 * NDIM + 1;

    SAMRAIBoxList ghost_boxes(ghost_box);
...
void
CCPoissonBoxRelaxationFACOperator::buildPatchLaplaceOperator_nonaligned(
    Mat& A,
    const SAMRAIPointer<SAMRAICellData<double> > C_data,
    const SAMRAIPointer<SAMRAISideData<double> > D_data,
    const SAMRAIPointer<SAMRAIPatch> patch,
...
// Set some general matrix options.
    ...
    std::vector<int> mat_stencil(stencil_sz);
    std::map<SAMRAIIndex, int, IndexComp> stencil_indices;
    int stencil_index = 0;
    static const int x_axis = 0;
#if (NDIM == 3)
    static const int y_axis = 1;
...
#endif
#if (NDIM == 2)
                    const SAMRAIIndex i(x_offset, y_offset);
#endif
#if (NDIM == 3)
                    const SAMRAIIndex i(x_offset, y_offset, z_offset);
#endif
    ...
    {
        const SAMRAIIndex& i = b();
        static const SAMRAIIndex i_stencil_center(0);
        ...
        for (unsigned int norm_axis = 0; norm_axis < NDIM; ++norm_axis)
        {
            ...
            for (unsigned int trans_axis = 0; trans_axis < NDIM; ++trans_axis)
            {
                ...
                {
                    const SAMRAISideIndex iupper(i, norm_axis, SAMRAISideIndex::Upper);
