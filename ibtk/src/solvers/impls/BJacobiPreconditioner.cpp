...
bool
BJacobiPreconditioner::solveSystem(SAMRAISAMRAIVectorReal<double>& x, SAMRAISAMRAIVectorReal<double>& b)
{
...
// Zero out the initial guess.
    ...
    for (int comp = 0; comp < x.getNumberOfComponents(); ++comp)
    {
        ...
        // component.
        SAMRAISAMRAIVectorReal<double> x_comp(
        ...
        SAMRAISAMRAIVectorReal<double> b_comp(
...
void
BJacobiPreconditioner::initializeSolverState(const SAMRAISAMRAIVectorReal<double>& x,
                                             const SAMRAISAMRAIVectorReal<double>& b)
{
    ...
    for (const auto& linearSolver_pair : d_pc_map)
    {
        ...
        SAMRAISAMRAIVectorReal<double> x_comp(x_name + "_component", hierarchy, coarsest_ln, finest_ln);
        x_comp.addComponent(
            x.getComponentVariable(comp), x.getComponentDescriptorIndex(comp), x.getControlVolumeIndex(comp));
        SAMRAISAMRAIVectorReal<double> b_comp(b_name + "_component", hierarchy, coarsest_ln, finest_ln);
        b_comp.addComponent(
            b.getComponentVariable(comp), b.getComponentDescriptorIndex(comp), b.getControlVolumeIndex(comp));
