...
compute_linear_axis_stencil(const int fine_index,
                            ...
                            const int ratio,
                            const bool periodic,
                            std::array<int, 2>& coarse_stencil,
...
// The nonzero entries of one row of the cell-centered linear prolongation matrix. A cell has at most one
// entry per corner of its coarse stencil.
struct LinearProlongationRow
{
    std::array<int, 1 << NDIM> cols;
    std::array<PetscScalar, 1 << NDIM> vals;
    int count = 0;
};

void
count_row_nnz(const LinearProlongationRow& row,
              const int local_row,
                                                      ...
                                                      const int coarse_ao_offset)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(fine_patch_level); p; p++)
    {
        ...
        for (Box<NDIM>::Iterator b(CellGeometry<NDIM>::toCellBox(fine_patch_box)); b; b++)
        {
            ...
            for (unsigned d = 0; d < depth; ++d)
            {
                const LinearProlongationRow row = compute_linear_cell_prolongation_row_entries(i_fine,
                                                                                               d,
                                                                                               fine_coarse_ratio,
                                                                                               coarse_domain_lower,
                                                                                               coarse_domain_upper,
                                                                                               ...
                                                                                               coarse_level_ao,
                                                                                               coarse_ao_offset);
                count_row_nnz(row, local_row[d], j_coarse_lower, j_coarse_upper, n_local, n_total, d_nnz, o_nnz);
            }
        }
    }
    ...
    for (PatchLevel<NDIM>::Iterator p(fine_patch_level); p; p++)
    {
        ...
        for (Box<NDIM>::Iterator b(CellGeometry<NDIM>::toCellBox(fine_patch_box)); b; b++)
        {
            ...
            for (unsigned d = 0; d < depth; ++d)
            {
                LinearProlongationRow row = compute_linear_cell_prolongation_row_entries(i_fine,
                                                                                         d,
                                                                                         fine_coarse_ratio,
                                                                                         coarse_domain_lower,
                                                                                         coarse_domain_upper,
                                                                                         ...
                                                                                         coarse_level_ao,
                                                                                         coarse_ao_offset);
                ierr = MatSetValues(mat, 1, &rows[d], row.count, row.cols.data(), row.vals.data(), INSERT_VALUES);
                IBTK_CHKERRQ(ierr);
            }
        }
    }
