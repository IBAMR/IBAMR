...
#endif
        Pointer<PatchNd> patch = patch_level->getPatch(patch_num[k]);
        Pointer<SideDataNd<int> > dof_index_data = patch->getPatchData(dof_index_idx);
#if !defined(NDEBUG)
        TBOX_ASSERT(dof_index_data->getDepth() == 1);
#endif
                                                            ...
                                                            const int coarse_ao_offset)
{
    ...
    {
        Pointer<PatchNd> fine_patch = fine_patch_level->getPatch(p());
        const BoxNd& fine_patch_box = fine_patch->getBox();
        Pointer<CellDataNd<int> > dof_fine_data = fine_patch->getPatchData(dof_index_idx);
                                                   ...
                                                   const int coarse_ao_offset)
{
    ...
    Pointer<CartesianGridGeometryNd> grid_geom = coarse_patch_level->getGridGeometry();
    ...
    for (PatchLevelNd::Iterator p(fine_patch_level); p; p++)
    {
        Pointer<PatchNd> fine_patch = fine_patch_level->getPatch(p());
        const BoxNd& fine_patch_box = fine_patch->getBox();
        Pointer<SideDataNd<int> > fine_dof_data = fine_patch->getPatchData(dof_index_idx);
                                                         ...
                                                         Pointer<CoarseFineBoundaryNd> /*cf_boundary*/)
{
    ...
    {
        Pointer<PatchNd> patch = patch_level->getPatch(p());
        Pointer<CellDataNd<int> > dof_data = patch->getPatchData(dof_index_idx);
                                                         ...
                                                         Pointer<CoarseFineBoundaryNd> cf_boundary)
{
    ...
    for (PatchLevelNd::Iterator p(patch_level); p; p++, ++patch_counter)
    {
        ...
        Pointer<SideDataNd<int> > dof_data = patch->getPatchData(dof_index_idx);
#if !defined(NDEBUG)
        {
            const int data_depth = dof_data->getDepth();
