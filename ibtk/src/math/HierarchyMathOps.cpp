...
HierarchyMathOps::HierarchyMathOps(std::string name,
    ...
    : d_object_name(std::move(name)),
      ...
      d_fc_var(new SAMRAIFaceVariable<double>(d_object_name + "::scratch_fc")),
      d_nc_s_var(new SAMRAINodeVariable<double>(d_object_name + "::scratch_nc_s", 1)),
      d_nc_v_var(new SAMRAINodeVariable<double>(d_object_name + "::scratch_nc_v", NDIM)),
      d_sc_var(new SAMRAISideVariable<double>(d_object_name + "::scratch_sc")),
      d_ec_var(new SAMRAIEdgeVariable<double>(d_object_name + "::scratch_ec")),
      d_oe_var(new SAMRAIOuteredgeVariable<double>(d_object_name + "::scratch_oe")),
      d_of_var(new SAMRAIOuterfaceVariable<double>(d_object_name + "::scratch_of")),
      d_on_s_var(new SAMRAIOuternodeVariable<double>(d_object_name + "::scratch_on_s", 1)),
      d_on_v_var(new SAMRAIOuternodeVariable<double>(d_object_name + "::scratch_on_v", NDIM)),
      d_os_var(new SAMRAIOutersideVariable<double>(d_object_name + "::scratch_os")),
      d_coarsen_op_name(coarsen_op_name),
      d_wgt_cc_var(new SAMRAICellVariable<double>(d_object_name + "::wgt_cc", 1)),
      d_wgt_fc_var(new SAMRAIFaceVariable<double>(d_object_name + "::wgt_fc", 1)),
      d_wgt_sc_var(new SAMRAISideVariable<double>(d_object_name + "::wgt_sc", 1))
{
    ...
    static const SAMRAIIntVector no_ghosts = 0;
    d_fc_var->setPatchDataFactory(new SAMRAIFaceDataFactory<double>(1, no_ghosts, fine_boundary_represents_var));
    d_nc_s_var->setPatchDataFactory(new SAMRAINodeDataFactory<double>(1, no_ghosts, fine_boundary_represents_var));
    d_nc_v_var->setPatchDataFactory(new SAMRAINodeDataFactory<double>(NDIM, no_ghosts, fine_boundary_represents_var));
    d_sc_var->setPatchDataFactory(new SAMRAISideDataFactory<double>(1, no_ghosts, fine_boundary_represents_var));
    d_ec_var->setPatchDataFactory(new SAMRAIEdgeDataFactory<double>(1, no_ghosts, fine_boundary_represents_var));

    d_of_var->setPatchDataFactory(new SAMRAIOuterfaceDataFactory<double>(1));
    d_on_s_var->setPatchDataFactory(new SAMRAIOuternodeDataFactory<double>(1));
    d_on_v_var->setPatchDataFactory(new SAMRAIOuternodeDataFactory<double>(NDIM));
    d_os_var->setPatchDataFactory(new SAMRAIOutersideDataFactory<double>(1));
    d_oe_var->setPatchDataFactory(new SAMRAIOuteredgeDataFactory<double>(1));

    static const SAMRAIIntVector ghosts = 1;
...
void
HierarchyMathOps::setPatchHierarchy(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy)
{
#if !defined(NDEBUG)
...
HierarchyMathOps::resetLevels(const int coarsest_ln, const int finest_ln)
{
    ...
    {
        SAMRAIPointer<SAMRAIPatchLevel> src_level = d_hierarchy->getPatchLevel(dst_ln + 1);
        SAMRAIPointer<SAMRAIPatchLevel> dst_level = d_hierarchy->getPatchLevel(dst_ln);
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            if (alpha_data->getDepth() > 1)
            {
                ...
                for (unsigned int axis = 0; axis < NDIM; ++axis)
                {
                    ...
                    SAMRAIBox boundary_box = SAMRAIBox::grow(patch_box, gcw);
                          ...
                          const int src2_depth)
{
    ...
    {
        SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
        SAMRAIPointer<SAMRAIVariable> dummy_var;
...
HierarchyMathOps::enforceHangingNodeConstraints(const int dst_idx, SAMRAIPointer<SAMRAINodeVariable<double>> dst_var)
{
    ...
    auto boundary_to_nodal = [](const SAMRAIBoundaryBox& bbox)
    {
        ...
        SAMRAIBox node_box = SAMRAINodeGeometry::toNodeBox(bbox.getBox());
    ...
    for (int ln = d_finest_ln; ln > d_coarsest_ln; --ln)
    {
        SAMRAICoarseFineBoundary cf_boundary(*d_hierarchy, ln, 0);
        SAMRAIPointer<SAMRAIPatchLevel> level = d_hierarchy->getPatchLevel(ln);
            ...
            for (int k = 0; k < bboxes.size(); ++k)
            {
                ...
                for (int d = 0; d < dst_data.getDepth(); ++d)
                {
                    ...
                    for (int i = lower(coordinate); i < upper(coordinate); i += ratio(coordinate))
                    {
                        ...
                        SAMRAINodeIndex n_left;
                        n_left(constant) = lower(constant);
                        n_left(coordinate) = i;
                        SAMRAINodeIndex n_right = n_left;
                        ...
                        {
                            SAMRAINodeIndex n_current = n_left;
                    ...
                    SAMRAIIntVector inc0(0);
                    inc0(coordinate0) = ratio(coordinate0);
                    SAMRAIIntVector inc1(0);
                    ...
                    for (int i = lower(coordinate0); i < upper(coordinate0); i += ratio(coordinate0))
                    {
                        for (int j = lower(coordinate1); j < upper(coordinate1); j += ratio(coordinate1))
                        {
                            ...
                            for (int ii = 0; ii <= ratio(coordinate0); ++ii)
                            {
                                ...
                                {
                                    SAMRAINodeIndex n_current = n_lower;
...
HierarchyMathOps::resetCoarsenOperators()
{
    ...
    d_of_coarsen_alg = new SAMRAICoarsenAlgorithm();
    d_of_coarsen_alg->registerCoarsen(d_fc_idx, // destination
                                      d_of_idx, // source
                                      d_of_coarsen_op);

    d_on_s_coarsen_alg = new SAMRAICoarsenAlgorithm();
    d_on_v_coarsen_alg = new SAMRAICoarsenAlgorithm();
    ...
    d_os_coarsen_alg = new SAMRAICoarsenAlgorithm();
    d_os_coarsen_alg->registerCoarsen(d_sc_idx, // destination
                                      d_os_idx, // source
                                      d_os_coarsen_op);

    d_oe_coarsen_alg = new SAMRAICoarsenAlgorithm();
...
HierarchyMathOps::resetCellWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        if (ln < d_finest_ln)
        {
            SAMRAIPointer<SAMRAIPatchLevel> next_finer_level = d_hierarchy->getPatchLevel(ln + 1);
            refined_region_boxes = next_finer_level->getBoxes();
            refined_region_boxes.coarsen(next_finer_level->getRatioToCoarserLevel());
        }

        const SAMRAIIntVector max_gcw(1);
        const SAMRAICoarseFineBoundary cf_bdry(*d_hierarchy, ln, max_gcw);
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            if (ln < d_finest_ln)
            {
                const SAMRAIIntVector& periodic_shift = d_grid_geom->getPeriodicShift(level->getRatio());
                for (int i = 0; i < refined_region_boxes.getNumberOfBoxes(); ++i)
                {
                    for (unsigned int axis = 0; axis < NDIM; ++axis)
                    {
                        if (periodic_shift(axis) != 0)
                        {
                            ...
                            {
                                SAMRAIIntVector periodic_offset = 0;
                                periodic_offset(axis) = sgn * periodic_shift(axis);
                                const SAMRAIBox refined_box =
                                    SAMRAIBox::shift(refined_region_boxes[i], periodic_offset);
...
HierarchyMathOps::resetFaceWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                SAMRAIBox face_lower_box = SAMRAIFaceGeometry::toFaceBox(patch_box, axis);
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                SAMRAIBox face_upper_box = SAMRAIFaceGeometry::toFaceBox(patch_box, axis);
            ...
            if (ln > d_coarsest_ln)
            {
                ...
                const SAMRAIArray<SAMRAIBoundaryBox>& cf_bdry_boxes = cf_bdry.getBoundaries(p(), bdry_type);
                for (int k = 0; k < cf_bdry_boxes.getSize(); ++k)
                {
                    const SAMRAIBox& bdry_box = cf_bdry_boxes[k].getBox();
                    ...
                    if (!pgeom->getTouchesRegularBoundary(axis, lower_upper))
                    {
                        ...
                        SAMRAIBox face_bdry_box = SAMRAIFaceGeometry::toFaceBox(bdry_box, axis);
                        array_ops.addScalar(
...
HierarchyMathOps::resetSideWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                SAMRAIBox side_lower_box = SAMRAISideGeometry::toSideBox(patch_box, axis);
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                SAMRAIBox side_upper_box = SAMRAISideGeometry::toSideBox(patch_box, axis);
            ...
            if (ln > d_coarsest_ln)
            {
                ...
                for (int k = 0; k < cf_bdry_boxes.getSize(); ++k)
                {
                    ...
                    {
                        const double extra_vol = 0.5 * static_cast<double>(ratio(axis)) * cell_vol;
                        SAMRAIBox side_bdry_box = SAMRAISideGeometry::toSideBox(bdry_box, axis);
                        array_ops.addScalar(
