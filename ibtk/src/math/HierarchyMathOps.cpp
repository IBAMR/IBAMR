...
HierarchyMathOps::HierarchyMathOps(std::string name,
    ...
    : d_object_name(std::move(name)),
      ...
      d_fc_var(new FaceVariableNd<double>(d_object_name + "::scratch_fc")),
      d_nc_s_var(new NodeVariableNd<double>(d_object_name + "::scratch_nc_s", 1)),
      d_nc_v_var(new NodeVariableNd<double>(d_object_name + "::scratch_nc_v", NDIM)),
      d_sc_var(new SideVariableNd<double>(d_object_name + "::scratch_sc")),
      d_ec_var(new EdgeVariableNd<double>(d_object_name + "::scratch_ec")),
      d_oe_var(new OuteredgeVariableNd<double>(d_object_name + "::scratch_oe")),
      d_of_var(new OuterfaceVariableNd<double>(d_object_name + "::scratch_of")),
      d_on_s_var(new OuternodeVariableNd<double>(d_object_name + "::scratch_on_s", 1)),
      d_on_v_var(new OuternodeVariableNd<double>(d_object_name + "::scratch_on_v", NDIM)),
      d_os_var(new OutersideVariableNd<double>(d_object_name + "::scratch_os")),
      d_coarsen_op_name(coarsen_op_name),
      d_wgt_cc_var(new CellVariableNd<double>(d_object_name + "::wgt_cc", 1)),
      d_wgt_fc_var(new FaceVariableNd<double>(d_object_name + "::wgt_fc", 1)),
      d_wgt_sc_var(new SideVariableNd<double>(d_object_name + "::wgt_sc", 1))
{
    ...
    d_fc_var->setPatchDataFactory(new FaceDataFactoryNd<double>(1, no_ghosts, fine_boundary_represents_var));
    d_nc_s_var->setPatchDataFactory(new NodeDataFactoryNd<double>(1, no_ghosts, fine_boundary_represents_var));
    d_nc_v_var->setPatchDataFactory(new NodeDataFactoryNd<double>(NDIM, no_ghosts, fine_boundary_represents_var));
    d_sc_var->setPatchDataFactory(new SideDataFactoryNd<double>(1, no_ghosts, fine_boundary_represents_var));
    d_ec_var->setPatchDataFactory(new EdgeDataFactoryNd<double>(1, no_ghosts, fine_boundary_represents_var));

    d_of_var->setPatchDataFactory(new OuterfaceDataFactoryNd<double>(1));
    d_on_s_var->setPatchDataFactory(new OuternodeDataFactoryNd<double>(1));
    d_on_v_var->setPatchDataFactory(new OuternodeDataFactoryNd<double>(NDIM));
    d_os_var->setPatchDataFactory(new OutersideDataFactoryNd<double>(1));
    d_oe_var->setPatchDataFactory(new OuteredgeDataFactoryNd<double>(1));

    static const IntVectorNd ghosts = 1;
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            if (alpha_data->getDepth() > 1)
            {
                ...
                for (unsigned int axis = 0; axis < NDIM; ++axis)
                {
                    ...
                    BoxNd boundary_box = BoxNd::grow(patch_box, gcw);
...
HierarchyMathOps::enforceHangingNodeConstraints(const int dst_idx, Pointer<NodeVariableNd<double> > dst_var)
{
    ...
    auto boundary_to_nodal = [](const BoundaryBoxNd& bbox)
    {
        ...
        BoxNd node_box = NodeGeometryNd::toNodeBox(bbox.getBox());
    ...
    for (int ln = d_finest_ln; ln > d_coarsest_ln; --ln)
    {
        CoarseFineBoundaryNd cf_boundary(*d_hierarchy, ln, 0);
        Pointer<PatchLevelNd> level = d_hierarchy->getPatchLevel(ln);
...
HierarchyMathOps::resetCoarsenOperators()
{
    ...
    d_of_coarsen_alg = new CoarsenAlgorithmNd();
    d_of_coarsen_alg->registerCoarsen(d_fc_idx, // destination
                                      d_of_idx, // source
                                      d_of_coarsen_op);

    d_on_s_coarsen_alg = new CoarsenAlgorithmNd();
    d_on_v_coarsen_alg = new CoarsenAlgorithmNd();
    ...
    d_os_coarsen_alg = new CoarsenAlgorithmNd();
    d_os_coarsen_alg->registerCoarsen(d_sc_idx, // destination
                                      d_os_idx, // source
                                      d_os_coarsen_op);

    d_oe_coarsen_alg = new CoarsenAlgorithmNd();
...
HierarchyMathOps::resetCellWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        const IntVectorNd max_gcw(1);
        const CoarseFineBoundaryNd cf_bdry(*d_hierarchy, ln, max_gcw);
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            if (ln < d_finest_ln)
            {
                const IntVectorNd& periodic_shift = d_grid_geom->getPeriodicShift(level->getRatio());
                for (int i = 0; i < refined_region_boxes.getNumberOfBoxes(); ++i)
                {
                    for (unsigned int axis = 0; axis < NDIM; ++axis)
                    {
                        if (periodic_shift(axis) != 0)
                        {
                            ...
                            {
                                IntVectorNd periodic_offset = 0;
                                periodic_offset(axis) = sgn * periodic_shift(axis);
                                const BoxNd refined_box = BoxNd::shift(refined_region_boxes[i], periodic_offset);
...
HierarchyMathOps::resetFaceWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                BoxNd face_lower_box = FaceGeometryNd::toFaceBox(patch_box, axis);
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                BoxNd face_upper_box = FaceGeometryNd::toFaceBox(patch_box, axis);
            ...
            if (ln > d_coarsest_ln)
            {
                ...
                const Array<BoundaryBoxNd>& cf_bdry_boxes = cf_bdry.getBoundaries(p(), bdry_type);
                for (int k = 0; k < cf_bdry_boxes.getSize(); ++k)
                {
                    const BoxNd& bdry_box = cf_bdry_boxes[k].getBox();
                    ...
                    if (!pgeom->getTouchesRegularBoundary(axis, lower_upper))
                    {
                        ...
                        BoxNd face_bdry_box = FaceGeometryNd::toFaceBox(bdry_box, axis);
                        array_ops.addScalar(
...
HierarchyMathOps::resetSideWeights(const int coarsest_ln, const int finest_ln)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                BoxNd side_lower_box = SideGeometryNd::toSideBox(patch_box, axis);
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                BoxNd side_upper_box = SideGeometryNd::toSideBox(patch_box, axis);
            ...
            if (ln > d_coarsest_ln)
            {
                ...
                for (int k = 0; k < cf_bdry_boxes.getSize(); ++k)
                {
                    ...
                    {
                        const double extra_vol = 0.5 * static_cast<double>(ratio(axis)) * cell_vol;
                        BoxNd side_bdry_box = SideGeometryNd::toSideBox(bdry_box, axis);
                        array_ops.addScalar(
