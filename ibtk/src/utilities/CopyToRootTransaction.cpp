...
CopyToRootTransaction::CopyToRootTransaction(const int src_proc,
                                             ...
                                             Pointer<PatchDataNd> dst_patch_data)
    : d_src_proc(src_proc),
      d_dst_proc(dst_proc),
...
{
    Pointer<PatchDataFactoryNd> pdat_factory =
        d_patch_level->getPatchDescriptor()->getPatchDataFactory(d_src_patch_data_idx);

    Pointer<GridGeometryNd> grid_geom = d_patch_level->getGridGeometry();
#if !defined(NDEBUG)
    TBOX_ASSERT(grid_geom->getDomainIsSingleBox());
#endif
    const BoxNd& dst_box = grid_geom->getPhysicalDomain()[0];
    Pointer<BoxGeometryNd> dst_box_geometry = pdat_factory->getBoxGeometry(dst_box);
    ...
    for (PatchLevelNd::Iterator p(d_patch_level); p; p++)
    {
        ...
        Pointer<PatchNd> patch = d_patch_level->getPatch(src_patch_num);
        const BoxNd& src_box = patch->getBox();
        Pointer<BoxGeometryNd> src_box_geometry = pdat_factory->getBoxGeometry(src_box);
        const BoxNd& src_mask = dst_box;
        const bool overwrite_interior = true;
        const IntVectorNd src_shift = 0;
        Pointer<BoxOverlapNd> box_overlap =
