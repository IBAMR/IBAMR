                ...
                double tol)
{
    ...
    const SAMRAIPointer<SAMRAIPatchHierarchy>& snapshot_hierarchy = snapshot.second;
...
#endif
    SAMRAIPointer<SAMRAIGridGeometry> new_geometry =
        current_hierarchy->getGridGeometry()->makeRefinedGridGeometry("SnapshotGeometry", 1, false);
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        SAMRAIPointer<SAMRAIPatchLevel> current_level = current_hierarchy->getPatchLevel(ln);
        ...
        SAMRAIPointer<SAMRAIPatchLevel> new_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        SAMRAIPointer<SAMRAIRefineSchedule> schedule = refine_alg->createSchedule(new_level, current_level);
                           ...
                           const double tol)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        SAMRAIPointer<SAMRAIPatchLevel> cur_level = current_hierarchy->getPatchLevel(ln);
        SAMRAIPointer<SAMRAIPatchLevel> snp_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        // Now copy the data.
        SAMRAIPointer<SAMRAIRefineAlgorithm> refine_alg = new SAMRAIRefineAlgorithm();
        SAMRAIPointer<SAMRAICartesianGridGeometry> grid_geom = current_hierarchy->getGridGeometry();
    ...
    {
        SAMRAIPointer<SAMRAIPatchLevel> level = current_hierarchy->getPatchLevel(ln);
