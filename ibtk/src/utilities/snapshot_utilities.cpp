...
update_snapshot(SnapshotCache& cache,
                int u_idx,
                double time,
                SAMRAIPointer<SAMRAIPatchHierarchy> current_hierarchy,
                double tol)
{
    // Make sure we are on a stored snapshot.
    const std::pair<double, Pointer<SAMRAIPatchHierarchy>>& snapshot = cache.getSnapshot(time, tol);
    ...
    const Pointer<SAMRAIPatchHierarchy>& snapshot_hierarchy = snapshot.second;
...
#endif
    Pointer<SAMRAIGridGeometry> new_geometry =
        current_hierarchy->getGridGeometry()->makeRefinedGridGeometry("SnapshotGeometry", 1, false);
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        Pointer<SAMRAIPatchLevel> current_level = current_hierarchy->getPatchLevel(ln);
        ...
        Pointer<SAMRAIPatchLevel> new_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        Pointer<SAMRAIRefineSchedule> schedule = refine_alg->createSchedule(new_level, current_level);
...
fill_snapshot_on_hierarchy(SnapshotCache& cache,
                           const int u_idx,
                           const double time,
                           Pointer<SAMRAIPatchHierarchy> current_hierarchy,
                           ...
                           const double tol)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        Pointer<SAMRAIPatchLevel> cur_level = current_hierarchy->getPatchLevel(ln);
        Pointer<SAMRAIPatchLevel> snp_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        double allocated_time =
            cur_level->getPatch(*SAMRAIPatchLevel::Iterator(cur_level))->getPatchData(u_idx)->getTime();
        ...
        // Now copy the data.
        Pointer<SAMRAIRefineAlgorithm> refine_alg = new SAMRAIRefineAlgorithm();
        Pointer<SAMRAICartesianGridGeometry> grid_geom = current_hierarchy->getGridGeometry();
        ...
        Pointer<SAMRAIRefineSchedule> schedule =
    ...
    {
        Pointer<SAMRAIPatchLevel> level = current_hierarchy->getPatchLevel(ln);
