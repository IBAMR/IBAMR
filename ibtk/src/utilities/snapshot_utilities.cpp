                ...
                double tol)
{
    ...
    const std::pair<double, SAMRAIPointer<PatchHierarchyNd> >& snapshot = cache.getSnapshot(time, tol);
    ...
    const SAMRAIPointer<PatchHierarchyNd>& snapshot_hierarchy = snapshot.second;
...
#endif
    SAMRAIPointer<GridGeometryNd> new_geometry =
...
fill_snapshot_on_hierarchy(SnapshotCache& cache,
                           const int u_idx,
                           const double time,
                           SAMRAIPointer<PatchHierarchyNd> current_hierarchy,
                           ...
                           const double tol)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        SAMRAIPointer<PatchLevelNd> cur_level = current_hierarchy->getPatchLevel(ln);
        SAMRAIPointer<PatchLevelNd> snp_level = snapshot_hierarchy->getPatchLevel(ln);
    ...
    {
        SAMRAIPointer<PatchLevelNd> level = current_hierarchy->getPatchLevel(ln);
