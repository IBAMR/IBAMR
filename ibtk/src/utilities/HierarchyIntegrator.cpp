...
HierarchyIntegrator::HierarchyIntegrator(std::string object_name,
    ...
    : d_object_name(std::move(object_name))
{
    ...
    d_coarsen_algs[SYNCH_CURRENT_DATA_ALG] = new CoarsenAlgorithmNd();
    d_coarsen_algs[SYNCH_NEW_DATA_ALG] = new CoarsenAlgorithmNd();
    return;
} // HierarchyIntegrator
...
void
HierarchyIntegrator::initializePatchHierarchy(SAMRAIPointer<PatchHierarchyNd> hierarchy,
...
HierarchyIntegrator::updateWorkloadEstimates()
{
    if (d_workload_idx != IBTK::invalid_index)
    {
        ...
        HierarchyCellDataOpsRealNd<double> hier_cc_data_ops(d_hierarchy);
...
SAMRAIPointer<GriddingAlgorithmNd>
...
void
HierarchyIntegrator::registerLoadBalancer(SAMRAIPointer<LoadBalancerNd> load_balancer)
{
#if !defined(NDEBUG)
    ...
    if (d_workload_idx == IBTK::invalid_index)
    {
        d_workload_var = new CellVariableNd<double>(d_object_name + "::workload");
...
void
HierarchyIntegrator::registerVisItDataWriter(SAMRAIPointer<VisItDataWriterNd> visit_writer)
{
...
void
HierarchyIntegrator::initializeLevelData(const SAMRAIPointer<BasePatchHierarchyNd> base_hierarchy,
                                         ...
                                         const bool allocate_data)
{
    ...
    if (!initial_time && (level_number > 0 || old_level))
    {
        ...
        std::vector<RefinePatchStrategyNd*> fill_after_regrid_prolong_patch_strategies;
    ...
    if (initial_time)
    {
        ...
        for (const auto& var : d_state_variables)
        {
            ...
            SAMRAIPointer<CartGridFunction> var_init = d_state_var_init_fcns[var];
            if (var_init)
            {
            ...
            else
            {
                for (PatchLevelNd::Iterator p(level); p; p++)
                {
                    ...
                    SAMRAIPointer<CellDataNd<double> > var_current_cc_data = patch->getPatchData(var_current_idx);
                    SAMRAIPointer<EdgeDataNd<double> > var_current_ec_data = patch->getPatchData(var_current_idx);
                    SAMRAIPointer<FaceDataNd<double> > var_current_fc_data = patch->getPatchData(var_current_idx);
                    SAMRAIPointer<NodeDataNd<double> > var_current_nc_data = patch->getPatchData(var_current_idx);
                    SAMRAIPointer<SideDataNd<double> > var_current_sc_data = patch->getPatchData(var_current_idx);
...
void
HierarchyIntegrator::resetHierarchyConfiguration(const SAMRAIPointer<BasePatchHierarchyNd> base_hierarchy,
                                                 ...
                                                 const int finest_level)
{
    ...
    for (const auto& prolong_alg : d_prolong_algs)
    {
        for (int ln = std::max(coarsest_level, 1); ln <= std::min(finest_level + 1, finest_level); ++ln)
        {
            ...
            d_prolong_scheds[prolong_alg.first][ln] = prolong_alg.second->createSchedule(
                level, SAMRAIPointer<PatchLevelNd>(), ln - 1, hierarchy, d_prolong_strategies[prolong_alg.first].get());
        }
    }
    ...
    for (const auto& coarsen_alg : d_coarsen_algs)
    {
        ...
        {
            SAMRAIPointer<PatchLevelNd> level = hierarchy->getPatchLevel(ln);
            SAMRAIPointer<PatchLevelNd> coarser_level = hierarchy->getPatchLevel(ln - 1);
...
void
HierarchyIntegrator::applyGradientDetector(const SAMRAIPointer<BasePatchHierarchyNd> hierarchy,
                                           const int level_number,
...
HierarchyIntegrator::registerVariable(int& current_idx,
                                      ...
                                      const IntVectorNd& scratch_ghosts,
                                      ...
                                      const bool register_for_restart)
{
    ...
    SAMRAIPointer<RefineOperatorNd> refine_operator = grid_geom->lookupRefineOperator(variable, refine_name);
    SAMRAIPointer<CoarsenOperatorNd> coarsen_operator = grid_geom->lookupCoarsenOperator(variable, coarsen_name);
...
void
HierarchyIntegrator::putToDatabase(SAMRAIPointer<Database> db)
{
...
HierarchyIntegrator::resetTimeDependentHierarchyDataSpecialized(const double new_time)
{
    ...
    // Swap PatchDataNd pointers between the current and new contexts.
    VariableDatabaseNd* var_db = VariableDatabaseNd::getDatabase();
    for (const auto& v : d_state_variables)
    {
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            {
                SAMRAIPointer<PatchNd> patch = level->getPatch(p());
                SAMRAIPointer<PatchDataNd> src_data = patch->getPatchData(src_idx);
                SAMRAIPointer<PatchDataNd> dst_data = patch->getPatchData(dst_idx);
#if !defined(NDEBUG)
...
void
HierarchyIntegrator::initializeLevelDataSpecialized(const SAMRAIPointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                    const int /*level_number*/,
                                                    const double /*init_data_time*/,
                                                    const bool /*can_be_refined*/,
                                                    const bool /*initial_time*/,
...
void
HierarchyIntegrator::resetHierarchyConfigurationSpecialized(const SAMRAIPointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                            const int /*coarsest_level*/,
                                                            const int /*finest_level*/)
{
    // intentionally blank
...
void
HierarchyIntegrator::applyGradientDetectorSpecialized(const SAMRAIPointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                      const int /*level_number*/,
                                                      const double /*error_data_time*/,
                                                      const int /*tag_index*/,
                                                      const bool /*initial_time*/,
...
void
HierarchyIntegrator::putToDatabaseSpecialized(SAMRAIPointer<Database> /*db*/)
{
    // intentionally blank
    return;
...
void
HierarchyIntegrator::addWorkloadEstimate(SAMRAIPointer<PatchHierarchyNd>, const int)
{
    // intentionally blank
    return;
} // addWorkloadEstimate
...
void
HierarchyIntegrator::executeApplyGradientDetectorCallbackFcns(const SAMRAIPointer<BasePatchHierarchyNd> hierarchy,
                                                              const int level_number,
                                                              const double error_data_time,
                                                              const int tag_index,
                                                              const bool initial_time,
...
void
HierarchyIntegrator::executeRegridHierarchyCallbackFcns(const SAMRAIPointer<BasePatchHierarchyNd> hierarchy,
...
HierarchyIntegrator::registerGhostfillRefineAlgorithm(const std::string& name,
                                                      SAMRAIPointer<RefineAlgorithmNd> ghostfill_alg,
                                                      std::unique_ptr<RefinePatchStrategyNd> ghostfill_patch_strategy)
{
#if !defined(NDEBUG)
...
HierarchyIntegrator::registerProlongRefineAlgorithm(const std::string& name,
                                                    SAMRAIPointer<RefineAlgorithmNd> prolong_alg,
                                                    std::unique_ptr<RefinePatchStrategyNd> prolong_patch_strategy)
{
#if !defined(NDEBUG)
...
HierarchyIntegrator::registerCoarsenAlgorithm(const std::string& name,
                                              SAMRAIPointer<CoarsenAlgorithmNd> coarsen_alg,
                                              std::unique_ptr<CoarsenPatchStrategyNd> coarsen_patch_strategy)
{
#if !defined(NDEBUG)
...
SAMRAIPointer<RefineAlgorithmNd>
...
SAMRAIPointer<CoarsenAlgorithmNd>
...
const std::vector<SAMRAIPointer<RefineScheduleNd> >&
...
const std::vector<SAMRAIPointer<CoarsenScheduleNd> >&
...
SAMRAIPointer<HierarchyMathOps>
HierarchyIntegrator::buildHierarchyMathOps(SAMRAIPointer<PatchHierarchyNd> hierarchy)
{
    if (!d_parent_integrator)
    {
        if (!d_hier_math_ops)
...
void
HierarchyIntegrator::setupTagBuffer(SAMRAIPointer<GriddingAlgorithmNd> gridding_alg)
{
...
void
HierarchyIntegrator::getFromInput(SAMRAIPointer<Database> db, bool is_from_restart)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(db);
#endif
