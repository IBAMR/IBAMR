...
void
HierarchyIntegrator::initializePatchHierarchy(Pointer<PatchHierarchyNd> hierarchy,
...
void
HierarchyIntegrator::registerLoadBalancer(Pointer<LoadBalancerNd> load_balancer)
{
#if !defined(NDEBUG)
...
void
HierarchyIntegrator::registerVisItDataWriter(Pointer<VisItDataWriterNd> visit_writer)
{
...
void
HierarchyIntegrator::initializeLevelData(const Pointer<BasePatchHierarchyNd> base_hierarchy,
        ...
        for (const auto& var : d_state_variables)
        {
            ...
            else
            {
                for (PatchLevelNd::Iterator p(level); p; p++)
                {
                    ...
                    Pointer<CellDataNd<double> > var_current_cc_data = patch->getPatchData(var_current_idx);
                    Pointer<EdgeDataNd<double> > var_current_ec_data = patch->getPatchData(var_current_idx);
                    Pointer<FaceDataNd<double> > var_current_fc_data = patch->getPatchData(var_current_idx);
                    Pointer<NodeDataNd<double> > var_current_nc_data = patch->getPatchData(var_current_idx);
                    Pointer<SideDataNd<double> > var_current_sc_data = patch->getPatchData(var_current_idx);
...
void
HierarchyIntegrator::resetHierarchyConfiguration(const Pointer<BasePatchHierarchyNd> base_hierarchy,
                                                 ...
                                                 const int finest_level)
{
    ...
    for (const auto& prolong_alg : d_prolong_algs)
    {
        for (int ln = std::max(coarsest_level, 1); ln <= std::min(finest_level + 1, finest_level); ++ln)
        {
            ...
            d_prolong_scheds[prolong_alg.first][ln] = prolong_alg.second->createSchedule(
                level, Pointer<PatchLevelNd>(), ln - 1, hierarchy, d_prolong_strategies[prolong_alg.first].get());
        }
    }
    ...
    for (const auto& coarsen_alg : d_coarsen_algs)
    {
        ...
        {
            Pointer<PatchLevelNd> level = hierarchy->getPatchLevel(ln);
            Pointer<PatchLevelNd> coarser_level = hierarchy->getPatchLevel(ln - 1);
...
void
HierarchyIntegrator::applyGradientDetector(const Pointer<BasePatchHierarchyNd> hierarchy,
                                           const int level_number,
                                      ...
                                      const bool register_for_restart)
{
    ...
    Pointer<RefineOperatorNd> refine_operator = grid_geom->lookupRefineOperator(variable, refine_name);
    Pointer<CoarsenOperatorNd> coarsen_operator = grid_geom->lookupCoarsenOperator(variable, coarsen_name);
...
HierarchyIntegrator::resetTimeDependentHierarchyDataSpecialized(const double new_time)
{
    ...
    for (const auto& v : d_state_variables)
    {
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            {
                Pointer<PatchNd> patch = level->getPatch(p());
                Pointer<PatchDataNd> src_data = patch->getPatchData(src_idx);
                Pointer<PatchDataNd> dst_data = patch->getPatchData(dst_idx);
#if !defined(NDEBUG)
...
void
HierarchyIntegrator::initializeLevelDataSpecialized(const Pointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                    const int /*level_number*/,
                                                    const double /*init_data_time*/,
                                                    const bool /*can_be_refined*/,
                                                    const bool /*initial_time*/,
...
void
HierarchyIntegrator::resetHierarchyConfigurationSpecialized(const Pointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                            const int /*coarsest_level*/,
                                                            const int /*finest_level*/)
{
    // intentionally blank
...
void
HierarchyIntegrator::applyGradientDetectorSpecialized(const Pointer<BasePatchHierarchyNd> /*hierarchy*/,
                                                      const int /*level_number*/,
                                                      const double /*error_data_time*/,
                                                      const int /*tag_index*/,
                                                      const bool /*initial_time*/,
...
void
HierarchyIntegrator::addWorkloadEstimate(Pointer<PatchHierarchyNd>, const int)
{
    // intentionally blank
    return;
} // addWorkloadEstimate
...
void
HierarchyIntegrator::executeApplyGradientDetectorCallbackFcns(const Pointer<BasePatchHierarchyNd> hierarchy,
                                                              const int level_number,
                                                              const double error_data_time,
                                                              const int tag_index,
                                                              const bool initial_time,
...
void
HierarchyIntegrator::executeRegridHierarchyCallbackFcns(const Pointer<BasePatchHierarchyNd> hierarchy,
...
HierarchyIntegrator::registerGhostfillRefineAlgorithm(const std::string& name,
                                                      Pointer<RefineAlgorithmNd> ghostfill_alg,
...
HierarchyIntegrator::registerProlongRefineAlgorithm(const std::string& name,
                                                    Pointer<RefineAlgorithmNd> prolong_alg,
...
HierarchyIntegrator::registerCoarsenAlgorithm(const std::string& name,
                                              Pointer<CoarsenAlgorithmNd> coarsen_alg,
...
const std::vector<Pointer<RefineScheduleNd> >&
...
const std::vector<Pointer<CoarsenScheduleNd> >&
...
Pointer<HierarchyMathOps>
HierarchyIntegrator::buildHierarchyMathOps(Pointer<PatchHierarchyNd> hierarchy)
{
    if (!d_parent_integrator)
    {
        if (!d_hier_math_ops)
...
void
HierarchyIntegrator::setupTagBuffer(Pointer<GriddingAlgorithmNd> gridding_alg)
{
