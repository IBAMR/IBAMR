...
LibMeshSystemVectors::LibMeshSystemVectors(const std::vector<libMesh::EquationSystems*>& equation_systems,
    ...
    : d_system_name(std::move(system_name))
{
    d_part_mask.resize(equation_systems.size());
    std::fill(d_part_mask.begin(), d_part_mask.end(), true);
    TBOX_ASSERT(equation_systems.size() > 0);
    for (libMesh::EquationSystems* es : equation_systems)
    {
...
LibMeshSystemVectors::LibMeshSystemVectors(const std::vector<bool>& part_mask, std::string system_name)
    : d_part_mask(part_mask), d_system_name(std::move(system_name))
{
}
