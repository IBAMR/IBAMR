...
namespace IBTK
{
    ...
    struct IndexPairLexical
    {
                   ...
                   const std::pair<hier::Index<NDIM>, hier::Index<NDIM>> &b)
        {
            ...
            return std::lexicographical_compare(std::begin(a_indices),
                                                std::end(a_indices),
                                                std::begin(b_indices),
                                                std::end(b_indices));
        }
    ...
    merge_boxes_by_longest_edge(const std::vector<SAMRAI::hier::Box<NDIM> > &input_boxes)
    {
        ...
        while (true)
        {
            ...
            std::map<std::pair<hier::Index<NDIM>, hier::Index<NDIM> >,
                     std::set<hier::Box<NDIM>, BoxLexical>, IndexPairLexical>
                face_to_boxes;
            for (const hier::Box<NDIM> &box : boxes)
            {
                ...
                if (NDIM == 2)
                {
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > left =
                        {{lower(0), lower(1)}, {lower(0), upper(1)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > right =
                        {{upper(0), lower(1)}, {upper(0), upper(1)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > bottom =
                        {{lower(0), lower(1)}, {upper(0), lower(1)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > top =
                        {{lower(0), upper(1)}, {upper(0), upper(1)}};
                ...
                {
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > left =
                        {{lower(0), lower(1), lower(2)}, {lower(0), upper(1), upper(2)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > right =
                        {{upper(0), lower(1), lower(2)}, {upper(0), upper(1), upper(2)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > front =
                        {{lower(0), lower(1), lower(2)}, {upper(0), lower(1), upper(2)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > back =
                        {{lower(0), upper(1), lower(2)}, {upper(0), upper(1), upper(2)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > bottom =
                        {{lower(0), lower(1), lower(2)}, {upper(0), upper(1), lower(2)}};
                    const std::pair<hier::Index<NDIM>, hier::Index<NDIM> > top =
                        {{lower(0), lower(1), upper(2)}, {upper(0), upper(1), upper(2)}};
                    face_to_boxes[left].insert(box);
                    face_to_boxes[right].insert(box);
            ...
            {
                std::vector<std::pair<hier::Index<NDIM>, hier::Index<NDIM> > >
                    faces_to_remove;
                for (const auto &pair : face_to_boxes)
                    if (pair.second.size() < 2)
                        faces_to_remove.push_back(pair.first);
                for (const auto &face : faces_to_remove)
                    face_to_boxes.erase(face);
            }

            tbox::plog << "number of common faces: " << face_to_boxes.size() << '\n';
            if (face_to_boxes.size() == 0)
                break;
            ...
            for (auto &face : faces)
            {
                ...
                TBOX_ASSERT(box_1.size() + box_2.size() - intersect_box.size()
                            == union_box.size());
            }
            ++round_n;
        }
        ...
        {
            long input_volume = 0;
            for (const hier::Box<NDIM> &box : input_boxes)
                input_volume += box.size();
            long output_volume = 0;
            for (const hier::Box<NDIM> &box : result)
                output_volume += box.size();
