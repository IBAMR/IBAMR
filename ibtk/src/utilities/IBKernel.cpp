...
IBKernel::IBKernel(const std::string& name)
{
    ...
    {
        TBOX_ERROR("IBKernel requires 1 to "
                   << MAX_NAME_LENGTH << " ASCII letters, digits, or underscores, excluding UNKNOWN: " << name << '\n');
    }
}
...
IBKernel::IBKernel(const char* name)
{
    ...
    {
        TBOX_ERROR("IBKernel requires a nonnull name with 1 to "
                   << MAX_NAME_LENGTH << " ASCII letters, digits, or underscores, excluding UNKNOWN\n");
    }
}
