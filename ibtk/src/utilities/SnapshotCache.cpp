...
SnapshotCache::clearSnapshots()
{
    ...
    {
        Pointer<SAMRAIPatchHierarchy> hierarchy = snapshot.second;
...
void
SnapshotCache::storeSnapshot(const int u_idx, const double time, Pointer<SAMRAIPatchHierarchy> hierarchy)
{
    // Create a snapshot of the patch hierarchy.
    Pointer<SAMRAIPatchHierarchy> snapshot_hierarchy = hierarchy->makeRefinedPatchHierarchy(
    ...
    {
        Pointer<SAMRAIPatchLevel> snapshot_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        Pointer<SAMRAIPatchLevel> old_level = hierarchy->getPatchLevel(ln);
        ...
        Pointer<SAMRAIRefineSchedule> schedule = refine_alg->createSchedule(snapshot_level, old_level);
...
void
SnapshotCache::getFromRestart(Pointer<SAMRAIGridGeometry> grid_geom)
{
    ...
    for (unsigned int i = 0; i < num_snapshots_stored; ++i)
    {
        ...
        Pointer<SAMRAIPatchHierarchy> hierarchy =
            new SAMRAIPatchHierarchy(d_object_name + "::SnapshotHierarchy_" + std::to_string(i), grid_geom, false);
