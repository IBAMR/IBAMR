...
SnapshotCache::clearSnapshots()
{
    ...
    {
        SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy = snapshot.second;
...
void
SnapshotCache::storeSnapshot(const int u_idx, const double time, SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy)
{
    // Create a snapshot of the patch hierarchy.
    SAMRAIPointer<SAMRAIPatchHierarchy> snapshot_hierarchy = hierarchy->makeRefinedPatchHierarchy(
    ...
    {
        SAMRAIPointer<SAMRAIPatchLevel> snapshot_level = snapshot_hierarchy->getPatchLevel(ln);
        ...
        SAMRAIPointer<SAMRAIPatchLevel> old_level = hierarchy->getPatchLevel(ln);
        ...
        SAMRAIPointer<SAMRAIRefineSchedule> schedule = refine_alg->createSchedule(snapshot_level, old_level);
...
void
SnapshotCache::putToDatabase(SAMRAIPointer<Database> db)
{
    ...
    for (const auto& snapshot : d_snapshots)
    {
        ...
        SAMRAIPointer<Database> snapshot_db = db->putDatabase("snapshot_hierarchy_" + std::to_string(i));
...
void
SnapshotCache::getFromRestart(SAMRAIPointer<SAMRAIGridGeometry> grid_geom)
{
