// ---------------------------------------------------------------------
//
// Copyright (c) 2023 - 2025 by the IBAMR developers
// All rights reserved.
//
...
compute_nonoverlapping_patch_boxes(const Pointer<SAMRAIBasePatchLevel>& c_level,
                                   const Pointer<SAMRAIBasePatchLevel>& f_level)
{
    const Pointer<SAMRAIPatchLevel> coarse_level = c_level;
    const Pointer<SAMRAIPatchLevel> fine_level = f_level;
...
do_interpolation(const int data_idx,
                 ...
                 const Pointer<SAMRAIPatch> patch,
                 const std::string& kernel,
                 std::vector<double>& velocities)
{
    Pointer<SAMRAIPatchData> data = patch->getPatchData(data_idx);
    Pointer<SAMRAICellData<double>> cc_data = data;
    Pointer<SAMRAISideData<double>> sc_data = data;
...
MarkerPatchHierarchy::reinit(const std::vector<IBTK::Point>& positions, const std::vector<IBTK::Point>& velocities)
{
    ...
    d_num_markers = positions.size();
    const auto rank = IBTK_MPI::getRank();
    const Pointer<SAMRAICartesianGridGeometry> grid_geom = d_hierarchy->getGridGeometry();
    ...
    {
        Pointer<SAMRAIPatchLevel> current_level = d_hierarchy->getPatchLevel(ln);
        Pointer<SAMRAIPatchLevel> finer_level =
...
MarkerPatchHierarchy::setVelocities(const int u_idx, const std::string& kernel)
{
    ...
    for (int ln = d_hierarchy->getFinestLevelNumber(); ln >= 0; --ln)
    {
        ...
        for (int p = 0; p < current_level->getNumberOfPatches(); ++p)
        {
            ...
            {
                Pointer<SAMRAIPatch> patch = current_level->getPatch(p);
