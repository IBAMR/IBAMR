...
CartCellDoubleLinearCoarsen::CartCellDoubleLinearCoarsen(IntVector<NDIM> gcw) : d_gcw(gcw)
{
    if (d_gcw.min() < 0)
    {
        TBOX_ERROR("CartCellDoubleLinearCoarsen: stencil width must be nonnegative.\n");
    }
}
                                     ...
                                     const IntVector<NDIM>& ratio) const
{
    ...
    if (cdata->getGhostCellWidth().min() != cdata->getGhostCellWidth().max() ||
        fdata->getGhostCellWidth().min() != fdata->getGhostCellWidth().max() || cdata->getDepth() != fdata->getDepth())
    {
        TBOX_ERROR("CartCellDoubleLinearCoarsen::coarsen(): data require uniform ghost widths and matching depths.\n");
    }
    for (int axis = 0; axis < NDIM; ++axis)
    {
        if (ratio(axis) <= 0)
        {
            TBOX_ERROR("CartCellDoubleLinearCoarsen::coarsen(): refinement ratios must be positive.\n");
        }
        if (ratio(axis) / 2 > d_gcw(axis) || fdata->getGhostCellWidth()(axis) < d_gcw(axis))
        {
            TBOX_ERROR(
                "CartCellDoubleLinearCoarsen::coarsen(): insufficient declared or allocated fine-cell stencil "
                "width.\n");
        }
    }
