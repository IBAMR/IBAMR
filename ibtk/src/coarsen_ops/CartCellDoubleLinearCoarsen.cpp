...
#if (NDIM == 2)
#define CC_LINEAR_COARSEN_FC IBTK_FC_FUNC_(cart_cell_linear_coarsen2d, CART_CELL_LINEAR_COARSEN2D)
#endif
#if (NDIM == 3)
#define CC_LINEAR_COARSEN_FC IBTK_FC_FUNC_(cart_cell_linear_coarsen3d, CART_CELL_LINEAR_COARSEN3D)
#endif
...
CartCellDoubleLinearCoarsen::CartCellDoubleLinearCoarsen(IntVector<NDIM> gcw) : d_gcw(gcw)
{
    if (d_gcw.min() < 0)
    {
        TBOX_ERROR("CartCellDoubleLinearCoarsen::CartCellDoubleLinearCoarsen():\n"
                   << "  the stencil width must be nonnegative.\n");
    }
}
                                     ...
                                     const IntVector<NDIM>& ratio) const
{
    ...
    TBOX_ASSERT(cdata->getGhostCellWidth().min() == cdata->getGhostCellWidth().max());
    TBOX_ASSERT(fdata->getGhostCellWidth().min() == fdata->getGhostCellWidth().max());
#endif

    for (int axis = 0; axis < NDIM; ++axis)
    {
        if (ratio(axis) / 2 > d_gcw(axis) || fdata->getGhostCellWidth()(axis) < d_gcw(axis))
        {
            TBOX_ERROR("CartCellDoubleLinearCoarsen::coarsen():\n"
                       << "  insufficient declared or allocated fine-cell stencil width.\n");
        }
    }
