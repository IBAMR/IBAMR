...
{
/*!
 * \brief Evaluate an isotropic or axis-relative tensor product of scalar kernels.
 *
 * Each scalar evaluator must be const-callable with a double and return
 * std::array<double, N> with its natural stencil width. A single evaluator
...
public:
    ...
    /*! \brief Construct a product of independent axis and transverse evaluators. */
    IBKernelTensorProductEvaluator(NormalEvaluator normal_evaluator, TangentialEvaluator tangential_evaluator);

    /*! \brief Return the natural stencil widths for a selected axis in two or three dimensions. */
    ...
    /*! \brief Return the tensor-product coefficients for a compile-time selected axis. */
    template <int Axis, std::size_t Dim>
    auto evaluate(const std::array<double, Dim>& r) const;

private:
    //! Scalar result types determine the natural stencil widths.
