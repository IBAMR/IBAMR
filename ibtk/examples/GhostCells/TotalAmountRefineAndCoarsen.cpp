...
bool
TotalAmountRefine::findRefineOperator(const Pointer<SAMRAIVariable>& var, const std::string& op_name) const
{
    // This operation is only valid on CellVariable's
    Pointer<SAMRAICellVariable<double>> cast_var = var;
                          ...
                          const SAMRAIIntVector& ratio) const
{
    ...
    Pointer<SAMRAICellData<double>> fine_data = fine.getPatchData(dst_component);
    Pointer<SAMRAICellData<double>> coarse_data = coarse.getPatchData(src_component);
...
bool
TotalAmountCoarsen::findCoarsenOperator(const Pointer<SAMRAIVariable>& var, const std::string& op_name) const
{
    // This operation is only valid on CellVariable's
    Pointer<SAMRAICellVariable<double>> cast_var = var;
    if (!cast_var.isNull() && op_name == s_object_name)
                            ...
                            const SAMRAIIntVector& ratio) const
{
    ...
    Pointer<SAMRAICellData<double>> fine_data = fine.getPatchData(src_component);
    Pointer<SAMRAICellData<double>> coarse_data = coarse.getPatchData(dst_component);
