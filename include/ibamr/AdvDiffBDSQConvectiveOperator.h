...
#ifndef included_AdvDiffBDSQConvectiveOperator
#define included_AdvDiffBDSQConvectiveOperator
...
public:
    ...
    /*!
     * \brief Static function to construct an AdvDiffBDSQConvectiveOperator.
     */
    ...
    {
        return new AdvDiffBDSQConvectiveOperator(object_name, Q_var, input_db, difference_form, conc_bc_coefs);
    }
...
private:
     ...
     */
    AdvDiffBDSQConvectiveOperator() = delete;
     ...
     */
    AdvDiffBDSQConvectiveOperator(const AdvDiffBDSQConvectiveOperator& from) = delete;
     ...
     */
    AdvDiffBDSQConvectiveOperator& operator=(const AdvDiffBDSQConvectiveOperator& that) = delete;

    bool limitingStepTwo(std::array<double, 6>& p,
                         const double* const dx,
                         SAMRAI::tbox::Pointer<SAMRAI::pdat::CellData<NDIM, double> > Q_data,
                         const SAMRAI::pdat::CellIndex<NDIM>& idx);
    bool testValues(std::array<double, 6>& p,
                    const double* const dx,
                    SAMRAI::tbox::Pointer<SAMRAI::pdat::CellData<NDIM, double> > Q_data,
                    const SAMRAI::pdat::CellIndex<NDIM>& idx);
    bool limitingStepThree(std::array<double, 6>& p,
                           const double* const dx,
                           SAMRAI::tbox::Pointer<SAMRAI::pdat::CellData<NDIM, double> > Q_data,
                           const SAMRAI::pdat::CellIndex<NDIM>& idx);
    double evaluateP(const std::array<double, 6>& p, const double x, const double y);

    double max(const double, const double, const double, const double);
    double min(const double, const double, const double, const double);
