...
public:
     ...
     */
    void setStokesSolver(SAMRAIPointer<StaggeredStokesSolver> stokes_solver);

    /*!
     * Get the solver for the time-dependent incompressible Stokes equations
     * used by this solver class.
     */
    SAMRAIPointer<StaggeredStokesSolver> getStokesSolver();
     ...
     */
    void registerMassDensityVariable(SAMRAIPointer<SAMRAIVariable> rho_var);

    /*!
     * Get the mass density variable registered with the hierarchy integrator.
     */
    SAMRAIPointer<SAMRAIVariable> getMassDensityVariable() const;

    /*!
     * Register a variable viscosity variable with the hierarchy integrator.
     */
    void registerViscosityVariable(SAMRAIPointer<SAMRAIVariable> mu_var);

    /*!
     * Get the viscosity variable registered with the hierarchy integrator.
     */
    SAMRAIPointer<SAMRAIVariable> getViscosityVariable() const;
    ...
    using ResetFluidPropertiesFcnPtr = void (*)(int property_idx,
                                                SAMRAIPointer<SAMRAIVariable> property_var,
     ...
     */
    virtual void
    registerBrinkmanPenalizationStrategy(SAMRAIPointer<IBAMR::BrinkmanPenalizationStrategy> brinkman_force);

    /*!
     * \brief Supply initial conditions for the density field, if maintained by the fluid integrator.
     */
    void registerMassDensityInitialConditions(SAMRAIPointer<IBTK::CartGridFunction> rho_init_fcn);

    /*!
     * \brief Supply initial conditions for the viscosity field, if maintained by the fluid integrator.
     */
    void registerViscosityInitialConditions(SAMRAIPointer<IBTK::CartGridFunction> mu_init_fcn);

    /*
     * \brief Pure virtual method to supply boundary conditions for the density field, if maintained by the fluid
     * integrator.
     */
    virtual void registerMassDensityBoundaryConditions(SAMRAIRobinBcCoefStrategy* rho_bc_coef) = 0;
     ...
     */
    void registerViscosityBoundaryConditions(SAMRAIRobinBcCoefStrategy* mu_bc_coef);
     ...
     */
    inline SAMRAIArray<double> getScalingFactor() const
    {
        return d_A_scale;
    }
     ...
     */
    inline SAMRAIRobinBcCoefStrategy* getViscosityBoundaryConditions() const
    {
        return d_mu_bc_coef;
    }
     ...
     */
    SAMRAIPointer<SAMRAIVariable> getOldVelocityVariable() const
    {
        return d_U_old_var;
...
protected:
     ...
     */
    void copySideToFace(const int U_fc_idx, const int U_sc_idx, SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy);
    ...
    SAMRAIPointer<IBTK::SideDataSynchronization> d_side_synch_op;

    SAMRAIPointer<IBTK::HierarchyGhostCellInterpolation> d_rho_bdry_bc_fill_op, d_mu_bdry_bc_fill_op;
    ...
    SAMRAIPointer<SAMRAIDatabase> d_stokes_solver_db, d_stokes_precond_db;
     ...
     */
    SAMRAIPointer<IBTK::CartGridFunction> d_rho_init_fcn, d_mu_init_fcn;
     ...
     */
    SAMRAIRobinBcCoefStrategy* d_mu_bc_coef = nullptr;
