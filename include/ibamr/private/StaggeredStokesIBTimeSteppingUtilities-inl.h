...
struct StaggeredStokesIBTimeSteppingSchedule
{
...
inline StaggeredStokesIBTimeSteppingSchedule
get_staggered_stokes_ib_time_stepping_schedule(const TimeSteppingType time_stepping_type,
                                               const double current_time,
                                               const double new_time,
                                               const std::string& caller)
{
    StaggeredStokesIBTimeSteppingSchedule schedule;
    schedule.half_time = current_time + 0.5 * (new_time - current_time);
    switch (time_stepping_type)
    {
    case BACKWARD_EULER:
        schedule.velocity_time = new_time;
        schedule.force_time = new_time;
        schedule.velocity_state = StaggeredStokesIBVelocityState::NEW;
        schedule.nonlinear_force_scale = 1.0;
        schedule.jacobian_force_scale = 1.0;
        break;
    case TRAPEZOIDAL_RULE:
        schedule.velocity_time = new_time;
        schedule.force_time = new_time;
        schedule.velocity_state = StaggeredStokesIBVelocityState::NEW;
        schedule.nonlinear_force_scale = 0.5;
        schedule.jacobian_force_scale = 0.5;
        break;
    case MIDPOINT_RULE:
        schedule.velocity_time = schedule.half_time;
        schedule.force_time = schedule.half_time;
        schedule.velocity_state = StaggeredStokesIBVelocityState::MIDPOINT_AVERAGE;
        schedule.nonlinear_force_scale = 1.0;
        schedule.jacobian_force_scale = 0.5;
        break;
    default:
