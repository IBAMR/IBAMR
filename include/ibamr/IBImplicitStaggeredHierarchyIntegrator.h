 ...
 * \c jacobian_delta_fcn names the delta function of the Jacobian's interpolation
 * matrix, which need not equal the kernel that the strategy uses for
 * interpolation and spreading. Every built-in kernel is accepted (see
 * IBTK::IBOperatorBuilder); setJacobianOperatorBuilder() selects any other
 * kernel. The strategy's minimum ghost width and the kernel's are both
 * accommodated.
 *
...
private:
    ...
    //! Builds the interpolation matrix of the Jacobian.
