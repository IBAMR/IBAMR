...
public:
     ...
     * RELAXED joins the standard Vanka patch of every cell incident to an
     * expanded velocity DOF and retains the expanded velocities. STRICT adds
     * lower-face seed components of the same cell and retains only cells whose
     * full velocity stencil is present after one row-neighbor expansion.
     *
     ...
     * inner set uses global coupled IDs; nonoverlap assigns each DOF that is in an
     * overlap to its first containing overlap. The overlaps need not contain every
     * DOF; see IBTK::check_dof_coverage().
     * DOF data must have valid adjacent-cell ghosts and remain unchanged during
     * construction. Only one MPI rank is supported. This routine constructs
     * subdomains; it does not apply them.
     */
