 ...
 *
 * Class AdvDiffConservativeCUIConvectiveOperator computes the convective derivative of a
...
public:
    ...
    /*!
     * \brief Static function to construct an AdvDiffConservativeCUIConvectiveOperator.
     */
    ...
    {
        return new AdvDiffConservativeCUIConvectiveOperator(object_name, Q_var, input_db, difference_form, bc_coefs);
    } // allocate_operator
     ...
     * \brief Compute the action of the convective operator.
     */
    void applyConvectiveOperator(int Q1_idx, int Q2_idx, int N_idx) override;
     ...
     */
    void setHeavisideVariable(SAMRAI::tbox::Pointer<SAMRAI::hier::Variable<NDIM> > H_var);

    /*!
     * \brief Set the patch data index corresponding to the mass density
     * to be used when computing the conservative convective derivative.
     */
    void setHeavysideIndex(int H_idx);

    /*!
     * \brief Set the boundary condition object of the mass density
     * to be used when computing the conservative convective derivative.
     */
    void setHeavisideBoundaryConditions(SAMRAI::solv::RobinBcCoefStrategy<NDIM>* H_bc_coef);
...
private:
    ...
    std::vector<SAMRAI::solv::RobinBcCoefStrategy<NDIM>*> d_H_bc_coefs;
