...
void
AdvDiffPPMConvectiveOperator::interpolateToFaceOnPatch(FaceDataNd<double>& q_interp_data,
                                                       ...
                                                       const PatchNd& patch)
{
    ...
    const IntVectorNd& Q_cell_data_gcw = Q_cell_data.getGhostCellWidth();
#if !defined(NDEBUG)
...
#endif
    const CellDataNd<double>& Q0_data = Q_cell_data;
    CellDataNd<double> Q1_data(patch_box, 1, Q_cell_data_gcw);
#if (NDIM == 3)
    CellDataNd<double> Q2_data(patch_box, 1, Q_cell_data_gcw);
#endif
    CellDataNd<double> dQ_data(patch_box, 1, Q_cell_data_gcw);
    CellDataNd<double> Q_L_data(patch_box, 1, Q_cell_data_gcw);
    CellDataNd<double> Q_R_data(patch_box, 1, Q_cell_data_gcw);
