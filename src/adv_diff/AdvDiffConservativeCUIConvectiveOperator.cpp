...
{
    TBOX_ERROR(
        "AdvDiffConservativeCUIConvectiveOperator::applyConvectiveOperator can handle only conservative form of "
...
AdvDiffConservativeCUIConvectiveOperator::applyConvectiveOperator(const int Q1_idx, const int Q2_idx, const int N_idx)
{
    ...
    Pointer<RefineAlgorithm<NDIM> > refine_alg2 = new RefineAlgorithm<NDIM>();
    Pointer<RefineOperator<NDIM> > refine_op2 = grid_geom->lookupRefineOperator(d_Q2_var, "CONSERVATIVE_LINEAR_REFINE");
    refine_alg2->registerRefine(d_Q2_scratch_idx, Q2_idx, d_Q2_scratch_idx, refine_op2);
    ...
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        ...
        refine_alg2->resetSchedule(d_ghostfill_scheds2[ln]);
        d_ghostfill_scheds2[ln]->fillData(d_solution_time);
        d_ghostfill_alg2->resetSchedule(d_ghostfill_scheds2[ln]);
                                                                  ...
                                                                  const SAMRAIVectorReal<NDIM, double>& out)
{
    ...
    d_ghostfill_alg2 = new RefineAlgorithm<NDIM>();
    Pointer<RefineOperator<NDIM> > refine_op2 = grid_geom->lookupRefineOperator(d_Q2_var, "CONSERVATIVE_LINEAR_REFINE");
    d_ghostfill_alg2->registerRefine(d_Q2_scratch_idx, in.getComponentDescriptorIndex(0), d_Q2_scratch_idx, refine_op2);
    if (d_outflow_bdry_extrap_type != "NONE")
        d_ghostfill_strategy2 = new CartExtrapPhysBdryOp(d_Q2_scratch_idx, d_outflow_bdry_extrap_type);

    d_ghostfill_scheds2.resize(d_finest_ln + 1);
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        Pointer<PatchLevel<NDIM> > level = d_hierarchy->getPatchLevel(ln);
        d_ghostfill_scheds2[ln] = d_ghostfill_alg2->createSchedule(level, ln - 1, d_hierarchy, d_ghostfill_strategy2);
    }
...
AdvDiffConservativeCUIConvectiveOperator::deallocateOperatorState()
{
    ...
    d_ghostfill_alg2.setNull();
    d_ghostfill_strategy2.setNull();
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        d_ghostfill_scheds1[ln].setNull();
        d_ghostfill_scheds2[ln].setNull();
    }
    d_ghostfill_scheds1.clear();
    d_ghostfill_scheds2.clear();
