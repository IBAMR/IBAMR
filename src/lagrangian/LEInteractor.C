...
// Last modified: <31.Aug.2007 01:46:01 griffith@box221.cims.nyu.edu>
    ...
    const std::string& spread_fcn)
{
    ...
    // Indicate whether we are near a physical boundary.
    std::vector<int> phys_bdry_lower(NDIM,0), phys_bdry_upper(NDIM,0);
    for (int axis = 0; axis < NDIM; ++axis)
    {
        if (pgeom->getTouchesRegularBoundary(axis,0)) phys_bdry_lower[axis] = 1;
        if (pgeom->getTouchesRegularBoundary(axis,1)) phys_bdry_upper[axis] = 1;
    }
    ...
    if (local_indices_size > 0)
    {
        ...
        else if (spread_fcn == "IB_4")
        {
            LAGRANGIAN_IB4_SPREAD_F77(
                dx,xLower,xUpper,depth,
                &phys_bdry_lower[0],&phys_bdry_upper[0],
                &local_indices[0],&periodic_offsets[0],local_indices_size,
                X_data,Q_data,
#if (NDIM == 2)
                ilower(0),iupper(0),ilower(1),iupper(1),
