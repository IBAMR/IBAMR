...
void
AcousticStreamingPETScVecUtilities::copyToPatchLevelVec(Vec& vec,
                                                        ...
                                                        Pointer<PatchLevel<NDIM> > patch_level)
{
    ...
    {
        TBOX_ERROR("AcousticStreamingPETScVecUtilities::copyToPatchLevelVec():\n"
...
void
AcousticStreamingPETScVecUtilities::copyFromPatchLevelVec(Vec& vec,
                                                          const int u_data_idx,
                                                          const int u_dof_index_idx,
                                                          const int p_data_idx,
                                                          const int p_dof_index_idx,
                                                          ...
                                                          Pointer<RefineSchedule<NDIM> > ghost_fill_sched)
{
    ...
    {
        TBOX_ERROR("AcousticStreamingPETScVecUtilities::copyFromPatchLevelVec():\n"
                   << "  unsupported data centering types for variables " << u_data_var->getName() << " and "
                   << p_data_var->getName() << "\n");
    }
    if (ghost_fill_sched)
...
AcousticStreamingPETScVecUtilities::constructDataSynchSchedule(const int u_data_idx,
                                                               const int p_data_idx,
                                                               Pointer<PatchLevel<NDIM> > patch_level)
{
    VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
    ...
    {
        TBOX_ERROR("AcousticStreamingPETScVecUtilities::constructDataSynchSchedule():\n"
                   << "  unsupported data centering types for variables " << u_data_var->getName() << " and "
                   << p_data_var->getName() << "\n");
    }
...
Pointer<RefineSchedule<NDIM> >
AcousticStreamingPETScVecUtilities::constructGhostFillSchedule(const int u_data_idx,
                                                               const int p_data_idx,
                                                               Pointer<PatchLevel<NDIM> > patch_level)
{
    RefineAlgorithm<NDIM> ghost_fill_alg;
...
void
AcousticStreamingPETScVecUtilities::constructPatchLevelDOFIndices(std::vector<int>& num_dofs_per_proc,
                                                                  const int u_dof_index_idx,
                                                                  const int p_dof_index_idx,
                                                                  Pointer<PatchLevel<NDIM> > patch_level)
{
    ...
    {
        TBOX_ERROR("AcousticStreamingPETScVecUtilities::constructPatchLevelDOFIndices():\n"
...
void
AcousticStreamingPETScVecUtilities::copyToPatchLevelVec_MAC(Vec& vec,
                                                            const int u_data_idx,
                                                            const int u_dof_index_idx,
                                                            const int p_data_idx,
                                                            const int p_dof_index_idx,
                                                            Pointer<PatchLevel<NDIM> > patch_level)
{
    ...
    for (PatchLevel<NDIM>::Iterator p(patch_level); p; p++)
    {
        ...
        TBOX_ASSERT(u_data->getDepth() == 2);
        TBOX_ASSERT(u_dof_index_data->getDepth() == 2);
#endif
        ...
        TBOX_ASSERT(p_data->getDepth() == 2);
        TBOX_ASSERT(p_dof_index_data->getDepth() == 2);
#endif
        ...
        for (int comp = 0; comp < 2; ++comp)
        {
            COPY_TO_PATCHLEVEL_VEC_MAC_FC(ilower(0),
                                          ...
                                          first_local,
                                          last_local,
                                          p_data->getPointer(comp),
                                          p_gcw.min(),
                                          u_data->getPointer(0, comp),
                                          u_data->getPointer(1, comp),
#if (NDIM == 3)
                                          u_data->getPointer(2, comp),
#endif
                                          u_gcw.min(),
                                          p_dof_index_data->getPointer(comp),
                                          p_dof_gcw.min(),
                                          u_dof_index_data->getPointer(0, comp),
                                          u_dof_index_data->getPointer(1, comp),
#if (NDIM == 3)
                                          u_dof_index_data->getPointer(2, comp),
#endif
                                          u_dof_gcw.min(),
                                          array);
        }
                                                                      ...
                                                                      Pointer<PatchLevel<NDIM> > patch_level)
{
    ...
    Pointer<SideVariable<NDIM, int> > patch_num_var = new SideVariable<NDIM, int>(
        "AcousticStreamingPETScVecUtilities::constructPatchLevelDOFIndices_side()::"
        "patch_num_var",
        /*depth*/ 1);
    ...
    Pointer<SideVariable<NDIM, bool> > u_mastr_loc_var = new SideVariable<NDIM, bool>(
        "AcousticStreamingPETScVecUtilities::"
        ...
        for (unsigned int component_axis = 0; component_axis < NDIM; ++component_axis)
        {
            ...
            for (Box<NDIM>::Iterator b(SideGeometry<NDIM>::toSideBox(patch_box, component_axis)); b; b++)
            {
                ...
                if ((*patch_num_data)(is) == patch_num && !periodic_image)
                {
                    ...
                    local_dof_count += u_dof_index_depth;
                }
            }
        }
        local_dof_count += p_dof_index_depth * CellGeometry<NDIM>::toCellBox(patch_box).size();
    }
    ...
    for (PatchLevel<NDIM>::Iterator p(patch_level); p; p++)
    {
        ...
        Pointer<SideData<NDIM, int> > u_dof_index_data = patch->getPatchData(u_dof_index_idx);
        const int u_dof_index_depth = u_dof_index_data->getDepth();
        u_dof_index_data->fillAll(-1); // DOFs in the ghost region abutting physical domain should be negative for PETSc
                                       // to ignore them
        Pointer<SideData<NDIM, bool> > u_mastr_loc_data = patch->getPatchData(u_mastr_loc_idx);
        Pointer<CellData<NDIM, int> > p_dof_index_data = patch->getPatchData(p_dof_index_idx);
        p_dof_index_data->fillAll(-1); // DOFs in the ghost region abutting physical domain should be negative for PETSc
                                       // to ignore them
        const int p_dof_index_depth = p_dof_index_data->getDepth();
            ...
            for (Box<NDIM>::Iterator b(bl()); b; b++)
            {
                ...
                for (unsigned int component_axis = 0; component_axis < NDIM; ++component_axis)
                {
                    if (data_boxes[component_axis].contains(ic))
                    {
                        ...
                        for (int d = 0; d < u_dof_index_depth; ++d)
                        {
                            (*u_dof_index_data)(is, d) = counter++;
                        }
                    }
                }
            }
        ...
        {
            for (int d = 0; d < p_dof_index_depth; ++d)
            {
                (*p_dof_index_data)(b(), d) = counter++;
            }
        }
    }
#if !defined(NDEBUG)
