    ...
    : ConvectiveOperator(std::move(object_name), difference_form), d_bc_coefs(std::move(bc_coefs))
{
    ...
    SAMRAIPointer<VariableContext> context = var_db->getContext("INSStaggeredCUIConvectiveOperator::CONTEXT");
...
void
INSStaggeredCUIConvectiveOperator::initializeOperatorState(const SAMRAIVectorRealNd<double>& in,
