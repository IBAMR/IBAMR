    ...
    SAMRAIPointer<SAMRAIPatchLevel> patch_level)
{
    ...
    std::array<SAMRAIIndex, uu_stencil_sz> uu_stencil(array_constant<SAMRAIIndex, uu_stencil_sz>(SAMRAIIndex(0)));
    ...
    std::array<std::array<SAMRAIIndex, up_stencil_sz>, NDIM> up_stencil(
        array_constant<std::array<SAMRAIIndex, up_stencil_sz>, NDIM>(
            array_constant<SAMRAIIndex, up_stencil_sz>(SAMRAIIndex(0))));
    ...
    std::array<SAMRAIIndex, pu_stencil_sz> pu_stencil(array_constant<SAMRAIIndex, pu_stencil_sz>(SAMRAIIndex(0)));
    ...
    for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
    {
        ...
        for (unsigned int axis = 0; axis < NDIM; ++axis)
        {
            ...
            {
                const SAMRAICellIndex& ic = b();
                const SAMRAISideIndex is(ic, axis, SAMRAISideIndex::Lower);
            ...
            for (unsigned int axis = 0, pu_stencil_index = 0; axis < NDIM; ++axis)
            {
                for (int side = 0; side <= 1; ++side, ++pu_stencil_index)
                {
                    const int pu_dof_index = (*u_dof_index_data)(
                        SAMRAISideIndex(ic + pu_stencil[pu_stencil_index], axis, SAMRAISideIndex::Lower));
...
// Set some general matrix options.
    ...
    for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
    {
        ...
        SAMRAISideData<double> uu_matrix_coefs(patch_box, uu_stencil_sz, no_ghosts);
        SAMRAISideData<double> up_matrix_coefs(patch_box, up_stencil_sz, no_ghosts);
        SAMRAICellData<double> pu_matrix_coefs(patch_box, pu_stencil_sz, no_ghosts);
    ...
    SAMRAIPointer<SAMRAICoarseFineBoundary> /*cf_boundary*/)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++, ++patch_counter)
    {
        ...
        SAMRAIPointer<SAMRAISideData<bool>> u_mastr_loc_data = patch->getPatchData(u_mastr_loc_idx);
        SAMRAIPointer<SAMRAISideData<int>> u_dof_data = patch->getPatchData(u_dof_index_idx);
        SAMRAIPointer<SAMRAICellData<int>> p_dof_data = patch->getPatchData(p_dof_index_idx);
#if !defined(NDEBUG)
        {
        ...
        for (int k = 0; k < n_patch_subdomains; ++k, ++subdomain_counter)
        {
            ...
            const SAMRAIBox& sub_box = nonoverlap_boxes[patch_counter][k];
            SAMRAIBox side_sub_box[NDIM];
            for (int axis = 0; axis < NDIM; ++axis)
            {
                side_sub_box[axis] = SAMRAISideGeometry::toSideBox(sub_box, axis);
            }

            // Get the local DOFs.
            for (int axis = 0; axis < NDIM; ++axis)
            {
                for (SAMRAIBox::Iterator b(side_sub_box[axis]); b; b++)
                {
                    const SAMRAISideIndex i_s(b(), axis, SAMRAISideIndex::Lower);
            ...
            for (SAMRAIBox::Iterator b(sub_box); b; b++)
            {
                const SAMRAICellIndex& i = b();
            ...
            const SAMRAIBox& overlap_sub_box = overlap_boxes[patch_counter][k];
            SAMRAIBox side_overlap_sub_box[NDIM];
            for (int axis = 0; axis < NDIM; ++axis)
            {
                side_overlap_sub_box[axis] = SAMRAISideGeometry::toSideBox(overlap_sub_box, axis);
            }

            // Get the overlap DOFs.
            for (int axis = 0; axis < NDIM; ++axis)
            {
                for (SAMRAIBox::Iterator b(side_overlap_sub_box[axis]); b; b++)
                {
                    const SAMRAISideIndex i_s(b(), axis, SAMRAISideIndex::Lower);
                    const int dof_idx = (*u_dof_data)(i_s);
            ...
            for (SAMRAIBox::Iterator b(overlap_sub_box); b; b++)
            {
                const SAMRAICellIndex& i = b();
                const int dof_idx = (*p_dof_data)(i);
                if (dof_idx >= 0) is_overlap[subdomain_counter].insert(dof_idx);
                                                            ...
                                                            SAMRAIPointer<SAMRAIPatchLevel> patch_level)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
    {
        ...
        for (int axis = 0; axis < NDIM; ++axis)
        {
            for (SAMRAIBox::Iterator b(side_patch_box[axis]); b; b++)
            {
                const SAMRAICellIndex& i = b();
                const SAMRAISideIndex i_s(i, axis, SAMRAISideIndex::Lower);
                const int dof_idx = (*u_dof_data)(i_s);
