 ...
 * - for each locally owned seed row, structurally coupled velocity DOFs are inserted.
 */
void
    ...
    const double relative_numerical_zero_tol)
{
    ...
    std::vector<std::set<int>> overlap_dof_sets(seed_velocity_dofs.size());
    std::vector<std::set<int>> nonoverlap_dof_sets(seed_velocity_dofs.size());
