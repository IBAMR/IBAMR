                                                           ...
                                                           int dst_ln)
{
    ...
    // A coarsen operator with a positive stencil width reads fine-level ghosts. The fine level has no DOFs
    // beyond its boundary with the coarser level, so these ghost cells hold zero, and the restriction there is
    // the volume adjoint of the prolongation. Fill the other ghosts on a copy, because filling the physical
    // boundary also overwrites normal boundary-face values for Dirichlet conditions. With no presmoothing, the source
    // is the caller's right-hand side.
                                                                  ...
                                                                  const SAMRAIVectorReal<NDIM, double>& rhs)
{
    ...
    IBAMR_DO_ONCE(geometry->addSpatialCoarsenOperator(new CartSideDoubleCubicCoarsen());
                  geometry->addSpatialCoarsenOperator(
                      new CartSideDoubleRT0Coarsen(IntVector<NDIM>(MAX_RT0_COARSEN_REFINEMENT_RATIO - 1)));
