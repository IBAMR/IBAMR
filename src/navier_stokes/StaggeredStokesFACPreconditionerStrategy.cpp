      ...
      d_coarse_solver_default_options_prefix(default_options_prefix + "level_0_")
{
    ...
    // Setup Timers.
    IBAMR_DO_ONCE(t_restrict_residual = SAMRAITimerManager::getManager()->getTimer(
...
void
StaggeredStokesFACPreconditionerStrategy::setPhysicalBcCoefs(const std::vector<SAMRAIRobinBcCoefStrategy*>& U_bc_coefs,
...
void
StaggeredStokesFACPreconditionerStrategy::setToZero(SAMRAISAMRAIVectorReal<double>& vec, int level_num)
{
    ...
    SAMRAIHierarchySideDataOpsReal<double> level_sc_data_ops(d_hierarchy, level_num, level_num);
    level_sc_data_ops.setToScalar(U_data_idx, 0.0, interior_only);
    SAMRAIHierarchyCellDataOpsReal<double> level_cc_data_ops(d_hierarchy, level_num, level_num);
    level_cc_data_ops.setToScalar(P_data_idx, 0.0, interior_only);
    return;
} // setToZero

void
StaggeredStokesFACPreconditionerStrategy::restrictResidual(const SAMRAISAMRAIVectorReal<double>& src,
                                                           ...
                                                           int dst_ln)
{
    ...
    {
        SAMRAIHierarchySideDataOpsReal<double> level_sc_data_ops(d_hierarchy, dst_ln, dst_ln);
        static const bool interior_only = false;
        level_sc_data_ops.copyData(U_dst_idx, U_src_idx, interior_only);
    }
    if (P_src_idx != P_dst_idx)
    {
        SAMRAIHierarchyCellDataOpsReal<double> level_cc_data_ops(d_hierarchy, dst_ln, dst_ln);
        static const bool interior_only = false;
...
void
StaggeredStokesFACPreconditionerStrategy::prolongError(const SAMRAISAMRAIVectorReal<double>& src,
                                                       SAMRAISAMRAIVectorReal<double>& dst,
                                                       int dst_ln)
{
...
void
StaggeredStokesFACPreconditionerStrategy::prolongErrorAndCorrect(const SAMRAISAMRAIVectorReal<double>& src,
                                                                 SAMRAISAMRAIVectorReal<double>& dst,
                                                                 int dst_ln)
{
    ...
    if (U_src_idx != U_dst_idx)
    {
        SAMRAIHierarchySideDataOpsReal<double> level_sc_data_ops_coarse(d_hierarchy, dst_ln - 1, dst_ln - 1);
        level_sc_data_ops_coarse.add(U_dst_idx, U_dst_idx, U_src_idx, interior_only);
    }
    if (P_src_idx != P_dst_idx)
    {
        SAMRAIHierarchyCellDataOpsReal<double> level_cc_data_ops_coarse(d_hierarchy, dst_ln - 1, dst_ln - 1);
    ...
    SAMRAIHierarchySideDataOpsReal<double> level_sc_data_ops_fine(d_hierarchy, dst_ln, dst_ln);
    level_sc_data_ops_fine.add(U_dst_idx, U_dst_idx, d_side_scratch_idx, interior_only);
    SAMRAIHierarchyCellDataOpsReal<double> level_cc_data_ops_fine(d_hierarchy, dst_ln, dst_ln);
...
bool
StaggeredStokesFACPreconditionerStrategy::solveCoarsestLevel(SAMRAISAMRAIVectorReal<double>& error,
...
void
StaggeredStokesFACPreconditionerStrategy::computeResidual(SAMRAISAMRAIVectorReal<double>& residual,
                                                          ...
                                                          int finest_level_num)
{
    ...
    SAMRAIPointer<SAMRAIVariableFillPattern> sc_fill_pattern =
        new SideNoCornersFillPattern(d_gcw, /*overwrite_interior*/ false);
    SAMRAIPointer<SAMRAIVariableFillPattern> cc_fill_pattern =
    ...
    d_level_math_ops[finest_level_num]->laplace(U_res_idx,
                                                ...
                                                U_res_idx,
                                                U_res_sc_var);
    SAMRAIHierarchySideDataOpsReal<double> level_sc_data_ops(d_hierarchy, coarsest_level_num, finest_level_num);
    ...
    d_level_math_ops[finest_level_num]->div(P_res_idx,
                                            ...
                                            d_new_time,
                                            /*cf_bdry_synch*/ true);
    SAMRAIHierarchyCellDataOpsReal<double> level_cc_data_ops(d_hierarchy, coarsest_level_num, finest_level_num);
    level_cc_data_ops.axpy(P_res_idx, -1.0, P_res_idx, P_rhs_idx, false);
    return;
} // computeResidual

void
StaggeredStokesFACPreconditionerStrategy::initializeOperatorState(const SAMRAISAMRAIVectorReal<double>& solution,
                                                                  const SAMRAISAMRAIVectorReal<double>& rhs)
{
    ...
    d_ghostfill_nocoarse_refine_algorithm->registerRefine(solution.getComponentDescriptorIndex(0),
                                                          solution.getComponentDescriptorIndex(0),
                                                          solution.getComponentDescriptorIndex(0),
                                                          SAMRAIPointer<SAMRAIRefineOperator>(),
                                                          d_U_op_stencil_fill_pattern);
    ...
    std::vector<SAMRAIRefinePatchStrategy*> bc_op_ptrs(2);
                                                                       ...
                                                                       const int dst_ln)
{
    ...
    SAMRAIRefineAlgorithm refine_alg;
    refine_alg.registerRefine(
        U_dst_idx, U_dst_idx, U_dst_idx, SAMRAIPointer<SAMRAIRefineOperator>(), d_U_op_stencil_fill_pattern);
    refine_alg.registerRefine(
        P_dst_idx, P_dst_idx, P_dst_idx, SAMRAIPointer<SAMRAIRefineOperator>(), d_P_op_stencil_fill_pattern);
...
StaggeredStokesFACPreconditionerStrategy::xeqScheduleDataSynch(const int U_dst_idx, const int dst_ln)
{
    SAMRAIRefineAlgorithm refine_alg;
    refine_alg.registerRefine(
        U_dst_idx, U_dst_idx, U_dst_idx, SAMRAIPointer<SAMRAIRefineOperator>(), d_U_synch_fill_pattern);
