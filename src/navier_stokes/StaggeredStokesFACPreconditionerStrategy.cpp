                                                           ...
                                                           int dst_ln)
{
    ...
    // RT0 restriction needs fine ghost values, including when zero-pre-sweep
    // FAC restricts the caller's right-hand side directly.
    if (d_U_restriction_method == "RT0_COARSEN")
    {
        xeqScheduleGhostFillNoCoarse(src_idxs, dst_ln + 1);
    }
