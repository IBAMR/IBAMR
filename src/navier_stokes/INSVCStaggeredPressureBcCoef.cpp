...
void
INSVCStaggeredPressureBcCoef::setPhysicalBcCoefs(const std::vector<SAMRAIRobinBcCoefStrategy*>& bc_coefs)
{
#if !defined(NDEBUG)
...
void
INSVCStaggeredPressureBcCoef::setBcCoefs(SAMRAIPointer<SAMRAIArrayData<double>>& acoef_data,
                                         ...
                                         double /*fill_time*/) const
{
...
#endif
    SAMRAIBox bc_coef_box = acoef_data->getBox();
#if !defined(NDEBUG)
...
#endif
    const SAMRAIBox ghost_box = u_target_data->getGhostBox() * u_current_data->getGhostBox();
    for (unsigned int d = 0; d < NDIM; ++d)
    {
    ...
    SAMRAIArray<double> A_scale = d_fluid_solver->getScalingFactor();
    ...
    for (SAMRAIBox::Iterator it(bc_coef_box); it; it++)
    {
        ...
        else if (traction_bc)
        {
            ...
            case TRACTION: // -p + 2*mu*du_n/dx_n = g.
            {
                ...
                SAMRAISideIndex i_s(i_i, bdry_normal_axis, is_lower ? SAMRAISideIndex::Lower : SAMRAISideIndex::Upper);
