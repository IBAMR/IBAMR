                ...
                for (int n = 0; n < n_physical_codim1_boxes; ++n)
                {
                    ...
                    for (BoxNd::Iterator it(bc_coef_box); it; it++)
                    {
                        ...
                        if (IBTK::rel_equal_eps(alpha, 1.0))
                            (*u_data)(SideIndexNd(i, bdry_normal_axis, SideIndexNd::Lower)) = gamma;
                    }
                }
            }
        }
    ...
    const short int bdry_tag) const
{
    ...
    for (int n = 0; n < n_physical_codim1_boxes; ++n)
    {
        ...
        const BoxNd& bc_coef_box = dirichlet_bdry_locs[n]->getBox();
        ...
        for (BoxNd::Iterator it(bc_coef_box); it; it++)
        {
                ...
                (bdry_locs_data(i, 0) == 1.0 && (bdry_tag & NORMAL_VELOCITY_BDRY)))
            {
                     ...
                     ++k, i_g(bdry_normal_axis) += (is_lower ? -1 : +1))
                {
                    ...
                    SideIndexNd i_g_s(i_g, bdry_normal_axis, is_lower ? SideIndexNd::Lower : SideIndexNd::Upper);
                    (*u_data)(i_g_s) = 0.0;
                    double div_u_g = 0.0;
                    for (unsigned int axis = 0; axis < NDIM; ++axis)
                    {
                        const SideIndexNd i_g_s_upper(i_g, axis, SideIndexNd::Upper);
                        const SideIndexNd i_g_s_lower(i_g, axis, SideIndexNd::Lower);
...
void
StaggeredStokesPhysicalBoundaryHelper::setupBcCoefObjects(const std::vector<RobinBcCoefStrategyNd*>& u_bc_coefs,
...
void
StaggeredStokesPhysicalBoundaryHelper::resetBcCoefObjects(const std::vector<RobinBcCoefStrategyNd*>& u_bc_coefs,
                                                          RobinBcCoefStrategyNd* p_bc_coef)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(u_bc_coefs.size() == NDIM);
