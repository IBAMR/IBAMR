...
void
copy_side_to_face(const int U_fc_idx, const int U_sc_idx, SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy)
{
...
INSVCStaggeredHierarchyIntegrator::INSVCStaggeredHierarchyIntegrator(std::string object_name,
    ...
    : INSHierarchyIntegrator(std::move(object_name),
                             input_db,
                             new SAMRAISideVariable<double>(object_name + "::U"),
                             "CONSERVATIVE_COARSEN",
                             "CONSERVATIVE_LINEAR_REFINE",
                             new SAMRAICellVariable<double>(object_name + "::P"),
                             "CONSERVATIVE_COARSEN",
                             "LINEAR_REFINE",
                             new SAMRAISideVariable<double>(object_name + "::F"),
                             "CONSERVATIVE_COARSEN",
                             "CONSERVATIVE_LINEAR_REFINE",
                             new SAMRAICellVariable<double>(object_name + "::Q"),
                             "CONSERVATIVE_COARSEN",
                             "CONSTANT_REFINE",
      ...
      d_mu_vc_interp_type(VC_HARMONIC_INTERP)
{
    ...
    if (!d_stokes_solver_db) d_stokes_solver_db = new SAMRAIMemoryDatabase("stokes_solver_db");
    ...
    if (!d_stokes_precond_db) d_stokes_precond_db = new SAMRAIMemoryDatabase("stokes_precond_db");
    ...
    d_N_old_var = new SAMRAISideVariable<double>(d_object_name + "::N_old");
    d_U_old_var = new SAMRAISideVariable<double>(d_object_name + "::U_old");

    d_U_cc_var = new SAMRAICellVariable<double>(d_object_name + "::U_cc", NDIM);
    d_F_cc_var = new SAMRAICellVariable<double>(d_object_name + "::F_cc", NDIM);
#if (NDIM == 2)
    d_Omega_var = new SAMRAICellVariable<double>(d_object_name + "::Omega");
#endif
#if (NDIM == 3)
    d_Omega_var = new SAMRAICellVariable<double>(d_object_name + "::Omega", NDIM);
#endif
    d_Div_U_var = new SAMRAICellVariable<double>(d_object_name + "::Div_U");

    d_U_regrid_var = new SAMRAISideVariable<double>(d_object_name + "::U_regrid");
    d_U_src_var = new SAMRAISideVariable<double>(d_object_name + "::U_src");
    d_indicator_var = new SAMRAISideVariable<double>(d_object_name + "::indicator");
    d_F_div_var = new SAMRAISideVariable<double>(d_object_name + "::F_div");
    d_EE_var = new SAMRAICellVariable<double>(d_object_name + "::EE", NDIM * NDIM);
...
void
INSVCStaggeredHierarchyIntegrator::initializeHierarchyIntegrator(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
                                                                 SAMRAIPointer<SAMRAIGriddingAlgorithm> gridding_alg)
{
    ...
    d_hier_cc_data_ops =
        hier_ops_manager->getOperationsDouble(new SAMRAICellVariable<double>("cc_var"), hierarchy, true);
    d_hier_fc_data_ops =
        hier_ops_manager->getOperationsDouble(new SAMRAIFaceVariable<double>("fc_var"), hierarchy, true);
    d_hier_sc_data_ops =
        hier_ops_manager->getOperationsDouble(new SAMRAISideVariable<double>("sc_var"), hierarchy, true);
    d_hier_nc_data_ops =
        hier_ops_manager->getOperationsDouble(new SAMRAINodeVariable<double>("nc_var"), hierarchy, true);
    d_hier_ec_data_ops =
        hier_ops_manager->getOperationsDouble(new SAMRAIEdgeVariable<double>("ec_var"), hierarchy, true);
    ...
    const SAMRAIIntVector node_ghosts = NODEG;
    const SAMRAIIntVector edge_ghosts = EDGEG;
    const SAMRAIIntVector no_ghosts = 0;
    const SAMRAIIntVector mu_cell_ghosts = MUCELLG;
    ...
    d_pressure_D_var = new SAMRAISideVariable<double>(d_object_name + "::pressure_D");
    d_pressure_D_idx = var_db->registerVariableAndContext(d_pressure_D_var, getCurrentContext(), side_ghosts);

    d_pressure_rhs_D_var = new SAMRAISideVariable<double>(d_object_name + "::pressure_rhs_D");
    d_pressure_rhs_D_idx = var_db->registerVariableAndContext(d_pressure_rhs_D_var, getCurrentContext(), side_ghosts);

    d_velocity_C_var = new SAMRAISideVariable<double>(d_object_name + "::velocity_C");
    d_velocity_C_idx = var_db->registerVariableAndContext(d_velocity_C_var, getCurrentContext(), no_ghosts);

    d_velocity_L_var = new SAMRAISideVariable<double>(d_object_name + "::velocity_L");
    d_velocity_L_idx = var_db->registerVariableAndContext(d_velocity_L_var, getCurrentContext(), side_ghosts);

    d_velocity_rhs_C_var = new SAMRAISideVariable<double>(d_object_name + "::velocity_rhs_C");
    d_velocity_rhs_C_idx = var_db->registerVariableAndContext(d_velocity_rhs_C_var, getCurrentContext(), no_ghosts);
#if (NDIM == 2)
    d_velocity_D_var = new SAMRAINodeVariable<double>(d_object_name + "::velocity_D");
    d_velocity_rhs_D_var = new SAMRAINodeVariable<double>(d_object_name + "::velocity_rhs_D");
#elif (NDIM == 3)
    d_velocity_D_var = new SAMRAIEdgeVariable<double>(d_object_name + "::velocity_D");
    d_velocity_rhs_D_var = new SAMRAIEdgeVariable<double>(d_object_name + "::velocity_rhs_D");
#endif
    ...
    d_velocity_D_cc_var = new SAMRAICellVariable<double>(d_object_name + "::velocity_D_cc");
    d_velocity_D_cc_idx = var_db->registerVariableAndContext(d_velocity_D_cc_var, getCurrentContext(), no_ghosts);

    d_temp_sc_var = new SAMRAISideVariable<double>(d_object_name + "::temp_sc");
    d_temp_sc_idx = var_db->registerVariableAndContext(d_temp_sc_var, getCurrentContext(), no_ghosts);
    d_temp_cc_var = new SAMRAICellVariable<double>(d_object_name + ":temp_cc",
                                                   /*depth*/ NDIM);
    d_temp_cc_idx = var_db->registerVariableAndContext(d_temp_cc_var, getCurrentContext(), cell_ghosts);

#if (NDIM == 2)
    d_mu_interp_var = new SAMRAINodeVariable<double>(d_object_name + "::mu_interp");
#elif (NDIM == 3)
    d_mu_interp_var = new SAMRAIEdgeVariable<double>(d_object_name + "::mu_interp");
#endif
    d_mu_interp_idx =
        var_db->registerVariableAndContext(d_mu_interp_var, getCurrentContext(), NDIM == 2 ? node_ghosts : edge_ghosts);

    d_N_full_var = new SAMRAISideVariable<double>(d_object_name + "N_full");
    ...
    SAMRAIPointer<SAMRAICoarsenOperator> coarsen_op = grid_geom->lookupCoarsenOperator(d_U_var, d_U_coarsen_type);
...
void
INSVCStaggeredHierarchyIntegrator::initializePatchHierarchy(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
                                                            SAMRAIPointer<SAMRAIGriddingAlgorithm> gridding_alg)
{
...
void
INSVCStaggeredHierarchyIntegrator::registerMassDensityVariable(SAMRAIPointer<SAMRAIVariable> rho_var)
{
#if !defined(NDEBUG)
...
void
INSVCStaggeredHierarchyIntegrator::registerViscosityVariable(SAMRAIPointer<SAMRAIVariable> mu_var)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(!d_mu_var);
    TBOX_ASSERT(!d_integrator_is_initialized);
...
void
INSVCStaggeredHierarchyIntegrator::registerViscosityBoundaryConditions(SAMRAIRobinBcCoefStrategy* mu_bc_coef)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(!d_integrator_is_initialized);
#endif
...
double
INSVCStaggeredHierarchyIntegrator::getStableTimestep(SAMRAIPointer<SAMRAIPatch> patch) const
{
...
INSVCStaggeredHierarchyIntegrator::initializeLevelDataSpecialized(
    ...
    const bool initial_time,
    const SAMRAIPointer<SAMRAIBasePatchLevel> base_old_level,
    const bool /*allocate_data*/)
{
    const SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy = base_hierarchy;
    const SAMRAIPointer<SAMRAIPatchLevel> old_level = base_old_level;
#if !defined(NDEBUG)
    ...
    if (!initial_time && level_number > 0)
    {
        ...
        SAMRAIComponentSelector scratch_data;
        ...
        if (old_level)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(old_level); p; p++)
            {
            ...
            // the old patch level.
            SAMRAIRefineAlgorithm copy_data;
            ...
            SAMRAIComponentSelector bc_fill_data;
        ...
        SAMRAIRefineAlgorithm fill_div_free_prolongation;
        SAMRAIPointer<SAMRAICartesianGridGeometry> grid_geom = d_hierarchy->getGridGeometry();
        fill_div_free_prolongation.registerRefine(d_U_current_idx, d_U_current_idx, d_U_regrid_idx, nullptr);
        SAMRAIPointer<SAMRAIRefineOperator> refine_op = grid_geom->lookupRefineOperator(d_U_var, d_U_refine_type);
        SAMRAIPointer<SAMRAICoarsenOperator> coarsen_op = grid_geom->lookupCoarsenOperator(d_U_var, d_U_coarsen_type);
...
{
    SAMRAIPointer<SAMRAIVariableContext> ctx = getCurrentContext();
...
INSVCStaggeredHierarchyIntegrator::preprocessOperatorsAndSolvers(const double current_time, const double new_time)
{
    ...
    {
        d_U_scratch_vec =
            new SAMRAISAMRAIVectorReal<double>(d_object_name + "::U_scratch_vec", d_hierarchy, coarsest_ln, finest_ln);
        d_U_scratch_vec->addComponent(d_U_var, d_U_scratch_idx, wgt_sc_idx, d_hier_sc_data_ops);

        d_P_scratch_vec =
            new SAMRAISAMRAIVectorReal<double>(d_object_name + "::P_scratch_vec", d_hierarchy, coarsest_ln, finest_ln);
        ...
        d_sol_vec =
            new SAMRAISAMRAIVectorReal<double>(d_object_name + "::sol_vec", d_hierarchy, coarsest_ln, finest_ln);
        ...
        d_rhs_vec =
            new SAMRAISAMRAIVectorReal<double>(d_object_name + "::rhs_vec", d_hierarchy, coarsest_ln, finest_ln);
