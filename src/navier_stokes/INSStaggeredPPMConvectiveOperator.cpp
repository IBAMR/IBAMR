    ...
    : ConvectiveOperator(std::move(object_name), difference_form), d_bc_coefs(std::move(bc_coefs))
{
    ...
    SAMRAIPointer<SAMRAIVariableContext> context = var_db->getContext("INSStaggeredPPMConvectiveOperator::CONTEXT");
    ...
    IBAMR_DO_ONCE(t_apply_convective_operator = SAMRAITimerManager::getManager()->getTimer(
                      ...
                      ")");
                  t_apply =
                      SAMRAITimerManager::getManager()->getTimer("IBAMR::INSStaggeredPPMConvectiveOperator::apply()");
...
void
INSStaggeredPPMConvectiveOperator::initializeOperatorState(const SAMRAISAMRAIVectorReal<double>& in,
