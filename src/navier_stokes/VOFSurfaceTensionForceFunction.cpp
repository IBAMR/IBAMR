...
TBOX_ASSERT(patch->checkAllocated(d_alpha_idx));
    ...
    // interface is phi created from the parent
    // Pointer<CellData<NDIM, double>> Phi = patch->getPatchData(d_phi_idx);

    // const int alpha_gcw = d_hf_column_half_width + 1;

    // CellData<NDIM, double> alpha(patch_box, 1, IntVector<NDIM>(alpha_gcw)); // local VOF scratch cell centered data
    //                                                                         // array
    // Box<NDIM> alpha_box = patch_box;                                        // copy patch interior boc
    // alpha_box.grow(IntVector<NDIM>(alpha_gcw));                             // expand in the ghost region
    ...
    //     std::array<double, NDIM> dPhi; // This is strictly for 2D, it is okay for testing but not final
    //     implementation dPhi[0] = 0.5 * ((*Phi)(ci + CellIndex<NDIM>(1, 0)) - (*Phi)(ci - CellIndex<NDIM>(1, 0))); //
    //     xplus, xminus dPhi[1] = 0.5 * ((*Phi)(ci + CellIndex<NDIM>(0, 1)) - (*Phi)(ci - CellIndex<NDIM>(0, 1)));
