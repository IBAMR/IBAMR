      ...
      d_velocity_bc_scaling(NDIM == 2 ? 2.0 : 5.0 / 3.0)
{
    ...
    d_W_cc_var = new SAMRAICellVariable<double>(d_object_name + "::W_cc", NDIM);
...
#if (NDIM == 2)
    d_W_nc_var = new SAMRAINodeVariable<double>(d_object_name + "::W_nc", 2);
    static const SAMRAIIntVector ghosts_nc = 0;
...
#endif
#if (NDIM == 3)
    d_W_ec_var = new SAMRAIEdgeVariable<double>(d_object_name + "::W_ec", 2);
    static const SAMRAIIntVector ghosts_ec = 0;
...
// one of the two, we do more work than necessary.
                    ...
                    {
                        const SAMRAINodeIndex i_n(it(), 0);
                    ...
                    {
                        for (SAMRAIBox::Iterator it(SAMRAIEdgeGeometry::toEdgeBox(patch_box, axis)); it; it++)
                        {
                            const SAMRAIEdgeIndex i_e(it(), axis, 0);
...
#if (NDIM == 2)
                const SAMRAIBox node_box = SAMRAINodeGeometry::toNodeBox(patch_box);
                for (int n = 0; n < n_physical_codim1_boxes; ++n)
                {
                    ...
                    for (int d = 0; d < NDIM; ++d)
                    {
                        SAMRAIRobinBcCoefStrategy* bc_coef = u_bc_coefs[d];
                        bc_coef->setBcCoefs(
                            acoef_data, bcoef_data, gcoef_data, var, *patch, trimmed_bdry_box, data_time);
                        for (SAMRAIBox::Iterator it(bc_coef_box * node_box); it; it++)
                        {
                            const SAMRAIIndex& i = it();
                            const SAMRAINodeIndex n_i(i, 0);
...
#endif
#if (NDIM == 3)
                SAMRAIBox edge_boxes[NDIM];
                for (int d = 0; d < NDIM; ++d)
                {
                    edge_boxes[d] = SAMRAIEdgeGeometry::toEdgeBox(patch_box, d);
                }
                for (int n = 0; n < n_physical_codim1_boxes; ++n)
                {
                    const SAMRAIBoundaryBox& bdry_box = physical_codim1_boxes[n];
                    ...
                    for (int edge_axis = 0; edge_axis < NDIM; ++edge_axis)
                    {
                        ...
                        for (int d = 0; d < NDIM; ++d)
                        {
                            ...
                            bc_coef->setBcCoefs(
                                acoef_data, bcoef_data, gcoef_data, var, *patch, trimmed_bdry_box, data_time);
                            for (SAMRAIBox::Iterator it(bc_coef_box * edge_boxes[edge_axis]); it; it++)
                            {
                                const SAMRAIIndex& i = it();
                                const SAMRAIEdgeIndex e_i(i, edge_axis, 0);
                                const double& alpha = (*acoef_data)(i, 0);
                                const double& beta = (*bcoef_data)(i, 0);
                                const bool velocity_bc = (alpha != 0.0 && beta == 0.0);
                                if (velocity_bc)
        ...
        SAMRAILocationIndexRobinBcCoefs bc_coef(d_object_name + "::bc_coef", SAMRAIPointer<SAMRAIDatabase>(nullptr));
        for (int d = 0; d < NDIM; ++d)
        {
            bc_coef.setBoundarySlope(2 * d, 0.0);
            bc_coef.setBoundarySlope(2 * d + 1, 0.0);
        }
        std::vector<SAMRAIRobinBcCoefStrategy*> bc_coefs(NDIM, &bc_coef);
