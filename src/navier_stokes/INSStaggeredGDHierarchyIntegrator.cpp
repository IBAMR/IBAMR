...
#include "ibamr/INSStaggeredGDHierarchyIntegrator.h"
...
INSStaggeredGDHierarchyIntegrator::INSStaggeredGDHierarchyIntegrator(std::string object_name,
                             ...
                             register_for_restart)
{
    ...
    IBTK_DO_ONCE(t_setup_plot_data_specialized =
                     set_timer("IBTK::INSStaggeredGDHierarchyIntegrator::setupPlotDataSpecialized()"););
    ...
    case TRAPEZOIDAL_RULE:
        break;
    default:
        TBOX_ERROR(d_object_name << "::INSStaggeredGDHierarchyIntegrator():\n"
    ...
    return;
} // INSStaggeredGDHierarchyIntegrator

INSStaggeredGDHierarchyIntegrator::~INSStaggeredGDHierarchyIntegrator()
{
    for (unsigned int d = 0; d < NDIM; ++d)
    {
...
INSStaggeredGDHierarchyIntegrator::getConvectiveOperator()
{
    if (d_creeping_flow)
    {
...
INSStaggeredGDHierarchyIntegrator::getVelocitySubdomainSolver()
{
...
Pointer<PoissonSolver>
INSStaggeredGDHierarchyIntegrator::getPressureSubdomainSolver()
{
...
void
INSStaggeredGDHierarchyIntegrator::setStokesSolver(Pointer<StaggeredStokesSolver> stokes_solver)
{
#if !defined(NDEBUG)
...
INSStaggeredGDHierarchyIntegrator::getStokesSolver()
{
    if (!d_stokes_solver)
    {
        d_stokes_solver =
...
void
INSStaggeredGDHierarchyIntegrator::setStokesSolverNeedsInit()
{
    d_stokes_solver_needs_init = true;
    return;
}
...
void
INSStaggeredGDHierarchyIntegrator::initializeHierarchyIntegrator(Pointer<PatchHierarchy<NDIM> > hierarchy,
...
void
INSStaggeredGDHierarchyIntegrator::initializePatchHierarchy(Pointer<PatchHierarchy<NDIM> > hierarchy,
                                                          Pointer<GriddingAlgorithm<NDIM> > gridding_alg)
{
...
void
INSStaggeredGDHierarchyIntegrator::preprocessIntegrateHierarchy(const double current_time,
...
void
INSStaggeredGDHierarchyIntegrator::integrateHierarchySpecialized(const double current_time,
                                                               const double new_time,
                                                               const int cycle_num)
{
    ...
    {
        IBAMR_DO_ONCE({
            pout << "INSStaggeredGDHierarchyIntegrator::integrateHierarchy():\n"
...
void
INSStaggeredGDHierarchyIntegrator::postprocessIntegrateHierarchy(const double current_time,
                                                               const double new_time,
...
void
INSStaggeredGDHierarchyIntegrator::setupSolverVectors(const Pointer<SAMRAIVectorReal<NDIM, double> >& sol_vec,
...
void
INSStaggeredGDHierarchyIntegrator::resetSolverVectors(const Pointer<SAMRAIVectorReal<NDIM, double> >& sol_vec,
                                                    const Pointer<SAMRAIVectorReal<NDIM, double> >& rhs_vec,
                                                    const double current_time,
                                                    const double /*new_time*/,
                                                    const int cycle_num)
...
void
INSStaggeredGDHierarchyIntegrator::removeNullSpace(const Pointer<SAMRAIVectorReal<NDIM, double> >& sol_vec)
{
...
double
INSStaggeredGDHierarchyIntegrator::getStableTimestep(Pointer<Patch<NDIM> > patch) const
{
...
void
INSStaggeredGDHierarchyIntegrator::putToDatabaseSpecialized(Pointer<Database> db)
{
...
void
INSStaggeredGDHierarchyIntegrator::getFromRestart()
{
...
void
INSStaggeredGDHierarchyIntegrator::regridHierarchyBeginSpecialized()
{
    const int coarsest_ln = 0;
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
...
void
INSStaggeredGDHierarchyIntegrator::regridHierarchyEndSpecialized()
{
    const int wgt_cc_idx = d_hier_math_ops->getCellWeightPatchDescriptorIndex();
...
void
INSStaggeredGDHierarchyIntegrator::initializeCompositeHierarchyDataSpecialized(const double /*init_data_time*/,
...
void
INSStaggeredGDHierarchyIntegrator::initializeLevelDataSpecialized(const Pointer<BasePatchHierarchy<NDIM> > base_hierarchy,
...
void
INSStaggeredGDHierarchyIntegrator::resetHierarchyConfigurationSpecialized(
...
void
INSStaggeredGDHierarchyIntegrator::applyGradientDetectorSpecialized(const Pointer<BasePatchHierarchy<NDIM> > hierarchy,
                                                                  const int level_number,
...
void
INSStaggeredGDHierarchyIntegrator::setupPlotDataSpecialized()
{
...
void
INSStaggeredGDHierarchyIntegrator::regridProjection()
{
    const int coarsest_ln = 0;
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
    const int wgt_cc_idx = d_hier_math_ops->getCellWeightPatchDescriptorIndex();
...
void
INSStaggeredGDHierarchyIntegrator::reinitializeOperatorsAndSolvers(const double current_time, const double new_time)
{
    const int coarsest_ln = 0;
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
    const double dt = new_time - current_time;
...
void
INSStaggeredGDHierarchyIntegrator::computeDivSourceTerm(const int F_idx, const int Q_idx, const int U_idx)
{
    const int coarsest_ln = 0;
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            default:
                TBOX_ERROR(
                    "INSStaggeredGDHierarchyIntegrator::computeDivSourceTerm():\n"
...
TimeSteppingType
INSStaggeredGDHierarchyIntegrator::getConvectiveTimeSteppingType(const int cycle_num)
{
    TimeSteppingType convective_time_stepping_type = d_convective_time_stepping_type;
...
double
INSStaggeredGDHierarchyIntegrator::getTimeStepSizeRatio() const
{
    if (getIntegratorStep() == 0)
    {
        return 1.0;
...
void
INSStaggeredGDHierarchyIntegrator::applyDivergencePreservingProlongation(const int U_idx,
                                                                       const int level_number,
