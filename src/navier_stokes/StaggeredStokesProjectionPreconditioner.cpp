                                         ...
                                         /*needs_pressure_solver*/ true)
{
    ...
    IBAMR_DO_ONCE(t_solve_system = TimerManager::getManager()->getTimer(
                  ...
                  t_initialize_solver_state =
                      TimerManager::getManager()->getTimer("IBAMR::StaggeredStokesProjectionPreconditioner::"
...
bool
StaggeredStokesProjectionPreconditioner::solveSystem(SAMRAIVectorRealNd<double>& x, SAMRAIVectorRealNd<double>& b)
{
    ...
    Pointer<SAMRAIVectorRealNd<double> > Phi_scratch_vec;
    Phi_scratch_vec =
    ...
    Pointer<SAMRAIVectorRealNd<double> > F_Phi_vec;
...
void
StaggeredStokesProjectionPreconditioner::initializeSolverState(const SAMRAIVectorRealNd<double>& x,
...
StaggeredStokesProjectionPreconditioner::setInitialGuessNonzero(bool initial_guess_nonzero)
{
    if (initial_guess_nonzero)
    {
        TBOX_ERROR(d_object_name + "::setInitialGuessNonzero()\n"
                   << "  class IBAMR::StaggeredStokesProjectionPreconditioner "
                      "requires a zero initial guess"
                   << std::endl);
    }
    return;
...
StaggeredStokesProjectionPreconditioner::setMaxIterations(int max_iterations)
{
    if (max_iterations != 1)
    {
        TBOX_ERROR(d_object_name + "::setMaxIterations()\n"
                   << "  class IBAMR::StaggeredStokesProjectionPreconditioner only "
