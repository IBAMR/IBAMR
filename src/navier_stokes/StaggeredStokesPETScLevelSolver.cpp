 ...
 *
 * Used in place of inferring coverage from an absence of type-1 coarse-fine boundary boxes on the level:
 * this is the actual property the pressure nullspace test needs, checked directly, and it does not depend
 * on d_cf_boundary having been computed for the level, which a hand-built test hierarchy need not do.
 */
bool
...
StaggeredStokesPETScLevelSolver::setOperatorMat(Mat operator_mat)
{
    if (d_is_initialized)
    {
        TBOX_ERROR(d_object_name << "::setOperatorMat():\n"
...
StaggeredStokesPETScLevelSolver::setAugmentedOperatorMat(Mat augmented_operator_mat)
{
    if (d_is_initialized)
    {
        TBOX_ERROR(d_object_name << "::setAugmentedOperatorMat():\n"
                                 << "  deallocate the solver state before changing the matrix.\n");
    }
                                                                  ...
                                                                  const SAMRAIVectorReal<NDIM, double>& /*b*/)
{
    ...
    {
        // The solver may attach its own nullspace to the caller's matrix, so keep the caller's for deallocation.
        int ierr = MatNullSpaceDestroy(&d_operator_mat_prior_nullsp);
        IBTK_CHKERRQ(ierr);
        ierr = MatGetNullSpace(d_petsc_mat, &d_operator_mat_prior_nullsp);
        IBTK_CHKERRQ(ierr);
        if (d_operator_mat_prior_nullsp)
        {
            ierr = PetscObjectReference(reinterpret_cast<PetscObject>(d_operator_mat_prior_nullsp));
            IBTK_CHKERRQ(ierr);
        }
    }
...
StaggeredStokesPETScLevelSolver::deallocateSolverStateSpecialized()
{
    if (d_operator_mat && d_petsc_mat == d_operator_mat)
    {
        ...
        if (d_petsc_nullsp)
        {
            // Replace the nullspace that this solver attached to the caller's matrix by the caller's own, if any.
            ierr = MatSetNullSpace(d_petsc_mat, d_operator_mat_prior_nullsp);
            IBTK_CHKERRQ(ierr);
        }
        ierr = MatNullSpaceDestroy(&d_operator_mat_prior_nullsp);
        IBTK_CHKERRQ(ierr);
        d_petsc_mat = nullptr;
        d_petsc_pc = nullptr;
    }
