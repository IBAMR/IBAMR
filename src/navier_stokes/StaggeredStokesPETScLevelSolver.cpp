                                                                 ...
                                                                 const std::string& default_options_prefix)
{
    ...
    d_u_dof_index_var = new SAMRAISideVariable<int>(object_name + "::u_dof_index");
    ...
    d_p_dof_index_var = new SAMRAICellVariable<int>(object_name + "::p_dof_index");
    ...
    d_u_nullspace_var = new SAMRAISideVariable<double>(object_name + "::u_nullspace_var");
    ...
    d_p_nullspace_var = new SAMRAICellVariable<double>(object_name + "::p_nullspace_var");
...
void
StaggeredStokesPETScLevelSolver::initializeSolverStateSpecialized(const SAMRAISAMRAIVectorReal<double>& x,
...
void
StaggeredStokesPETScLevelSolver::copyToPETScVec(Vec& petsc_x, SAMRAISAMRAIVectorReal<double>& x)
{
    const int u_idx = x.getComponentDescriptorIndex(0);
    const int p_idx = x.getComponentDescriptorIndex(1);
...
void
StaggeredStokesPETScLevelSolver::copyFromPETScVec(Vec& petsc_x, SAMRAISAMRAIVectorReal<double>& x)
{
    const int u_idx = x.getComponentDescriptorIndex(0);
    const int p_idx = x.getComponentDescriptorIndex(1);
                                              ...
                                              SAMRAISAMRAIVectorReal<double>& b)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(d_level); p; p++)
    {
        ...
        const SAMRAIArray<SAMRAIBoundaryBox>& type_1_cf_bdry = level_zero ?
