...
INSStaggeredStabilizedPPMConvectiveOperator::applyConvectiveOperator(const int U_idx, const int N_idx)
{
    ...
    Pointer<GridGeometryNd> grid_geometry = d_hierarchy->getGridGeometry();
    for (int ln = d_coarsest_ln; ln <= d_finest_ln; ++ln)
    {
        Pointer<PatchLevelNd> level = d_hierarchy->getPatchLevel(ln);
        const IntVectorNd& ratio = level->getRatio();
        const BoxNd domain_box = BoxNd::refine(grid_geometry->getPhysicalDomain()[0], ratio);
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            Pointer<SideDataNd<double> > N_upwind_data =
            ...
            std::array<Pointer<FaceDataNd<double> >, NDIM> U_half_upwind_data;
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                ...
                U_half_upwind_data[axis] = new FaceDataNd<double>(side_boxes[axis], 1, ghosts);
            }
...
// the control volumes.
            ...
            for (unsigned int axis = 0; axis < NDIM; ++axis)
            {
                Pointer<SideDataNd<double> > dU_data =
                    new SideDataNd<double>(U_data->getBox(), U_data->getDepth(), U_data->getGhostCellWidth());
                Pointer<SideDataNd<double> > U_L_data =
                    new SideDataNd<double>(U_data->getBox(), U_data->getDepth(), U_data->getGhostCellWidth());
                Pointer<SideDataNd<double> > U_R_data =
                    new SideDataNd<double>(U_data->getBox(), U_data->getDepth(), U_data->getGhostCellWidth());
                Pointer<SideDataNd<double> > U_scratch1_data =
                    new SideDataNd<double>(U_data->getBox(), U_data->getDepth(), U_data->getGhostCellWidth());
#if (NDIM == 3)
                Pointer<SideDataNd<double> > U_scratch2_data =
                    new SideDataNd<double>(U_data->getBox(), U_data->getDepth(), U_data->getGhostCellWidth());
#endif
#if (NDIM == 2)
            ...
            if (patch_geom->getTouchesRegularBoundary())
            {
                Pointer<SideDataNd<double> > N_PPM_data =
                    new SideDataNd<double>(N_data->getBox(), N_data->getDepth(), N_data->getGhostCellWidth());
                ...
                for (unsigned int location_index = 0; location_index < 2 * NDIM; ++location_index)
                {
                    ...
                    if (d_open_bdry[location_index] && patch_geom->getTouchesRegularBoundary(axis, side))
                    {
                        for (unsigned int d = 0; d < NDIM; ++d)
                        {
                            ...
                            for (BoxNd::Iterator b(SideGeometryNd::toSideBox(bdry_box * patch_box, d)); b; b++)
                            {
                                const hier::IndexNd& i = b();
                                const SideIndexNd i_s(i, d, SideIndexNd::Lower);
                                const double x =
...
void
INSStaggeredStabilizedPPMConvectiveOperator::initializeOperatorState(const SAMRAIVectorRealNd<double>& in,
