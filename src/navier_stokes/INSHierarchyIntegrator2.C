...
void
INSHierarchyIntegrator2::registerDivergenceSpecification(
...
void
INSHierarchyIntegrator2::registerIrregularCellPatchDescriptorIndex(
...
///
///  allow the INSHierarchyIntegrator2 to be used as a hierarchy integrator.
///
    ...
    SAMRAI::tbox::Pointer<SAMRAI::mesh::GriddingAlgorithm<NDIM> > gridding_alg)
{
    ...
    //
    // All other data is managed by the INSHierarchyIntegrator2.
    ...
    if (d_second_order_pressure_update)
    {
        d_helmholtz1_spec = new SAMRAI::solv::PoissonSpecifications(d_object_name+"::helmholtz1_spec");
        d_helmholtz1_op = new STOOLS::CCLaplaceOperator(d_object_name+"::helmholtz1_op",*d_helmholtz1_spec,std::vector<const SAMRAI::solv::RobinBcCoefStrategy<NDIM>*>(NDIM,NULL));
...
///      resetHierDataToPreadvanceState()
///
///  allow the INSHierarchyIntegrator2 to provide data management for a time
    ...
    const double new_time)
{
    ...
    if (!d_Q_var.isNull())
    {
        computeDivSourceTerm(
            ...
            coarsest_ln, finest_ln);
        d_hier_cc_data_ops->add(d_G_idx, d_G_idx, d_F_div_current_idx);
    }
    ...
    // Rescale grad_Phi so that it has the appropriate units to enfore
    // incompressibility.
    ...
    const double new_time)
{
    ...
    if (!d_Q_set.isNull())
    {
        d_hier_cc_data_ops->add(d_Grad_P_current_idx, d_Grad_P_current_idx, d_F_div_new_idx);
    }
    ...
    if (!d_Q_set.isNull())
    {
        d_hier_cc_data_ops->subtract(d_Grad_P_current_idx, d_Grad_P_current_idx, d_F_div_new_idx);
    }
    ...
    const double new_time)
{
    ...
    // Compute u^(*) from U^(*).
    SAMRAI::tbox::Pointer<SAMRAI::geom::CartesianGridGeometry<NDIM> > grid_geom = d_hierarchy->getGridGeometry();
    SAMRAI::tbox::Pointer<SAMRAI::xfer::RefineOperator<NDIM> > bdry_fill_op = grid_geom->lookupRefineOperator(d_U_var, "CONSTANT_REFINE");
    ...
    const bool override_current_pressure)
{
    ...
    if (d_second_order_pressure_update)
    {
        ...
        if (d_do_log) SAMRAI::tbox::plog << "INSHierarchyIntegrator2::updatePressure(): number of iterations = " << d_helmholtz4_solver->getNumIterations() << "\n";
        if (d_do_log) SAMRAI::tbox::plog << "INSHierarchyIntegrator2::updatePressure(): residual norm        = " << d_helmholtz4_solver->getResidualNorm()  << "\n";
    ...
    const int finest_level)
{
    ...
    // Indicate that all Helmholtz solvers must be re-initialized.
    d_helmholtz_solvers_need_init = true;
...
SAMRAI::tbox::Pointer<SAMRAI::pdat::CellVariable<NDIM,double> >
INSHierarchyIntegrator2::getDivergenceVar()
{
    return d_Q_var;
}// getDivergenceVar
...
void
INSHierarchyIntegrator2::computeDivSourceTerm(
    const int F_idx,
    const int Q_idx,
    const int u_idx,
...
void
INSHierarchyIntegrator2::resetCellVelocityBoundaryConditions(
    const int U_idx,
    const double time,
...
void
INSHierarchyIntegrator2::resetMACVelocityBoundaryConditions(
    const int u_idx,
    const double time,
    const std::vector<SAMRAI::tbox::Pointer<SAMRAI::xfer::CoarsenSchedule<NDIM> > >& cscheds,
    const int coarsest_ln,
