    ...
    : ConvectiveOperator(std::move(object_name), difference_form)
{
    ...
    SAMRAIPointer<SAMRAIVariableContext> context = var_db->getContext("INSCollocatedPPMConvectiveOperator::CONTEXT");
    ...
    {
        d_u_extrap_var = new SAMRAIFaceVariable<double>(u_extrap_var_name, NDIM);
        d_u_extrap_idx = var_db->registerVariableAndContext(d_u_extrap_var, context, SAMRAIIntVector(0));
    }
#if !defined(NDEBUG)
    ...
    {
        d_u_flux_var = new SAMRAIFaceVariable<double>(u_flux_var_name, NDIM);
        d_u_flux_idx = var_db->registerVariableAndContext(d_u_flux_var, context, SAMRAIIntVector(0));
    }
#if !defined(NDEBUG)
    ...
    IBAMR_DO_ONCE(t_apply_convective_operator = SAMRAITimerManager::getManager()->getTimer(
                      ...
                      ")");
                  t_apply =
                      SAMRAITimerManager::getManager()->getTimer("IBAMR::INSCollocatedPPMConvectiveOperator::apply()");
...
#endif
            SAMRAICellData<double>& U0_data = *U_data;
            SAMRAICellData<double> U1_data(patch_box, 1, U_data_gcw);
#if (NDIM == 3)
            SAMRAICellData<double> U2_data(patch_box, 1, U_data_gcw);
#endif
            SAMRAICellData<double> dU_data(patch_box, 1, U_data_gcw);
            SAMRAICellData<double> U_L_data(patch_box, 1, U_data_gcw);
            SAMRAICellData<double> U_R_data(patch_box, 1, U_data_gcw);
...
void
INSCollocatedPPMConvectiveOperator::initializeOperatorState(const SAMRAISAMRAIVectorReal<double>& in,
                                                            const SAMRAISAMRAIVectorReal<double>& out)
{
    ...
    SAMRAIPointer<SAMRAIRefineOperator> refine_op =
        grid_geom->lookupRefineOperator(d_U_var, "CONSERVATIVE_LINEAR_REFINE");
    d_ghostfill_alg = new SAMRAIRefineAlgorithm();
