           ...
           void* ctx)
{
    ...
    const int ls_idx = var_db->mapVariableAndContextToIndex(
        ls_tagger->d_ls_container.getLevelSetVariable(),
        ls_tagger->d_ls_container.getAdvDiffHierarchyIntegrator()->getCurrentContext());
...
LevelSetMassLossFixer::putToDatabase(Pointer<Database> db)
{
    ...
    db->putDouble("ncells", d_ls_container.d_ncells);
                       ...
                       void* ctx)
{
    ...
    Pointer<AdvDiffHierarchyIntegrator> adv_diff_integrator =
        mass_fixer->d_ls_container.getAdvDiffHierarchyIntegrator();
    ...
    const double ncells = mass_fixer->d_ls_container.getInterfaceHalfWidth();
    ...
    VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
    const int ls_idx = var_db->mapVariableAndContextToIndex(mass_fixer->d_ls_container.getLevelSetVariable(),
                       ...
                       void* ctx)
{
    ...
    // Hence the application time would be the new time and the variable context would be the new context.
    VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
    const int fluid_ls_idx = var_db->mapVariableAndContextToIndex(mass_fixer->d_ls_container.getLevelSetVariable(0),
                                                                  adv_diff_integrator->getNewContext());
    const int solid_ls_idx = var_db->mapVariableAndContextToIndex(mass_fixer->d_ls_container.getLevelSetVariable(1),
                                                                  adv_diff_integrator->getNewContext());
...
LevelSetMassLossFixer::getFromInput(Pointer<Database> input_db)
{
    ...
    d_ls_container.d_ncells = input_db->getDoubleWithDefault("half_width", 1.0);
...
LevelSetMassLossFixer::getFromRestart()
{
    ...
    d_ls_container.d_ncells = db->getDouble("ncells");
