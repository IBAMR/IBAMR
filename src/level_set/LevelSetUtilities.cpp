           ...
           void* ctx)
{
    ...
    const int ls_idx = var_db->mapVariableAndContextToIndex(
        ls_tagger->getLevelSetContainer().getLevelSetVariable(),
        ls_tagger->getLevelSetContainer().getAdvDiffHierarchyIntegrator()->getCurrentContext());
...
LevelSetMassLossFixer::putToDatabase(Pointer<Database> db)
{
    ...
    db->putDouble("ncells", getLevelSetContainer().getInterfaceHalfWidth());
                       ...
                       void* ctx)
{
    ...
    Pointer<AdvDiffHierarchyIntegrator> adv_diff_integrator =
        mass_fixer->getLevelSetContainer().getAdvDiffHierarchyIntegrator();
    ...
    const double ncells = mass_fixer->getLevelSetContainer().getInterfaceHalfWidth();
    ...
    VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
    const int ls_idx = var_db->mapVariableAndContextToIndex(mass_fixer->getLevelSetContainer().getLevelSetVariable(),
                                                            adv_diff_integrator->getNewContext());
                       ...
                       void* ctx)
{
    ...
    // Hence the application time would be the new time and the variable context would be the new context.
    VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
    const int fluid_ls_idx = var_db->mapVariableAndContextToIndex(
        mass_fixer->getLevelSetContainer().getLevelSetVariable(0), adv_diff_integrator->getNewContext());
    const int solid_ls_idx = var_db->mapVariableAndContextToIndex(
        mass_fixer->getLevelSetContainer().getLevelSetVariable(1), adv_diff_integrator->getNewContext());
...
LevelSetMassLossFixer::getFromInput(Pointer<Database> input_db)
{
    ...
    getLevelSetContainer().setInterfaceHalfWidth(input_db->getDoubleWithDefault("half_width", 1.0));
...
LevelSetMassLossFixer::getFromRestart()
{
    ...
    getLevelSetContainer().setInterfaceHalfWidth(db->getDouble("ncells"));
