...
compute_heaviside_integrals(Pointer<HierarchyMathOps> hier_math_ops, int phi_idx, double ncells)
{
    ...
    Pointer<SAMRAIPatchHierarchy> patch_hier = hier_math_ops->getPatchHierarchy();
    ...
    for (int ln = 0; ln <= hier_finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> wgt_data = patch->getPatchData(wgt_cc_idx);
...
compute_heaviside_integrals(Pointer<HierarchyMathOps> hier_math_ops, int phi_idx, int psi_idx, double ncells)
{
    ...
    for (int ln = 0; ln <= hier_finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> phi_data = patch->getPatchData(phi_idx);
            Pointer<SAMRAICellData<double>> psi_data = patch->getPatchData(psi_idx);
            Pointer<SAMRAICellData<double>> wgt_data = patch->getPatchData(wgt_cc_idx);
...
{
void
tagLSCells(Pointer<SAMRAIBasePatchHierarchy> hierarchy,
           ...
           void* ctx)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(level); p; p++)
    {
        ...
        Pointer<SAMRAICellData<double>> ls_data = patch->getPatchData(ls_idx);
...
void
LevelSetMassLossFixer::putToDatabase(Pointer<SAMRAIDatabase> db)
{
...
{
    if (SAMRAIRestartManager::getManager()->isFromRestart()) return;
                       ...
                       void* ctx)
{
    ...
    Pointer<SAMRAIPatchHierarchy> patch_hier = adv_diff_integrator->getPatchHierarchy();
    ...
    double q = 0.0;
    SAMRAIHierarchyCellDataOpsReal<double> hier_cc_ops(patch_hier, 0, hier_finest_ln);
...
void
LevelSetMassLossFixer::getFromInput(Pointer<SAMRAIDatabase> input_db)
{
