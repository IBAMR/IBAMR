...
{
// Timers.
static SAMRAIPointer<Timer> t_collectNeighboringPatchElements;
static SAMRAIPointer<Timer> t_buildIntersectionMap;
...
FESurfaceDistanceEvaluator::mapIntersections()
{
    ...
    // indexing.
    SAMRAIPointer<PatchLevelNd> level = d_patch_hierarchy->getPatchLevel(finest_ln);
    ...
    auto el_it = d_mesh.active_local_elements_begin();
    auto el_end = d_mesh.active_local_elements_end();
    ...
    SAMRAIPointer<CartesianGridGeometryNd> grid_geom = level->getGridGeometry();
...
FESurfaceDistanceEvaluator::computeSignedDistance(int n_idx, int d_idx)
{
    ...
    for (PatchLevelNd::Iterator p(level); p; p++)
    {
        ...
        for (BoxNd::Iterator it(patch_box); it; it++)
        {
            ...
            if (contains_key)
            {
                std::set<const Elem*>& elem_set = d_cell_elem_neighbor_map[ci];
...
FESurfaceDistanceEvaluator::calculateSurfaceNormals()
{
    ...
    auto el_it = d_mesh.active_elements_begin();
    auto el_end = d_mesh.active_elements_end();
    if (d_use_vol_extracted_bdry_mesh)
    {
    ...
    for (; el_it != el_end; ++el_it)
    {
        ...
        std::array<const Node*, 2> edge_nodes;
        ...
        for (std::size_t edge_n = 0; edge_n < 3; ++edge_n)
        {
            ...
            std::pair<const Node*, const Node*> edge(edge_nodes[0], edge_nodes[1]);
                                                        ...
                                                        double large_distance)
{
    ...
    SAMRAIPointer<SAMRAI::hier::VariableNd> data_var;
    ...
    SAMRAIPointer<PatchLevelNd> level = patch_hierarchy->getPatchLevel(finest_ln);
    ...
    SAMRAIPointer<RefineScheduleNd> ghost_fill_sched = ghost_fill_alg.createSchedule(level);
    ...
    while (n_global_updates > 0)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            SAMRAIPointer<CellDataNd<double> > D_iter_data = patch->getPatchData(D_iter_idx);
...
FESurfaceDistanceEvaluator::getClosestPointandAngleWeightedNormal3D(const IBTK::Vector3d& P, const libMesh::Elem* elem)
{
    const libMesh::Point& n0 = elem->point(0);
    const libMesh::Point& n1 = elem->point(1);
    const libMesh::Point& n2 = elem->point(2);
    ...
    std::array<const Node*, 2> edge_nodes;
    std::set<const Elem*> elem_set;
    std::pair<const Node*, const Node*> edge;
...
FESurfaceDistanceEvaluator::collectNeighboringPatchElements(int level_number)
{
    ...
    SAMRAIPointer<PatchLevelNd> level = d_patch_hierarchy->getPatchLevel(level_number);
