...
IBFEInstrumentPanel::IBFEInstrumentPanel(SAMRAIPointer<SAMRAIDatabase> input_db, const int part)
                                        ...
                                        const double data_time)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> u_cc_data = patch->getPatchData(U_data_idx);
            Pointer<SAMRAISideData<double>> u_sc_data = patch->getPatchData(U_data_idx);
            if (u_cc_data)
            {
            ...
            Pointer<SAMRAICellData<double>> p_cc_data = patch->getPatchData(P_data_idx);
            if (p_cc_data)
            {
                LEInteractor::interpolate(
    ...
    for (unsigned int meter_idx = 0; meter_idx < d_num_meters; ++meter_idx)
    {
        ...
        {
            Pointer<SAMRAIPatchLevel> level = hierarchy->getPatchLevel(ln);
            Pointer<SAMRAIBoxTree> box_tree = level->getBoxTree();
            ...
            SAMRAIBox cell_box(i, i);
            SAMRAIArray<int> local_patch_idxs;
        ...
        if (x_centroid_global_ln == x_centroid_local_ln)
        {
            ...
            Pointer<SAMRAIPatchLevel> level = hierarchy->getPatchLevel(x_centroid_local_ln);
            if (x_centroid_local_patch_idx != IBTK::invalid_index)
            {
                Pointer<SAMRAIPatch> patch = level->getPatch(x_centroid_local_patch_idx);
                Pointer<SAMRAICellData<double>> p_cc_data = patch->getPatchData(P_data_idx);
                if (p_cc_data)
                {
