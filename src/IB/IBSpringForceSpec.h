...
{
/*!
 * \brief Class IBSpringForceSpec provies a mechanism for specifying
 * the data necessary to compute the forces generated by a network of
 * springs.
 *
 * Springs are connections between two particular nodes of the
 * Lagrangian mesh.  Forces generated by springs have the feature that
 * the force applied to one end of the spring is the negation of the
 * force applied to the other end of the spring.  Consequently, to
 * determine the total nodal forces, we only need to \em compute the
 * force at one end of the spring (e.g., at the master node).  Then,
 * the negation of that force can be applied to the opposite end of
 * the spring (e.g., at the slave node).
 *
 * IBSpringForceSpec objects are stored as Stashable data associated
 * with only the master nodes in the mesh.
 */
...
public:
    ...
    /*!
     * \return A const refrence to the slave node indices for the
     * springs attached to the master node.
     */
    const std::vector<int>& getSlaveNodeIndices() const;

    /*!
     * \return A non-const refrence to the slave node indices for the
     * springs attached to the master node.
     */
    std::vector<int>& getSlaveNodeIndices();

    /*!
     * \return A const reference to the force function index of the
     * springs attached to the master node.
     */
     ...
     * the springs attached to the master node.
     */
    std::vector<int>& getForceFunctionIndices();

    /*!
     * \return A const reference to the stiffnesses of the springs
     * attached to the master node.
     */
    const std::vector<double>& getStiffnesses() const;

    /*!
     * \return A non-const reference to the stiffnesses of the springs
     * attached to the master node.
     */
    std::vector<double>& getStiffnesses();

    /*!
     * \return A const reference to the resting length of the springs
     * attached to the master node.
     */
    const std::vector<double>& getRestingLengths() const;

    /*!
     * \return A non-const reference to the resting length of the
     * springs attached to the master node.
     */
