...
void
BrinkmanPenalizationRigidBodyDynamics::putToDatabase(SAMRAIPointer<SAMRAIDatabase> db)
{
...
void
BrinkmanPenalizationRigidBodyDynamics::getFromInput(SAMRAIPointer<SAMRAIDatabase> input_db, bool is_from_restart)
{
                ...
                if (phi <= alpha && phi >= -alpha)
                {
                    ...
                    for (unsigned int d = 0; d < NDIM; ++d)
                    {
                        ...
                        else
                        {
                            ...
                            Ut(d) = (*u_new_data)(SAMRAISideIndex(c_u, d, SAMRAISideIndex::Lower)) +
                                    (*u_new_data)(SAMRAISideIndex(c_u, d, SAMRAISideIndex::Upper)) +
                                    (*u_new_data)(SAMRAISideIndex(c_l, d, SAMRAISideIndex::Lower)) +
                                    (*u_new_data)(SAMRAISideIndex(c_l, d, SAMRAISideIndex::Upper));
                            Ut(d) *= 0.25;
                        }
                    }
