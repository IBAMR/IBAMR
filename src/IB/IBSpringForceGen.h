...
/*!
 * \brief Function to compute the Jacobian of the force generated by a linear
 * spring with either a zero or a non-zero resting length.
 *
 * \param dF_dX          The force Jacobian associated with the "master" node.
 * \param D              The displacement between the "master" and "slave" nodes, i.e., \f$ \vec{X}_{l} - \vec{X}_{k} \f$.
 * \param stf            The stiffness of the spring.
 * \param rst            The resting length of the spring (possibly zero).
 * \param lag_mastr_idx  The Lagrangian index of the "master" node associated with the spring.
 ...
 */
inline void
default_linear_spring_force_jacobian(
    double dF_dX[NDIM*NDIM],
    const double D[NDIM],
    const double& stf,
    const double& rst,
    const int& lag_mastr_idx,
    const int& lag_slave_idx)
{
    ...
    /*
     * Compute the unit tangent vectors between the "master" and "slave" nodes.
     */
    double tau[NDIM];
    for (int d = 0; d < NDIM; ++d)
    {
    ...
    /*
     * Compute the Jacobian of the force at the "master" node.
     */
    const double& kappa = stf;
    const double& R0 = rst;
    if (R > std::numeric_limits<double>::epsilon())
    {
        for (int beta = 0; beta < NDIM; ++beta)
        {
            for (int alpha = 0; alpha < NDIM; ++alpha)
            {
                ...
                dF_dX[alpha+beta*NDIM] = kappa*(1.0-R0/R)*delta_alpha_beta + kappa*(R0/R)*tau[alpha]*tau[beta];
            }
        }
    }
    else
    {
        for (int beta = 0; beta < NDIM; ++beta)
        {
            ...
            {
                const double delta_alpha_beta = (alpha == beta ? 1.0 : 0.0);
                dF_dX[alpha+beta*NDIM] = kappa*delta_alpha_beta;
            }
        }
    }
    return;
...
public:
    ...
    /*!
     * \brief Register a spring force Jacobian specification function with the
     * force generator.
     *
     * These functions are employed to compute the force Jacobian generated by a
     * particular spring for the specified displacement, spring constant, rest
     * length, and Lagrangian index.
     *
     * \note By default, function default_linear_spring_force_jacobian() is
     * associated with \a force_fcn_idx 0.
     */
    void
...
private:
    ...
    std::map<int,void (*)(double dF_dX[NDIM*NDIM], const double D[NDIM], const double& stf, const double& rst, const int& lag_mastr_idx, const int& lag_slave_idx)> d_force_jacobian_fcn_map;
};
}// namespace IBAMR
