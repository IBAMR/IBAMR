...
{
    // Setup Timers.
    IBAMR_DO_ONCE(t_apply = SAMRAITimerManager::getManager()->getTimer("IBAMR::CIBStaggeredStokesOperator::apply()");
...
CIBStaggeredStokesOperator::apply(Vec x, Vec y)
{
    ...
    Pointer<SAMRAISAMRAIVectorReal<double>> vx0, vy0;
    IBTK::PETScSAMRAIVectorReal::getSAMRAIVectorRead(vx[0], &vx0);
    IBTK::PETScSAMRAIVectorReal::getSAMRAIVector(vy[0], &vy0);
    SAMRAISAMRAIVectorReal<double>& u_p = *vx0;
    Vec L = vx[1];
    Vec U = vx[2];
    SAMRAISAMRAIVectorReal<double>& g_f = *vy0;
    Vec V = vy[1];
    Vec F = vy[2];
    ...
    Pointer<SAMRAISideVariable<double>> U_sc_var = u_p.getComponentVariable(0);
    Pointer<SAMRAICellVariable<double>> P_cc_var = u_p.getComponentVariable(1);
    Pointer<SAMRAISideVariable<double>> A_U_sc_var = g_f.getComponentVariable(0);
    Pointer<SAMRAICellVariable<double>> A_P_cc_var = g_f.getComponentVariable(1);
    ...
    ib_method_ops->spreadForce(A_U_idx, nullptr, std::vector<Pointer<SAMRAIRefineSchedule>>(), half_time);
    ...
    ib_method_ops->interpolateVelocity(
        U_idx, std::vector<Pointer<SAMRAICoarsenSchedule>>(), std::vector<Pointer<SAMRAIRefineSchedule>>(), half_time);
...
void
CIBStaggeredStokesOperator::initializeOperatorState(const SAMRAISAMRAIVectorReal<double>& in,
...
CIBStaggeredStokesOperator::modifyRhsForBcs(Vec y)
{
    ...
    Vec* vy;
    VecNestGetSubVecs(y, nullptr, &vy);
    Pointer<SAMRAISAMRAIVectorReal<double>> vy0;
    IBTK::PETScSAMRAIVectorReal::getSAMRAIVector(vy[0], &vy0);
    SAMRAISAMRAIVectorReal<double>& b = *vy0;
    Vec W = vy[1];
    ...
    if (!d_homogeneous_bc)
    {
        Vec V;
        VecDuplicate(W, &V);
        Pointer<SAMRAISAMRAIVectorReal<double>> x = b.cloneVector("");
...
CIBStaggeredStokesOperator::imposeSolBcs(Vec x)
{
    Vec* vx;
    VecNestGetSubVecs(x, nullptr, &vx);
    Pointer<SAMRAISAMRAIVectorReal<double>> vx0;
