...
{
// Timers.
static SAMRAITimer* t_initialize_hierarchy_independent_data;
static SAMRAITimer* t_initialize_hierarchy_dependent_data;
static SAMRAITimer* t_read_instrument_data;
static SAMRAITimer* t_write_plot_data;
...
linear_interp(const Point& X,
              const SAMRAIIndex& i_cell,
              const Point& X_cell,
              const SAMRAICellData<double>& v,
              const SAMRAIIndex& /*patch_lower*/,
              const SAMRAIIndex& /*patch_upper*/,
    ...
    for (int i_shift2 = (is_lower[2] ? -1 : 0); i_shift2 <= (is_lower[2] ? 0 : 1); ++i_shift2)
    {
        ...
        for (int i_shift1 = (is_lower[1] ? -1 : 0); i_shift1 <= (is_lower[1] ? 0 : 1); ++i_shift1)
        {
            for (int i_shift0 = (is_lower[0] ? -1 : 0); i_shift0 <= (is_lower[0] ? 0 : 1); ++i_shift0)
            {
                ...
                const SAMRAIIndex i(i_shift0 + i_cell(0),
                                    i_shift1 + i_cell(1)
#if (NDIM == 3)
                                        ,
                                    i_shift2 + i_cell(2)
...
IBInstrumentPanel::IBInstrumentPanel(std::string object_name, SAMRAIPointer<SAMRAIDatabase> input_db)
...
// intentionally blank
    ...
    // Setup Timers.
    IBAMR_DO_ONCE(
        t_initialize_hierarchy_independent_data = SAMRAITimerManager::getManager()->getTimer(
            "IBAMR::IBInstrumentPanel::initializeHierarchyIndependentData()");
        t_initialize_hierarchy_dependent_data =
            SAMRAITimerManager::getManager()->getTimer("IBAMR::IBInstrumentPanel::initializeHierarchyDependentData()");
        t_read_instrument_data =
            SAMRAITimerManager::getManager()->getTimer("IBAMR::IBInstrumentPanel::readInstrumentData()");
        t_write_plot_data = SAMRAITimerManager::getManager()->getTimer("IBAMR::IBInstrumentPanel::writePlotData()"););
    return;
} // IBInstrumentPanel
...
void
IBInstrumentPanel::initializeHierarchyIndependentData(const SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
...
void
IBInstrumentPanel::initializeHierarchyDependentData(const SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
                                                    LDataManager* const l_data_manager,
                                                    ...
                                                    const double data_time)
{
    ...
    const SAMRAIIntVector& ratio_to_level_zero = hierarchy->getPatchLevel(finest_ln)->getRatio();
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        const SAMRAIBox domain_box_level = SAMRAIBox::refine(domain_box, ratio);
        const SAMRAIIndex& domain_box_level_lower = domain_box_level.lower();
        const SAMRAIIndex& domain_box_level_upper = domain_box_level.upper();
        ...
        SAMRAIPointer<SAMRAIPatchLevel> finer_level =
            (ln < finest_ln ? hierarchy->getPatchLevel(ln + 1) : SAMRAIPointer<SAMRAIBasePatchLevel>(nullptr));
        const SAMRAIIntVector& finer_ratio = (ln < finest_ln ? finer_level->getRatio() : SAMRAIIntVector(1));
        const SAMRAIBox finer_domain_box_level = SAMRAIBox::refine(domain_box, finer_ratio);
        const SAMRAIIndex& finer_domain_box_level_lower = finer_domain_box_level.lower();
        const SAMRAIIndex& finer_domain_box_level_upper = finer_domain_box_level.upper();
        ...
        for (unsigned int l = 0; l < d_num_meters; ++l)
        {
            ...
            for (unsigned int m = 0; m < d_X_web[l].shape()[0]; ++m)
            {
                for (unsigned int n = 0; n < d_X_web[l].shape()[1]; ++n)
                {
                    ...
                    const SAMRAIIndex i = IndexUtilities::getCellIndex(
                        X, domainXLower, domainXUpper, dx.data(), domain_box_level_lower, domain_box_level_upper);
                    const SAMRAIIndex finer_i = IndexUtilities::getCellIndex(X,
                                                                             domainXLower,
                                                                             domainXUpper,
                                                                             finer_dx.data(),
...
void
IBInstrumentPanel::getFromInput(SAMRAIPointer<SAMRAIDatabase> db)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(db);
#endif
