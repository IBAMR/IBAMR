    ...
    const int finest_reset_ln)
{
    ...
    for (int ln = std::min(d_finest_ln - 1, finest_reset_ln); ln >= std::max(d_coarsest_ln, coarsest_reset_ln - 1);
         --ln)
    {
        Pointer<PatchLevelNd> fine_level = d_hierarchy->getPatchLevel(ln + 1);
        Pointer<PatchLevelNd> coarse_level = d_hierarchy->getPatchLevel(ln);
    ...
    for (int ln = std::min(d_finest_ln, finest_reset_ln); ln >= std::max(d_coarsest_ln, coarsest_reset_ln - 1); --ln)
    {
        if (ln == d_finest_ln)
        {
            ...
            Pointer<PatchLevelNd> finest_level = d_hierarchy->getPatchLevel(d_finest_ln);
