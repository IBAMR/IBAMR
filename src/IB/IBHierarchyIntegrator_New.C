...
// Last modified: <11.Jan.2007 13:39:39 boyce@bigboy.nyconnect.com>
    ...
    SAMRAI::tbox::Pointer<SAMRAI::mesh::GriddingAlgorithm<NDIM> > gridding_alg)
{
    ...
    const SAMRAI::hier::IntVector<NDIM> pres_ghosts = 1;
    ...
    const bool rebalance_coarsest)
{
    ...
    // Synchronize the Cartesian grid velocity and pressure fields on
    // the patch hierarchy.
    assert(false);
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        if (ln == coarsest_ln)
        {
            // Initialize the Cartesian force density to equal zero on
            // the coarsest level in the patch hierarchy.  This will
            // be corrected below.
            for (SAMRAI::hier::PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM> > patch = level->getPatch(p());
        ...
        // Compute the Lagrangian forces and spread them to the
        // Cartesian grid.
            ...
            for (SAMRAI::hier::PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                const SAMRAI::tbox::Pointer<LNodeIndexData> idx_data = patch->
                    getPatchData(d_lag_data_manager->
                                 getLNodeIndexPatchDescriptorIndex());
                LEInteractor::spread(
                    f_new_data, F_data[ln], X_new_data[ln], idx_data,
                    patch, SAMRAI::hier::Box<NDIM>::grow(patch_box,d_ghosts),
                    d_delta_fcn, ENFORCE_PERIODIC_BCS);
            }
        }
        ...
        if (ln > coarsest_ln)
        {
            // Coarsen the Cartesian force from the current level to
            // the next coarser level in the hierarchy.
            assert(false);
        }
    }

    // The pressure at start_time is not an initial value for the
    // incompressible Navier-Stokes equations, so we must solve for it
    // by cycling the solution for the first timestep:
    //
