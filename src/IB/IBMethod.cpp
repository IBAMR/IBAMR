...
IBMethod::IBMethod(std::string object_name, SAMRAIPointer<Database> input_db, bool register_for_restart)
...
void
IBMethod::setupTagBuffer(Array<int>& tag_buffer, SAMRAIPointer<GriddingAlgorithmNd> gridding_alg) const
{
                            ...
                            const double data_time)
{
    ...
    // Build the Jacobian matrix.
    const int finest_ln = d_hierarchy->getFinestLevelNumber();
    SAMRAIPointer<PatchLevelNd> finest_level = d_hierarchy->getPatchLevel(finest_ln);
                            ...
                            const double data_time)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            const SAMRAIPointer<CellDataNd<double> > q_data = patch->getPatchData(q_data_idx);
            for (int n = 0; n < d_n_src[ln]; ++n)
            {
                ...
                const hier::IndexNd i_center = IndexUtilities::getCellIndex(d_X_src[ln][n], grid_geom, ratio);
                BoxNd stencil_box(i_center, i_center);
                for (unsigned int d = 0; d < NDIM; ++d)
                {
                ...
                for (BoxNd::Iterator b(patch_box * stencil_box); b; b++)
                {
    ...
    if (d_normalize_source_strength)
    {
        ...
        {
            SAMRAIPointer<PatchLevelNd> level = d_hierarchy->getPatchLevel(ln);
            BoxListNd level_bdry_boxes(bdry_boxes);
            ...
            for (PatchLevelNd::Iterator p(level); p; p++)
            {
                ...
                const BoxNd& patch_box = patch->getBox();
                const SAMRAIPointer<CellDataNd<double> > q_data = patch->getPatchData(q_data_idx);
                for (BoxListNd::Iterator blist(level_bdry_boxes); blist; blist++)
                {
                    for (BoxNd::Iterator b(blist() * patch_box); b; b++)
                    {
                        (*q_data)(b()) += q_norm;
                    }
                }
    ...
    if (d_normalize_source_strength)
    {
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (PatchLevelNd::Iterator p(level); p; p++)
            {
                SAMRAIPointer<PatchNd> patch = level->getPatch(p());
                const BoxNd& patch_box = patch->getBox();
                const SAMRAIPointer<CellDataNd<double> > p_data = patch->getPatchData(p_data_idx);
                const SAMRAIPointer<CellDataNd<double> > wgt_data = patch->getPatchData(wgt_idx);
                for (BoxListNd::Iterator blist(level_bdry_boxes); blist; blist++)
                {
                    for (BoxNd::Iterator b(blist() * patch_box); b; b++)
                    {
...
void
IBMethod::initializePatchHierarchy(SAMRAIPointer<PatchHierarchyNd> hierarchy,
...
void
IBMethod::registerLoadBalancer(SAMRAIPointer<LoadBalancerNd> load_balancer, int workload_data_idx)
{
...
void
IBMethod::addWorkloadEstimate(SAMRAIPointer<PatchHierarchyNd> hierarchy, const int workload_data_idx)
{
...
void
IBMethod::beginDataRedistribution(SAMRAIPointer<PatchHierarchyNd> /*hierarchy*/,
                                  SAMRAIPointer<GriddingAlgorithmNd> /*gridding_alg*/)
{
...
void
IBMethod::endDataRedistribution(SAMRAIPointer<PatchHierarchyNd> hierarchy,
                                SAMRAIPointer<GriddingAlgorithmNd> /*gridding_alg*/)
{
...
void
IBMethod::initializeLevelData(SAMRAIPointer<BasePatchHierarchyNd> hierarchy,
                              int level_number,
                              double init_data_time,
                              bool can_be_refined,
                              bool initial_time,
...
void
IBMethod::resetHierarchyConfiguration(SAMRAIPointer<BasePatchHierarchyNd> hierarchy,
                                      int coarsest_level,
                                      int finest_level)
{
    const int finest_hier_level = hierarchy->getFinestLevelNumber();
...
void
IBMethod::applyGradientDetector(SAMRAIPointer<BasePatchHierarchyNd> base_hierarchy,
                                int level_number,
                                ...
                                bool uses_richardson_extrapolation_too)
{
    ...
    if (d_ib_source_fcn && !initial_time && hierarchy->finerLevelExists(level_number))
    {
        ...
        SAMRAIPointer<PatchLevelNd> finer_level = hierarchy->getPatchLevel(finer_level_number);
        const IntVectorNd& finer_ratio = finer_level->getRatio();
        for (int n = 0; n < d_n_src[finer_level_number]; ++n)
        {
            ...
            const BoxNd coarsened_stencil_box = BoxNd::coarsen(stencil_box, finer_level->getRatioToCoarserLevel());
            for (PatchLevelNd::Iterator p(level); p; p++)
            {
                SAMRAIPointer<PatchNd> patch = level->getPatch(p());
...
void
IBMethod::putToDatabase(SAMRAIPointer<Database> db)
{
...
void
IBMethod::getFromInput(SAMRAIPointer<Database> db, bool is_from_restart)
{
    if (!is_from_restart)
    {
