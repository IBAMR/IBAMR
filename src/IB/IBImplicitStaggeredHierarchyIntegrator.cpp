...
void
IBImplicitStaggeredHierarchyIntegrator::initializeHierarchyIntegrator(Pointer<PatchHierarchyNd> hierarchy,
                                                                    ...
                                                                    const int cycle_num)
{
    ...
    Pointer<VariableNd> u_var = ins_hier_integrator->getVelocityVariable();
    const int u_scratch_idx = var_db->mapVariableAndContextToIndex(u_var, scratch_ctx);

    Pointer<VariableNd> p_var = ins_hier_integrator->getPressureVariable();
    ...
    Pointer<SAMRAIVectorRealNd<double> > eul_rhs_vec = eul_sol_vec->cloneVector(d_object_name + "::eulerian_rhs_vec");
...
IBImplicitStaggeredHierarchyIntegrator::IBFunction_position(SNES /*snes*/, Vec x, Vec f)
{
    ...
    Pointer<SAMRAIVectorRealNd<double> > u, f_u;
    ...
    Pointer<VariableNd> u_var = d_ins_hier_integrator->getVelocityVariable();
    const int u_current_idx = var_db->mapVariableAndContextToIndex(u_var, current_ctx);
...
IBImplicitStaggeredHierarchyIntegrator::IBJacobianSetup_velocity(SNES /*snes*/, Vec x, Mat A, Mat /*B*/)
{
    ...
    Pointer<SAMRAIVectorRealNd<double> > u;
    IBTK::PETScSAMRAIVectorReal::getSAMRAIVectorRead(x, &u);
    VariableDatabaseNd* var_db = VariableDatabaseNd::getDatabase();
    Pointer<VariableContext> current_ctx = d_ins_hier_integrator->getCurrentContext();
    Pointer<VariableNd> u_var = d_ins_hier_integrator->getVelocityVariable();
...
IBImplicitStaggeredHierarchyIntegrator::IBPCApply_position(Vec x, Vec y)
{
    ...
    Pointer<SAMRAIVectorRealNd<double> > eul_x, eul_y;
...
{
    Pointer<SAMRAIVectorRealNd<double> > f_g, u_p;
