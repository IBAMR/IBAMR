      ...
      d_ib_implicit_ops(ib_implicit_ops)
{
    ...
    {
        SAMRAIPointer<SAMRAIDatabase> stokes_ib_pc_db = input_db->getDatabase("stokes_ib_precond_db");
    ...
    SAMRAIPointer<SAMRAIGriddingAlgorithm> gridding_alg)
{
    ...
    SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
    d_u_dof_index_var = new SAMRAISideVariable<int>(d_object_name + "::u_dof_index");
    d_p_dof_index_var = new SAMRAICellVariable<int>(d_object_name + "::p_dof_index");
...
void
IBImplicitStaggeredHierarchyIntegrator::putToDatabaseSpecialized(SAMRAIPointer<SAMRAIDatabase> db)
{
                                                                    ...
                                                                    const int cycle_num)
{
    ...
    SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
    SAMRAIPointer<SAMRAIVariableContext> scratch_ctx = ins_hier_integrator->getScratchContext();

    const int wgt_cc_idx = d_hier_math_ops->getCellWeightPatchDescriptorIndex();
    const int wgt_sc_idx = d_hier_math_ops->getSideWeightPatchDescriptorIndex();

    SAMRAIPointer<SAMRAIVariable> u_var = ins_hier_integrator->getVelocityVariable();
    const int u_scratch_idx = var_db->mapVariableAndContextToIndex(u_var, scratch_ctx);

    SAMRAIPointer<SAMRAIVariable> p_var = ins_hier_integrator->getPressureVariable();
                                                                    ...
                                                                    const int cycle_num)
{
    ...
    SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
    SAMRAIPointer<SAMRAIVariableContext> current_ctx = ins_hier_integrator->getCurrentContext();
    SAMRAIPointer<SAMRAIVariableContext> scratch_ctx = ins_hier_integrator->getScratchContext();
...
IBImplicitStaggeredHierarchyIntegrator::IBFunction_position(SNES /*snes*/, Vec x, Vec f)
{
    ...
    SAMRAIPointer<SAMRAIVariable> u_var = d_ins_hier_integrator->getVelocityVariable();
    const int u_current_idx = var_db->mapVariableAndContextToIndex(u_var, current_ctx);
