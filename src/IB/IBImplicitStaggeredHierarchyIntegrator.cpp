      ...
      d_ib_implicit_ops(ib_method_ops)
{
    ...
    if (input_db)
    {
        ...
        {
            d_jac_delta_fcn = input_db->getString("jacobian_delta_fcn");
        }
        if (input_db->keyExists("eliminate_eulerian_vars") || input_db->keyExists("eliminate_lagrangian_vars"))
        {
            TBOX_ERROR(d_object_name << "::IBImplicitStaggeredHierarchyIntegrator():\n"
                                     << "  the eliminate_eulerian_vars and eliminate_lagrangian_vars inputs are "
                                        "no longer supported. This class uses the velocity-pressure "
                                        "formulation; the position formulation has been removed. Remove these "
                                        "keys from the input file.\n");
        }
                                                                     ...
                                                                     const int num_cycles)
{
    ...
    default:
        TBOX_ERROR(d_object_name << "::preprocessIntegrateHierarchy():\n"
                                 << "  time_stepping_type = " << enum_to_string<TimeSteppingType>(d_time_stepping_type)
                                 << " is unsupported; use " << enum_to_string<TimeSteppingType>(BACKWARD_EULER) << ", "
                                 << enum_to_string<TimeSteppingType>(TRAPEZOIDAL_RULE) << ", or "
                                                                      ...
                                                                      const int cycle_num)
{
    ...
    const detail::StaggeredStokesIBTimeStepParameters schedule = detail::get_staggered_stokes_ib_time_step_parameters(
    ...
    {
        TBOX_ERROR(d_object_name << "::integrateHierarchySpecialized():\n  the nonlinear Stokes-IB solve failed.\n");
    }
    ...
    detail::advance_staggered_stokes_ib_strategy(*d_ib_implicit_ops,
                                                                        ...
                                                                        const double new_time)
{
    ...
    default:
        TBOX_ERROR(
            d_object_name << "::reinitializeOperatorsAndSolvers():\n  unsupported viscous time stepping type.\n");
    }
    ...
    d_stokes_op->initializeOperatorState(*d_sol_vec, *d_rhs_vec);
...
IBImplicitStaggeredHierarchyIntegrator::deallocateOperatorsAndSolvers()
{
    ...
    if (d_stokes_op && d_stokes_op->getIsInitialized())
    {
