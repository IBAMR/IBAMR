...
#include "SAMRAILoadBalanceStrategy.h"
...
IBFEMethod::registerStressNormalizationPart(unsigned int part)
{
    ...
    IBTK::setup_system_vectors(d_equation_systems[part].get(),
                               ...
                               vector_names,
                               SAMRAIRestartManager::getManager()->isFromRestart());
    return;
...
void
IBFEMethod::setupTagBuffer(SAMRAIArray<int>& tag_buffer, SAMRAIPointer<SAMRAIGriddingAlgorithm> gridding_alg) const
{
...
IBFEMethod::getMaxPointDisplacement() const
{
    ...
    return max_displacement / get_min_patch_dx(dynamic_cast<const SAMRAIPatchLevel&>(
...
IBFEMethod::preprocessIntegrateData(double current_time, double new_time, int num_cycles)
{
    ...
    for (unsigned int part = 0; part < d_meshes.size(); ++part)
    {
        const double patch_dx = get_min_patch_dx(dynamic_cast<const SAMRAIPatchLevel&>(
                                ...
                                const double data_time)
{
    ...
    std::vector<SAMRAIPointer<SAMRAIRefineSchedule>> no_fill(u_ghost_fill_scheds.size());
    for (unsigned int part = 0; part < d_meshes.size(); ++part)
    {
                        ...
                        const double data_time)
{
    ...
    SAMRAIVariableDatabase::getDatabase()->mapIndexToVariable(f_data_idx, f_var);
    auto f_active_data_ops = SAMRAIHierarchyDataOpsManager::getManager()->getOperationsDouble(f_var, hierarchy, true);
    ...
    if (f_phys_bdry_op)
    {
        for (int ln = getCoarsestPatchLevelNumber(); ln <= getFinestPatchLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                SAMRAIPointer<SAMRAIPatchData> f_data = patch->getPatchData(f_scratch_data_idx);
    ...
    // Accumulate forces spread into patch ghost regions.
    {
        if (!d_ghost_data_accumulator)
        {
            ...
            // moment.
            const SAMRAIPointer<SAMRAIPatchLevel> level = hierarchy->getPatchLevel(ln);
    ...
    for (int coarse_ln = 0; coarse_ln < getFinestPatchLevelNumber(); ++coarse_ln)
    {
        ...
        auto f_data_ops = SAMRAIHierarchyDataOpsManager::getManager()->getOperationsDouble(f_var, hierarchy, true);
...
{
    const SAMRAIIntVector ghosts = 1;
    d_lagrangian_workload_var = new SAMRAICellVariable<double>(d_object_name + "::lagrangian_workload");
...
void
IBFEMethod::initializePatchHierarchy(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy,
...
void
IBFEMethod::addWorkloadEstimate(SAMRAIPointer<SAMRAIPatchHierarchy> hierarchy, const int workload_data_idx)
{
    ...
    if (d_do_log)
    {
        ...
        SAMRAIHierarchyCellDataOpsReal<double> hier_cc_data_ops(hierarchy, 0, hierarchy->getFinestLevelNumber());
...
void
IBFEMethod::beginDataRedistribution(SAMRAIPointer<SAMRAIPatchHierarchy> /*hierarchy*/,
                                    SAMRAIPointer<SAMRAIGriddingAlgorithm> /*gridding_alg*/)
{
    ...
    {
        {
            for (int ln = 0; ln <= getFinestPatchLevelNumber(); ++ln)
            {
                SAMRAIPointer<SAMRAIPatchLevel> level =
                    d_secondary_hierarchy->getSecondaryHierarchy()->getPatchLevel(ln);
...
void
IBFEMethod::endDataRedistribution(SAMRAIPointer<SAMRAIPatchHierarchy> /*hierarchy*/,
                                  SAMRAIPointer<SAMRAIGriddingAlgorithm> /*gridding_alg*/)
{
...
void
IBFEMethod::initializeLevelData(SAMRAIPointer<SAMRAIBasePatchHierarchy> /*hierarchy*/,
                                int /*level_number*/,
                                double /*init_data_time*/,
                                bool /*can_be_refined*/,
                                bool /*initial_time*/,
...
void
IBFEMethod::resetHierarchyConfiguration(SAMRAIPointer<SAMRAIBasePatchHierarchy> hierarchy,
                                        int /*coarsest_level*/,
                                        int /*finest_level*/)
{
...
void
IBFEMethod::applyGradientDetector(SAMRAIPointer<SAMRAIBasePatchHierarchy> base_hierarchy,
                                  int level_number,
...
void
IBFEMethod::putToDatabase(SAMRAIPointer<SAMRAIDatabase> db)
{
...
SAMRAIPointer<SAMRAIBasePatchHierarchy>
...
IBFEMethod::doInitializeFEEquationSystems()
{
    ...
    for (unsigned int part = 0; part < d_meshes.size(); ++part)
    {
        ...
        d_ghosts = SAMRAIIntVector::max(d_ghosts, d_active_fe_data_managers[part]->getGhostCellWidth());
                                           ...
                                           const unsigned int part)
{
    ...
    std::vector<std::vector<std::unique_ptr<NumericVector<double>>>> PK1_stress_fcn_ghosted_system_vecs(num_PK1_fcns);
    for (unsigned int k = 0; k < num_PK1_fcns; ++k)
    {
...
IBFEMethod::getProlongationSchedule(const int level_number, const int coarse_data_idx, const int fine_data_idx)
{
    ...
    if (d_prolongation_schedules.count(key) == 0)
    {
        ...
        SAMRAIPointer<SAMRAIVariable> f_var;
        SAMRAIVariableDatabase::getDatabase()->mapIndexToVariable(coarse_data_idx, f_var);
        {
            SAMRAIPointer<SAMRAIVariable> f_var_2;
            SAMRAIVariableDatabase::getDatabase()->mapIndexToVariable(fine_data_idx, f_var_2);
        ...
        if (cc_data)
        {
            SAMRAIPointer<SAMRAICartesianGridGeometry> geometry = hierarchy->getGridGeometry();
        ...
        SAMRAIPointer<SAMRAIPatchLevel> fine_level = hierarchy->getPatchLevel(level_number + 1);
        ...
        SAMRAIPointer<SAMRAIRefineSchedule> schedule =
...
void
IBFEMethod::commonConstructor(const SAMRAIPointer<SAMRAIDatabase>& input_db, int max_levels)
{
...
void
IBFEMethod::getFromInput(const SAMRAIPointer<SAMRAIDatabase>& db, bool /*is_from_restart*/)
{
    ...
    {
        d_fe_data_manager_db = new SAMRAIInputDatabase("FEDataManager");
    }
