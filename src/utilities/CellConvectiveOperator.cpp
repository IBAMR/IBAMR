...
CellConvectiveOperator::CellConvectiveOperator(std::string object_name,
                                               Pointer<SAMRAICellVariable<double>> Q_cell_var,
      ...
      d_Q_cell_var(Q_cell_var)
{
    ...
    if (d_Q_cell_var)
    {
        Pointer<SAMRAICellDataFactory<double>> Q_pdat_fac = d_Q_cell_var->getPatchDataFactory();
        ...
        {
            d_q_flux_var = new SAMRAIFaceVariable<double>(q_flux_var_name, Q_depth);
            d_q_flux_idx = var_db->registerVariableAndContext(d_q_flux_var, context, SAMRAIIntVector(0));
        }
        ...
        {
            d_q_interp_var = new SAMRAIFaceVariable<double>(q_interp_var_name, Q_depth);
            d_q_interp_idx = var_db->registerVariableAndContext(d_q_interp_var, context, SAMRAIIntVector(0));
        }
    ...
    IBAMR_DO_ONCE(
        t_apply_convective_operator =
            SAMRAITimerManager::getManager()->getTimer("IBAMR::CellConvectiveOperator::applyConvectiveOperator()");
        t_initialize_operator_state =
            SAMRAITimerManager::getManager()->getTimer("IBAMR::CellConvectiveOperator::initializeOperatorState()");
        t_deallocate_operator_state =
            SAMRAITimerManager::getManager()->getTimer("IBAMR::CellConvectiveOperator::deallocateOperatorState()"););
    return;
...
CellConvectiveOperator::interpolateToFaceOnHierarchy(int q_interp_idx, int Q_cell_idx, int u_idx, bool synch_cf_bdry)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> Q_ghost_data = patch->getPatchData(d_Q_ghost_idx);
            Pointer<SAMRAIFaceData<double>> q_interp_data = patch->getPatchData(q_interp_idx);
            Pointer<SAMRAIFaceData<double>> u_data = patch->getPatchData(u_idx);
...
CellConvectiveOperator::evaluateAdvectiveFluxOnHierarchy(int q_flux_idx, int Q_cell_idx, int u_idx, bool synch_cf_bdry)
{
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            auto patch = level->getPatch(p());
            Pointer<SAMRAICellData<double>> Q_ghost_data = patch->getPatchData(d_Q_ghost_idx);
            Pointer<SAMRAIFaceData<double>> q_flux_data = patch->getPatchData(q_flux_idx);
            Pointer<SAMRAIFaceData<double>> u_data = patch->getPatchData(u_idx);
                                                              ...
                                                              bool synch_cf_bdry)
{
    ...
    for (int ln = finest_ln; ln >= coarsest_ln; --ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> N_cell_data = patch->getPatchData(N_cell_idx);
                                                                  ...
                                                                  bool synch_cf_bdry)
{
    ...
    // Setup communications algorithms.
    SAMRAICoarsenAlgorithm flux_coarsen_alg, interp_coarsen_alg;
...
void
CellConvectiveOperator::evaluateAdvectiveFluxOnPatch(SAMRAIFaceData<double>& q_flux_data,
                                                     ...
                                                     const SAMRAIPatch& patch)
{
    ...
    const auto& u_data_gcw = u_data.getGhostCellWidth();
    const SAMRAIIntVector& q_flux_data_gcw = q_flux_data.getGhostCellWidth();
    SAMRAIFaceData<double> q_interp_data(patch_box, q_flux_data.getDepth(), q_flux_data_gcw);
...
void
CellConvectiveOperator::initializeOperatorState(const SAMRAISAMRAIVectorReal<double>& in,
                                                const SAMRAISAMRAIVectorReal<double>& out)
{
    ...
    d_Q_cell_refine_alg = new SAMRAIRefineAlgorithm();
    ...
    d_q_flux_coarsen_alg = new SAMRAICoarsenAlgorithm();
    ...
    d_q_interp_coarsen_alg = new SAMRAICoarsenAlgorithm();
    ...
    {
        Pointer<SAMRAIPatchLevel> level = d_hierarchy->getPatchLevel(ln);
        Pointer<SAMRAIPatchLevel> coarser_level = (ln > coarsest_ln) ? d_hierarchy->getPatchLevel(ln - 1) : nullptr;
