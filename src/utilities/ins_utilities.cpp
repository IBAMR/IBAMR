...
computeNetInflowPhysicalBoundary(Pointer<HierarchyMathOps> hier_math_ops, int u_idx, int bdry_loc_idx)
{
    ...
    for (int ln = 0; ln <= hier_finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
        {
            ...
            Pointer<SAMRAISideData<double>> wgt_data = patch->getPatchData(wgt_sc_idx);
            ...
            for (int n = 0; n < physical_codim1_boxes.size(); ++n)
            {
                ...
                SAMRAIBoundaryBox trimmed_box = PhysicalBoundaryUtilities::trimBoundaryCodim1Box(bdry_box, *patch);
                SAMRAIBox integration_box = PhysicalBoundaryUtilities::makeSideBoundaryCodim1Box(trimmed_box);

                // Ensure the integration box is of width one in the normal direction
                integration_box.upper(axis) = integration_box.lower(axis);

                for (SAMRAIBox::Iterator b(integration_box); b; b++)
                {
