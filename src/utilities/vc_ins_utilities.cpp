...
SetFluidProperties::SetFluidProperties(const std::string& object_name,
                                       Pointer<AdvDiffHierarchyIntegrator> adv_diff_solver,
                                       Pointer<SAMRAICellVariable<double>> ls_gas_var,
                                                   ...
                                                   const double new_time)
{
    ...
    Pointer<SAMRAICellVariable<double>> rho_cc_var = rho_var;
    if (rho_cc_var)
    {
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                const Pointer<SAMRAICellData<double>> ls_data = patch->getPatchData(ls_idx);
                Pointer<SAMRAICellData<double>> rho_data = patch->getPatchData(rho_idx);
    ...
    Pointer<SAMRAISideVariable<double>> rho_sc_var = rho_var;
    if (rho_sc_var)
    {
        // Note, this method requires ghost cells to be filled for the level set variable
        SAMRAIRobinBcCoefStrategy* ls_bc_coef = d_adv_diff_solver->getPhysicalBcCoefs(d_ls_gas_var).front();
        SAMRAIIntVector cell_ghosts = 1;
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                const SAMRAIBox& patch_box = patch->getBox();
                const Pointer<SAMRAICellData<double>> ls_data = patch->getPatchData(ls_scratch_idx);
                Pointer<SAMRAISideData<double>> rho_data = patch->getPatchData(rho_idx);
                                                   ...
                                                   const double new_time)
{
    ...
    if (rho_cc_var)
    {
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                const SAMRAIBox& patch_box = patch->getBox();
                const Pointer<SAMRAICellData<double>> ls_solid_data = patch->getPatchData(ls_solid_idx);
                const Pointer<SAMRAICellData<double>> ls_gas_data = patch->getPatchData(ls_gas_idx);
                Pointer<SAMRAICellData<double>> rho_data = patch->getPatchData(rho_idx);
    ...
    {
        // Note, this method requires ghost cells to be filled for the level set variable
        SAMRAIRobinBcCoefStrategy* ls_solid_bc_coef = d_adv_diff_solver->getPhysicalBcCoefs(d_ls_solid_var).front();
        SAMRAIRobinBcCoefStrategy* ls_gas_bc_coef = d_adv_diff_solver->getPhysicalBcCoefs(d_ls_gas_var).front();
        SAMRAIIntVector cell_ghosts = 1;
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                const SAMRAIBox& patch_box = patch->getBox();
                const Pointer<SAMRAICellData<double>> ls_solid_data = patch->getPatchData(ls_solid_scratch_idx);
                const Pointer<SAMRAICellData<double>> ls_gas_data = patch->getPatchData(ls_gas_scratch_idx);
                Pointer<SAMRAISideData<double>> rho_data = patch->getPatchData(rho_idx);
...
#define HARMONIC_CC_TO_SC_RHO_3_PHASE 1
                                                              ...
                                                              const double new_time)
    {
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAICellData<double>> mu_data = patch->getPatchData(mu_idx);
                                                              ...
                                                              const double new_time)
    {
        ...
        Pointer<SAMRAICellVariable<double>> mu_cc_var = mu_var;
        if (mu_cc_var)
        {
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                                        ...
                                        Pointer<SAMRAIPatchLevel> /*patch_level*/)
    {
        ...
        if (d_grav_type == "FULL")
        {
            ...
            const Pointer<SAMRAISideData<double>> rho_data = patch->getPatchData(rho_ins_idx);
            for (int axis = 0; axis < NDIM; ++axis)
            {
                for (SAMRAIBox::Iterator it(SAMRAISideGeometry::toSideBox(patch_box, axis)); it; it++)
                {
        ...
        else if (d_grav_type == "FLOW")
        {
            ...
            Pointer<SAMRAICartesianPatchGeometry> patch_geom = patch->getPatchGeometry();
            const double* const patch_dx = patch_geom->getDx();
            const Pointer<SAMRAICellData<double>> ls_gas_data = patch->getPatchData(d_ls_gas_scratch_idx);
