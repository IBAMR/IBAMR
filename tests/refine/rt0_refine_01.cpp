                        ...
                        const double* dx)
{
    ...
    for (int d = 0; d < NDIM; ++d)
    {
        ...
        transverse_sum += xd;
        transverse_prod *= (xd + 0.1 * static_cast<double>(d + 1));
    }
    return std::sin(2.0 * pi * x_axis) + 0.35 * std::cos(pi * transverse_sum) + 0.1 * x_axis * x_axis +
                          ...
                          const double tol)
{
    ...
    const int test_failures = max_interior_error > tol ? 1 : 0;
