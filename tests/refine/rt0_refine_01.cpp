...
test_rt0_schedule(Pointer<PatchHierarchy<NDIM>> hierarchy, int u_idx, int refined_idx)
{
    ...
    if (!(affine_error < 1.0e-12 && divergence_error < 1.0e-12 && divergence_norm > 0.0))
    {
        TBOX_ERROR("RT0 schedule invariants failed: affine error = "
                   << affine_error << ", divergence error = " << divergence_error
                   << ", coarse divergence norm = " << divergence_norm << ".\n");
    }
    return 0;
}
