    ...
    TimerManager::createManager(nullptr);
    {
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                SAMRAIPointer<SAMRAISideData<double>> u_data = patch->getPatchData(u_sc_idx);
        ...
        // The rest is just book-keeping, this is the actual test:
        {
            ...
            SAMRAIPointer<SAMRAICoarsenOperator> coarsen_op = new IBTK::CartSideDoubleRT0Coarsen(ratio);
            coarsen_alg->registerCoarsen(u_sc_idx, u_sc_idx, coarsen_op);
            SAMRAIPointer<SAMRAICoarsenSchedule> coarsen_sched =
        ...
        SAMRAIPointer<SAMRAIPatchLevel> level = patch_hierarchy->getPatchLevel(0);
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            pout << "patch number " << p() << '\n';
            SAMRAIPointer<SAMRAIPatch> patch = level->getPatch(p());
            SAMRAIPointer<SAMRAISideData<double>> e_data = patch->getPatchData(exact_sc_idx);
