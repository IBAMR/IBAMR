...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<CellVariableNd<double> > c_var = new CellVariableNd<double>("c_v");
        Pointer<SideVariableNd<double> > s_var = new SideVariableNd<double>("s_v");
        const int gcw = 1;
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (PatchLevelNd::Iterator p(level); p; p++)
            {
                ...
                {
                    Pointer<CellDataNd<double> > data = patch->getPatchData(c_idx);
                ...
                {
                    Pointer<SideDataNd<double> > data = patch->getPatchData(s_idx);
