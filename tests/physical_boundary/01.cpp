...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<SAMRAICellVariable<double>> c_var = new SAMRAICellVariable<double>("c_v");
        Pointer<SAMRAISideVariable<double>> s_var = new SAMRAISideVariable<double>("s_v");
        const int gcw = 1;
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                {
                    Pointer<SAMRAICellData<double>> data = patch->getPatchData(c_idx);
                ...
                {
                    Pointer<SAMRAISideData<double>> data = patch->getPatchData(s_idx);
