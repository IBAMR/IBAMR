...
void velocity_convergence(SAMRAIPointer<PatchHierarchyNd> patch_hierarchy,
                          const int u_idx,
...
void pressure_convergence(SAMRAIPointer<PatchHierarchyNd> patch_hierarchy,
                          const int p_idx,
                          const double data_time,
                          const string& data_dump_dirname);
...
void
velocity_convergence(SAMRAIPointer<PatchHierarchyNd> patch_hierarchy,
                     const int u_idx,
                     const double /*data_time*/,
                     const string& /*data_dump_dirname*/)
{
...
void
pressure_convergence(SAMRAIPointer<PatchHierarchyNd> patch_hierarchy,
                     const int p_idx,
                     const double /*data_time*/,
                     const string& /*data_dump_dirname*/)
{
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            for (BoxListNd::Iterator lit(iterate_box_list); lit; lit++)
            {
                ...
                for (BoxNd::Iterator bit(iterate_box); bit; bit++)
                {
                    ...
                    const double u0 = (*u_data)(SideIndexNd(lower_idx, 0, SideIndexNd::Lower));
                    const double u1 = (*u_data)(SideIndexNd(upper_idx, 0, SideIndexNd::Lower));
