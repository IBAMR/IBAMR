...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<IBAMR::IIMethod> ib_method_ops = make_samrai_shared<IIMethod>(
            "IIMethod",
        ...
        vector<RobinBcCoefStrategyNd*> u_bc_coefs(NDIM, static_cast<RobinBcCoefStrategyNd*>(nullptr));
        ...
        std::unique_ptr<ExodusII_IO> lower_exodus_io(uses_exodus ? new ExodusII_IO(lower_mesh) : nullptr);
        std::unique_ptr<ExodusII_IO> upper_exodus_io(uses_exodus ? new ExodusII_IO(upper_mesh) : nullptr);
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            for (BoxListNd::Iterator lit(iterate_box_list); lit; lit++)
            {
                ...
                for (BoxNd::Iterator bit(iterate_box); bit; bit++)
                {
                    ...
                    // CellIndexNd upper_idx = lower_idx;
                    // upper_idx(0) += 1;
                    ...
                    //~ const double u1 =
                    //~ (*u_data)(SideIndexNd(upper_idx, 0, SideIndexNd::Lower));
                         ...
                         const string& /*data_dump_dirname*/)
{
    ...
    for (int ln = finest_ln; ln >= coarsest_ln; --ln)
    {
        ...
        for (PatchLevelNd::Iterator p(level); p; p++)
        {
            ...
            // Loop over the boxes and store the location and interpolated value.
            //~ SAMRAIPointer<Data<NDIM, double> > p_data = patch->getPatchData(p_idx);
