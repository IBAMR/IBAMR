...
public:
     ...
     */
    void
    setLevelSetPatchData(int D_idx, SAMRAIPointer<IBTK::HierarchyMathOps> hier_math_ops, double time, bool initial_time)
    {
...
private:
    ...
    void getExtremeCoords(std::vector<IBTK::Vector>& corners, SAMRAIPointer<IBTK::HierarchyMathOps> hier_math_ops)
    {
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            const SAMRAIPointer<IBTK::LMesh> mesh = d_lag_data_manager->getLMesh(ln);
