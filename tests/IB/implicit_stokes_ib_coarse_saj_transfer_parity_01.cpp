...
/*
 * Check that the RT0 transfer matrices and the RT0 SAMRAI transfer schedules agree when they are
 * applied to the same fine matrix.
 *
 * Let B be the velocity block of J^T A J, where A is the Lagrangian force Jacobian and J is the
 * interpolation matrix. Let P be the RT0 prolongation matrix and D the diagonal matrix of restriction
 * scaling. Every column of D P^T B P is compared with the result of prolonging the corresponding coarse
 * basis vector with the RT0 refinement schedule, applying B, and restricting with the RT0 coarsening
 * schedule. The sign, time-step, and cell-volume factors of the FAC operator, and its construction of
 * the coarse operator, are outside this test.
 */
...
// Return the largest difference between a column of coarse_matrix and the result of prolonging the
// corresponding coarse basis vector with the refinement schedule, applying fine_op, and restricting
// with the coarsening schedule.
double
max_coarse_column_error(Mat coarse_matrix,
                        Mat fine_op,
                        ...
                        IntVector<NDIM> gcw)
{
    ...
    Vec x = nullptr, y = nullptr, z = nullptr, X = nullptr, Y = nullptr;
    ...
    for (int col = 0; col < n_total_coarse; ++col)
    {
        ...
        IBTK_CHKERRQ(ierr);
        ierr = MatMult(coarse_matrix, x, z);
        IBTK_CHKERRQ(ierr);
        ...
        ierr = VecAXPY(y, -1.0, z);
        IBTK_CHKERRQ(ierr);
        double column_error = 0.0;
        ierr = VecNorm(y, NORM_INFINITY, &column_error);
        IBTK_CHKERRQ(ierr);
        max_error = accumulate_error(max_error, column_error);
    }
...
main(int argc, char* argv[])
{
    ...
    integrator_db->putDouble("start_time", current_time);
    integrator_db->putDouble("rho", 1.0);
    integrator_db->putDouble("mu", 1.0);
    ...
    Pointer<IBExplicitHierarchyIntegrator> ib_integrator = new IBExplicitHierarchyIntegrator(
        "IBExplicitHierarchyIntegrator", integrator_db, ib_method_ops, ins_integrator, false);
    ...
    const IBTK::IBOperatorBuilder interpolation_builder{ IBTK::IBKernelTensorProduct("IB_4") };
    const int u_full_dof_index_idx = var_db->registerVariableAndContext(
        u_full_dof_var, scratch_ctx, IntVector<NDIM>(interpolation_builder.getMinimumGhostWidth()));
    ...
    {
        TBOX_ERROR("The fine-level spreading operator is trivial: max entry = " << saj_inf_norm << ", tolerance = "
                                                                                << saj_nontrivial_tol << ".\n");
    }
    ...
    const double inf_norm = max_coarse_column_error(SAJ_coarse_matrix,
                                                    SAJ_u_fine,
                                                    num_u_dofs_coarse,
                                                    num_u_dofs_fine,
                                                    u_data_var,
    ...
    {
        TBOX_ERROR("Coarse matrix and schedule disagree: max column error = " << inf_norm << ", tolerance = "
                                                                              << parity_tol << ".\n");
    }
    plog << std::setprecision(12) << "fine coupling max entry = " << saj_inf_norm << '\n'
         << "coarse matrix/schedule max column error = " << inf_norm << '\n';
