...
run_level_state(Pointer<AppInitializer> app)
{
    ...
    pout << "shell_solve_error = " << max_level_solve_error << '\n'
         ...
         << "domain_nullspace_valid = " << (domain_valid ? "true" : "false") << std::endl;
    if (!cc_valid || !sc_valid || !stokes_valid || !domain_valid)
    {
        TBOX_ERROR("A level-state check failed; see the printed validity flags.\n");
    }
    return 0;
}
} // namespace
