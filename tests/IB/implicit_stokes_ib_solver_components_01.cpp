...
#include "../navier_stokes/level_solver_test_utilities.h"
#include "../tests.h"
...
set_operator_velocity(int idx, Pointer<PatchLevel<NDIM>> level, double amplitude, double phase)
{
    for (PatchLevel<NDIM>::Iterator p(level); p; p++)
    {
        ...
        for (int axis = 0; axis < NDIM; ++axis)
        {
            ...
            {
                // Reduces to 1 - axis for NDIM == 2; generalizes to a well-defined, in-bounds transverse
                // axis for NDIM == 3, where this function is not currently exercised (see the caller).
                const int other = (axis + 1) % NDIM;
...
run_operators(Pointer<AppInitializer> app)
{
    ...
    if (!use_fixed_le_operators)
    {
        // Enabling fixed operators after preprocessIntegrateData() leaves their data uninitialized, so
        // updateFixedLEOperators() must reject it.
    ...
    if (app->getInputDatabase()->getBoolWithDefault("supplied_action_only", false))
    {
        ...
        stokes->initializeOperatorState(*direction, *action);
    ...
    if (physical_boundary)
    {
        ...
        for (int cycle = 0; cycle < 2; ++cycle)
        {
            ...
            method->updateFixedLEOperators();
            stokes->initializeOperatorState(*base, *residual);
    ...
    if (context_check == "apply_before_form" || context_check == "cleared_matrix")
    {
        jacobian.setTimeInterval(current, next);
        jacobian.setSolutionTime(force_time);
        stokes->initializeOperatorState(*base, *residual);
        jacobian.initializeOperatorState(*base, *residual);
        if (context_check == "cleared_matrix")
        {
            // Form on the strategy path first, so the invalidation below is exercised, not a linearization that
            // was never formed in the first place.
            jacobian.formJacobian(*base);
            // A supplied matrix records the base state without forming the strategy linearization, which must
            // invalidate the one just formed above: the strategy action must still require a new formJacobian()
            // after the matrix is cleared.
            Mat supplied = nullptr;
            ierr = MatCreateSeqAIJ(PETSC_COMM_SELF, 1, 1, 1, nullptr, &supplied);
            IBTK_CHKERRQ(ierr);
            jacobian.setIBCouplingJacobian(supplied);
            ierr = MatDestroy(&supplied);
            IBTK_CHKERRQ(ierr);
            jacobian.formJacobian(*base);
            jacobian.setIBCouplingJacobian(nullptr);
        }
        jacobian.apply(*base, *residual);
        return 0;
    }
    ...
    if (context_check == "frozen")
    {
        // The operators use the coupling configuration that the strategy holds and never change it: moving the
        // positions without updateFixedLEOperators() leaves the configuration alone through initialization,
        // the residual, and the Jacobian, and updateFixedLEOperators() then moves it.
        Vec frozen = method->getFinestLevelLECouplingPositions(force_time);
        Vec before = nullptr;
        ierr = VecDuplicate(frozen, &before);
        IBTK_CHKERRQ(ierr);
        ierr = VecCopy(frozen, before);
        IBTK_CHKERRQ(ierr);
        ...
        jacobian.initializeOperatorState(*base, *residual);
        base->setToScalar(0.0);
        set_operator_velocity(u, level, 0.12, 0.1);
        nonlinear.apply(*base, *residual);
        jacobian.formJacobian(*base);
        jacobian.apply(*direction, *residual);
        const auto configuration_change = [&]()
        {
            Vec current_configuration = method->getFinestLevelLECouplingPositions(force_time);
            Vec difference_vec = nullptr;
            PetscErrorCode error = VecDuplicate(before, &difference_vec);
            IBTK_CHKERRQ(error);
            error = VecWAXPY(difference_vec, -1.0, current_configuration, before);
            IBTK_CHKERRQ(error);
            PetscReal change = 0.0;
            error = VecNorm(difference_vec, NORM_INFINITY, &change);
            IBTK_CHKERRQ(error);
            error = VecDestroy(&difference_vec);
            IBTK_CHKERRQ(error);
            return static_cast<double>(change);
        };
        const double operator_change = configuration_change();
        method->updateFixedLEOperators();
        const double update_change = configuration_change();
        ierr = VecDestroy(&before);
        IBTK_CHKERRQ(ierr);
        plog << "configuration change by the operators = " << operator_change << '\n'
             << "configuration change by updateFixedLEOperators = "
             << (update_change > 5.0e-3 ? "moved" : "did not move") << '\n';
        if (operator_change != 0.0 || update_change <= 5.0e-3)
        {
            TBOX_ERROR("The operators changed the coupling configuration, or updateFixedLEOperators() did not.\n");
        }
        return 0;
    }
    ...
    IBTK_CHKERRQ(ierr);
    // The owner freezes the coupling at the current configuration, which the residual evaluations above have moved.
    method->updateFixedLEOperators();
    stokes->initializeOperatorState(*base, *residual);
    jacobian.initializeOperatorState(*base, *residual);
    ...
    // Check operator accuracy against tolerances, not equality of cancellation-sensitive errors. Every check
    // aborts on failure, so the output records how many were made.
    int checks_passed = 0;
    for (const auto& check : { std::make_pair("nonlinear_residual", residual_valid),
                               std::make_pair("assembled_jacobian", assembled_valid),
                               std::make_pair("centered_fd", derivative_valid),
                               std::make_pair("nonlinear_apply_add", nonlinear_add_valid),
                               std::make_pair("jacobian_apply_add", jacobian_add_valid),
                               std::make_pair("initial_supplied_action", initial_supplied_valid),
                               std::make_pair("retained_supplied_action", retained_valid),
                               std::make_pair("supplied_form_jacobian", supplied_form_valid),
                               std::make_pair("reinstalled_supplied_action", supplied_valid),
                               std::make_pair("time_state_scaling", time_valid),
                               std::make_pair("nontrivial_coupling", nontrivial),
                               std::make_pair("updated_base_state", base_valid),
                               std::make_pair("boundary_forwarding", boundary_valid),
                               std::make_pair("operator_lifecycle", lifecycle_valid) })
    {
        if (!check.second)
        {
    ...
    pout << "operator checks passed = " << checks_passed << '\n';
    method->postprocessIntegrateData(current, next, 1);
...
double max_matrix_error = 0.0, max_mapping_error = 0.0;
...
run_level_operator(Pointer<AppInitializer> app, bool augmentation)
{
    ...
    pout << "matrix_identity_valid = " << (identity ? "true" : "false") << '\n'
         ...
         << "velocity_pressure_mapping_error = " << max_mapping_error << std::endl;
    if (!identity || !creator_valid || !values_valid || !solves || !references_valid)
    {
        TBOX_ERROR("A level-operator matrix or solve check failed; see the printed validity flags.\n");
    }
    return 0;
}
...
int
run_level_nullspace(Pointer<AppInitializer> app)
{
    ...
    for (const bool caller_nullspace : { false, true })
    {
        // A solver that uses a supplied matrix directly attaches its nullspace to that matrix only while
        // initialized, and then restores the matrix's own nullspace, if it had one.
        LevelFixture fixture(app->getComponentDatabase("CartesianGeometry"));
        LevelSolverProbe<StaggeredStokesPETScLevelSolver> solver("supplied_nullspace", level_solver_database());
        Mat creator = level_test_matrix(fixture.full_size, 4.0);
        MatNullSpace own = nullptr;
        PetscErrorCode ierr = 0;
        if (caller_nullspace)
        {
            ierr = MatNullSpaceCreate(PETSC_COMM_WORLD, PETSC_TRUE, 0, nullptr, &own);
            IBTK_CHKERRQ(ierr);
            ierr = MatSetNullSpace(creator, own);
            IBTK_CHKERRQ(ierr);
        }
        solver.setOperatorMat(creator);
        solver.setComponentsHaveNullSpace(false, true);
        ...
        {
            solver.initializeSolverState(*fixture.x, *fixture.b);
            ierr = MatGetNullSpace(creator, &nullspace);
            IBTK_CHKERRQ(ierr);
            domain_valid = domain_valid && nullspace != nullptr && nullspace != own;
            solver.deallocateSolverState();
            ierr = MatGetNullSpace(creator, &nullspace);
            IBTK_CHKERRQ(ierr);
            domain_valid = domain_valid && nullspace == own;
        }
        solver.setOperatorMat(nullptr);
        ierr = MatNullSpaceDestroy(&own);
        IBTK_CHKERRQ(ierr);
        ierr = MatDestroy(&creator);
        IBTK_CHKERRQ(ierr);
    }
    pout << "domain_nullspace_valid = " << (domain_valid ? "true" : "false") << std::endl;
    if (!domain_valid)
    {
        TBOX_ERROR("A level nullspace check failed; see the printed validity flag.\n");
    }
    return 0;
}
} // namespace
...
{
    {
        ...
        {
            TBOX_ERROR("nonlinear operator produced non-finite norm\n");
        }
        else if (nonlinear_side_norm <= 1.0e-14 && nonlinear_cell_norm <= 1.0e-14)
        {
            if (!expect_trivial_nonlinear)
            {
                TBOX_ERROR("nonlinear operator action is trivial\n");
            }
        }
        else if (expect_trivial_nonlinear)
        {
            TBOX_ERROR("nonlinear operator action is nontrivial when SPRING_STIFFNESS, RHO, and MU are zero\n");
        }
        ...
        if (!jv_finite)
        {
            TBOX_ERROR("jacobian norms are non-finite\n");
        }
        else if (jv_side_norm <= 1.0e-14 && jv_cell_norm <= 1.0e-14)
        {
            TBOX_ERROR("jacobian action is trivial\n");
        }
        nonlinear_op.apply(*eul_sol_vec, *f_probe);
        ...
        if (application_only)
        {
            ...
            for (int lifetime = 0; lifetime < 2; ++lifetime)
            {
                ...
                int total_dofs = 0, total_patches = 0, max_patch_dofs = 0;
                ...
                for (int ln = 0; ln <= finest_ln; ++ln)
                {
                    ...
                    // The installed patches must be those of the public constructor, applied to this level's matrix.
                    std::vector<std::set<int>> expected_patches;
                    ...
                    StaggeredStokesPETScMatUtilities::construct_patch_level_pressure_cell_seeded_cav_patches(
                        ...
                        elasticity);
                    std::vector<std::set<int>> installed_patches;
                    for (const IS is : *overlap)
                    {
                        PetscInt count = 0;
                        const PetscInt* dofs = nullptr;
                        ierr = ISGetLocalSize(is, &count);
                        IBTK_CHKERRQ(ierr);
                        ierr = ISGetIndices(is, &dofs);
                        IBTK_CHKERRQ(ierr);
                        installed_patches.emplace_back(dofs, dofs + count);
                        ierr = ISRestoreIndices(is, &dofs);
                        IBTK_CHKERRQ(ierr);
                    }
                    for (const std::set<int>& patch : installed_patches)
                    {
                ...
                // A standard Vanka patch has one pressure DOF and the 2 * NDIM velocity DOFs on the faces of its cell.
                // Coupling through the structure must enlarge at least one patch.
                if (max_patch_dofs <= 2 * NDIM + 1)
                {
                    TBOX_ERROR("No CAV patch exceeds the " << 2 * NDIM + 1 << " DOFs of a standard Vanka patch.\n");
                }
                plog << "lifetime " << lifetime + 1 << " levels " << finest_ln + 1 << " dofs " << total_dofs
                     << " patches " << total_patches << " max_patch_dofs " << max_patch_dofs << " correction_L2 "
                     << correction_norm << " elasticity_L2 " << elasticity_norm << " reconstruction_error "
                     << reconstruction_error << " elasticity_error " << elasticity_error << '\n';
        ...
        auto check_fac_residual_work_vector_cache = [&](double& reuse_error)
        {
            ...
            {
                TBOX_ERROR("FAC residual Vec allocation check failed: creations = "
                           << petsc_vec_creation_count << ", destructions = " << petsc_vec_destruction_count << '\n');
            }
#endif
            ...
            {
                TBOX_ERROR("FAC comparison failed: operator_error = "
                           << std::setprecision(17) << operator_error << ", composition_error = " << composition_error
                           << ", elastic_action_norm = " << elastic_action_norm << '\n');
            }
        ...
        {
            TBOX_ERROR("Failed check: !fac_residual_repeat_valid.\n");
        }
        ...
        if (verify_galerkin_operator_borrowing)
        {
            ...
            {
                TBOX_ERROR(
                    "Failed check: !galerkin_operator_available_valid || !galerkin_operator_creator_lifetime_valid.\n");
            }
        }
        ...
        {
            TBOX_ERROR("Initialized Jacobian action failed: error = "
                       << std::setprecision(17) << initialized_action_error << ", bound = " << initialized_action_bound
                       << '\n');
        }
        ...
        if (!linear_success)
        {
            TBOX_ERROR("krylov linear solve failed\n");
        }
        if (!verify_physical_residuals)
        {
            pout << "krylov_linear_iterations = " << linear_solver->getNumIterations() << std::endl;
        }
        if (input_db->keyExists("MAX_KRYLOV_ITERATIONS") &&
            linear_solver->getNumIterations() > input_db->getInteger("MAX_KRYLOV_ITERATIONS"))
        {
            TBOX_ERROR("The Krylov solve took " << linear_solver->getNumIterations() << " iterations, more than "
                                                << input_db->getInteger("MAX_KRYLOV_ITERATIONS") << ".\n");
        }
        ...
        {
            TBOX_ERROR("Krylov residual check failed: actual_residual = "
                       << std::setprecision(17) << actual_residual << ", reported_residual = "
                       << linear_solver->getResidualNorm() << ", bound = " << residual_bound << '\n');
        }
        ...
        {
            TBOX_ERROR("krylov linear solve produced non-finite norm\n");
        }
        else if (linear_side_norm <= 1.0e-14 && linear_cell_norm <= 1.0e-14)
        {
            TBOX_ERROR("krylov linear solve action is trivial\n");
        }
        ...
        if (verify_physical_residuals)
        {
            ...
            for (const Pointer<SAMRAIVectorReal<NDIM, double>>& diagnostic_vec : { linear_action, original_residual })
            {
            ...
            {
                TBOX_ERROR("FGMRES physical residual exceeded 1e-10: " << original_relative_residual << '\n');
            }
            ...
            for (const Pointer<SAMRAIVectorReal<NDIM, double>>& vector : { linear_action, original_residual })
            {
                free_vector_components(*vector);
            }
        }
...
main(int argc, char* argv[])
{
    ...
    if (test_case == "level_nullspace")
    {
        return run_level_nullspace(app);
    }
