...
// Application-defined, move-only evaluator state must survive its originating scope.
...
check_kernels()
{
    ...
    double error = std::max(
        { sample_error(Kernels::BSpline<1>{}, -0.25, std::array<double, 1>{ 1.0 }),
          sample_error(Kernels::BSpline<2>{}, 0.25, std::array<double, 2>{ 0.75, 0.25 }),
          sample_error(Kernels::BSpline<3>{}, 1.0, std::array<double, 3>{ 0.125, 0.75, 0.125 }),
          sample_error(Kernels::BSpline<4>{}, 1.5, std::array<double, 4>{ 1.0 / 48, 23.0 / 48, 23.0 / 48, 1.0 / 48 }),
          sample_error(
              Kernels::BSpline<5>{}, 1.5, std::array<double, 5>{ 1.0 / 24, 11.0 / 24, 11.0 / 24, 1.0 / 24, 0 }),
          sample_error(Kernels::BSpline<6>{},
                       2.5,
                       std::array<double, 6>{
                           1.0 / 3840, 237.0 / 3840, 1682.0 / 3840, 1682.0 / 3840, 237.0 / 3840, 1.0 / 3840 }),
          sample_error(Kernels::IB3{}, 1.0, std::array<double, 3>{ 1.0 / 6, 2.0 / 3, 1.0 / 6 }),
          sample_error(Kernels::IB4{}, 1.5, std::array<double, 4>{ a, b, b, a }),
          sample_error(
              Kernels::IB5{},
              1.5,
              std::array<double, 5>{
    ...
    error = std::max(error,
                     sample_error(Kernels::BSpline<7>{},
                                  2.75,
    ...
    error = std::max(error,
                     sample_error(Kernels::BSpline<8>{},
                                  3.5,
    ...
    const double moments = std::max({ moment_error(Kernels::BSpline<2>{}),
                                      moment_error(Kernels::BSpline<3>{}),
                                      moment_error(Kernels::BSpline<4>{}),
                                      moment_error(Kernels::BSpline<5>{}),
                                      moment_error(Kernels::BSpline<6>{}),
                                      moment_error(Kernels::BSpline<7>{}),
                                      moment_error(Kernels::BSpline<8>{}),
                                      moment_error(Kernels::IB3{}),
                                      moment_error(Kernels::IB4{}),
                                      moment_error(Kernels::IB5{}),
                                      moment_error(Kernels::IB6{}) });
...
SCInterpolationOperator
select_probe_factor(const IBKernel& kernel, const Select& select)
{
    if (kernel == IBKernel("PROBE"))
    {
        return select(ProbeEvaluator{});
    }
    if (kernel == IBKernel::BSPLINE_1)
    {
        return select(Kernels::BSpline<1>{});
    }
    if (kernel == IBKernel::BSPLINE_2)
    {
        return select(Kernels::BSpline<2>{});
    }
    TBOX_ASSERT(kernel == IBKernel::IB_4);
    return select(Kernels::IB4{});
}

SCInterpolationOperator
probe_operation(const IBKernelTensorProduct& kernel)
{
    return select_probe_factor(
        kernel[0],
        [&](auto normal)
        {
            return select_probe_factor(
                ...
                [&](auto tangential)
                {
                    return SCInterpolationOperator(
                        kernel, IBKernelTensorProductEvaluator{ std::move(normal), std::move(tangential) });
                });
        });
}
    ...
    for (bool normal_odd : { true, false })
    {
        ...
        if (normal_odd)
        {
            PETScMatUtilities::constructPatchLevelSCInterpOp(
                matrix,
                IBKernelTensorProductEvaluator{ Kernels::BSpline<3>{}, Kernels::BSpline<2>{} },
                X,
                counts,
                dof,
                level);
        ...
        else
        {
            PETScMatUtilities::constructPatchLevelSCInterpOp(
                matrix,
                IBKernelTensorProductEvaluator{ Kernels::BSpline<2>{}, Kernels::BSpline<3>{} },
                X,
                counts,
                dof,
                level);
...
main(int argc, char* argv[])
{
    ...
    if (input_file.find("unsupported.") != std::string::npos)
    {
    ...
    int failures = 0;
    {
        ...
        const bool unsupported = input_file.find("unsupported.") != std::string::npos;
        ...
        if (setup_probe)
        {
            ...
            {
                const SCInterpolationOperator wide(IBKernel("BSPLINE_8"),
                                                   IBKernelTensorProductEvaluator{ Kernels::BSpline<8>{} });
                Mat matrix = nullptr;
                method->constructInterpOp(matrix, wide, counts, dof, 0.125);
        ...
        if (unsupported)
        {
            ...
            method->constructInterpOp(
                matrix,
                SCInterpolationOperator(IBKernel(app->getInputDatabase()->getString("matrix_kernel"))),
                counts,
                dof,
                0.125);
            ierr = MatDestroy(&matrix);
        ...
        const SCInterpolationOperator custom = probe_operation({ IBKernel("PROBE"), IBKernel::BSPLINE_2 });
        const SCInterpolationOperator alternate(
            custom.getKernel(), IBKernelTensorProductEvaluator{ Kernels::BSpline<3>{}, Kernels::BSpline<2>{} });
        ...
        for (int step = 0; step < 2; ++step)
        {
            ...
            // Distinct evaluators with the same specification remain independent.
            Mat direct = nullptr, builtin = nullptr, bound_matrix = nullptr;
            PETScMatUtilities::constructPatchLevelSCInterpOp(
                direct,
                IBKernelTensorProductEvaluator{ ProbeEvaluator{}, Kernels::BSpline<2>{} },
                X,
                counts,
                dof,
                level);
            ...
            method->constructInterpOp(bound_matrix, custom, counts, dof, new_time);
            ierr = MatEqual(direct, bound_matrix, &equal);
            IBTK_CHKERRQ(ierr);
            if (!equal)
            {
                ++failures;
            }
            method->constructInterpOp(bound_matrix, alternate, counts, dof, new_time);
            ierr = MatEqual(builtin, bound_matrix, &equal);
            IBTK_CHKERRQ(ierr);
            failures += !equal;
            Vec changed_positions = nullptr;
            ierr = VecDuplicate(X, &changed_positions);
            IBTK_CHKERRQ(ierr);
            ierr = VecCopy(X, changed_positions);
            IBTK_CHKERRQ(ierr);
            PetscInt n_coordinates;
            ierr = VecGetLocalSize(changed_positions, &n_coordinates);
            IBTK_CHKERRQ(ierr);
            PetscScalar* coordinates;
            ierr = VecGetArray(changed_positions, &coordinates);
            IBTK_CHKERRQ(ierr);
            // Reorder the tabulated probes without changing their reference weights.
            std::reverse(coordinates, coordinates + n_coordinates);
            ierr = VecRestoreArray(changed_positions, &coordinates);
            IBTK_CHKERRQ(ierr);
            Mat changed_matrix = nullptr;
            custom.constructMatrix(changed_matrix, changed_positions, counts, dof, level);
            placement = check_matrix(changed_matrix, changed_positions, dofs, 3, 2) && placement;
            ierr = MatEqual(direct, changed_matrix, &equal);
            IBTK_CHKERRQ(ierr);
            failures += equal;
            ierr = MatDestroy(&changed_matrix);
            IBTK_CHKERRQ(ierr);
            ierr = VecDestroy(&changed_positions);
            IBTK_CHKERRQ(ierr);
            if (max_bspline_order >= 3)
            {
                method->constructInterpOp(bound_matrix,
                                          SCInterpolationOperator({ IBKernel::BSPLINE_3, IBKernel::BSPLINE_2 }),
                                          counts,
                                          dof,
                                          new_time);
                ierr = MatEqual(builtin, bound_matrix, &equal);
            ...
            // Explicit evaluators are independent of the configured B-spline limit.
            PETScMatUtilities::constructPatchLevelSCInterpOp(
                direct,
                IBKernelTensorProductEvaluator{ Kernels::BSpline<7>{}, Kernels::BSpline<2>{} },
                X,
                counts,
                dof,
                level);
            placement = check_matrix(direct, X, dofs, 7, 2, true) && placement;
            const SCInterpolationOperator seven(
                { IBKernel("BSPLINE_7"), IBKernel::BSPLINE_2 },
                IBKernelTensorProductEvaluator{ Kernels::BSpline<7>{}, Kernels::BSpline<2>{} });
            method->constructInterpOp(bound_matrix, seven, counts, dof, new_time);
            ierr = MatEqual(direct, bound_matrix, &equal);
            IBTK_CHKERRQ(ierr);
            if (!equal)
            {
            ...
            PETScMatUtilities::constructPatchLevelSCInterpOp(
                direct, IBKernelTensorProductEvaluator{ Kernels::IB6{} }, X, counts, dof, level);
            method->constructInterpOp(bound_matrix, SCInterpolationOperator(IBKernel::IB_6), counts, dof, new_time);
            ierr = MatEqual(direct, bound_matrix, &equal);
            IBTK_CHKERRQ(ierr);
            if (!equal)
            {
            ...
            for (int cw = 1; cw <= 4; ++cw)
            {
                for (int tw = 1; tw <= 4; ++tw)
                {
                    ...
                    strategy.constructInterpOp(
                        matrix, probe_operation({ kernel[cw - 1], kernel[tw - 1] }), counts, dof, new_time);
                    placement = check_matrix(matrix, X, dofs, cw, tw) && placement;
                    if (cw == tw)
                    {
                        Mat scalar = nullptr;
                        strategy.constructInterpOp(scalar, probe_operation(kernel[cw - 1]), counts, dof, new_time);
                        PetscBool equal;
                        ierr = MatEqual(matrix, scalar, &equal);
                        IBTK_CHKERRQ(ierr);
                        scalar_equivalence = scalar_equivalence && equal;
                        probe_operation(kernel[cw - 1]).constructMatrix(scalar, X, counts, dof, level);
                        ierr = MatEqual(matrix, scalar, &equal);
                        IBTK_CHKERRQ(ierr);
                        scalar_equivalence = scalar_equivalence && equal;
            ...
            for (bool normal : { true, false })
            {
                ...
                Mat matrix = nullptr;
                method->constructInterpOp(matrix,
                                          SCInterpolationOperator({ IBKernel("BSPLINE_" + std::to_string(cw)),
                                                                    IBKernel("BSPLINE_" + std::to_string(tw)) }),
                                          counts,
                                          dof,
                                          new_time);
            ...
            Mat half = nullptr;
            method->constructInterpOp(half, SCInterpolationOperator(kernel[0]), counts, dof, current_time + 0.0625);
