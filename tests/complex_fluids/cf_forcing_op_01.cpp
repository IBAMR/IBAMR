        ...
        if (test_draw)
        {
            ...
            for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
            {
                ...
                for (SAMRAIPatchLevel::Iterator p(level); p; p++)
                {
                    ...
                    Pointer<SAMRAICellData<double>> draw_data = patch->getPatchData(
                        var_db->getVariable("ComplexFluid::conform_draw"), var_db->getContext("ComplexFluid::CONTEXT"));
                    Pointer<SAMRAICellData<double>> C_data = patch->getPatchData(cf_forcing->getVariableIdx());
