                                  ...
                                  void* ctx)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(finest_level); p; p++)
    {
        ...
        Pointer<SAMRAICellData<double>> interface_cell_bool_data = patch->getPatchData(interface_cell_patch_idx);
    ...
    for (SAMRAIPatchLevel::Iterator p(finest_level); p; p++)
    {
        ...
        const double* const patch_dx = patch_geom->getDx();
        Pointer<SAMRAICellData<double>> g_cc_data = patch->getPatchData(g_cc_scratch_idx);
        Pointer<SAMRAICellData<double>> interface_cell_data = patch->getPatchData(interface_cell_patch_idx);
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<SAMRAICellVariable<double>> phi_solid_var = new SAMRAICellVariable<double>(ls_name);
        ...
        SAMRAIRobinBcCoefStrategy* q_bc_coef = nullptr;
        ...
        Pointer<SAMRAICellVariable<double>> interface_cell_var = new SAMRAICellVariable<double>("temp_ghost_cc_var");
        ...
        {
            Pointer<SAMRAICellVariable<double>> F_var = new SAMRAICellVariable<double>("F", 1);
        ...
        const int phi_scratch_idx =
            var_db->registerVariableAndContext(phi_solid_var, var_db->getContext("scratch"), SAMRAIIntVector(1));
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                ...
                for (SAMRAIPatchLevel::Iterator p(level); p; p++)
                {
                    ...
                    const SAMRAIBox& patch_box = patch->getBox();
                    const Pointer<SAMRAICartesianPatchGeometry> patch_geom = patch->getPatchGeometry();
                    Pointer<SAMRAICellData<double>> phi_data = patch->getPatchData(phi_idx);
                    Pointer<SAMRAICellData<double>> H_data = patch->getPatchData(phi_cloned_idx);
