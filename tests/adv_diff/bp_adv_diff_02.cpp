                                  ...
                                  void* ctx)
{
    ...
    for (SAMRAIPatchLevel::Iterator p(finest_level); p; p++)
    {
        ...
        SAMRAIPointer<SAMRAICellData<double>> interface_cell_bool_data = patch->getPatchData(interface_cell_patch_idx);
    ...
    for (SAMRAIPatchLevel::Iterator p(finest_level); p; p++)
    {
        ...
        const double* const patch_dx = patch_geom->getDx();
        SAMRAIPointer<SAMRAICellData<double>> g_cc_data = patch->getPatchData(g_cc_scratch_idx);
        SAMRAIPointer<SAMRAICellData<double>> interface_cell_data = patch->getPatchData(interface_cell_patch_idx);
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> phi_solid_var = new SAMRAICellVariable<double>(ls_name);
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> interface_cell_var =
            new SAMRAICellVariable<double>("temp_ghost_cc_var");
        ...
        {
            SAMRAIPointer<SAMRAICellVariable<double>> F_var = new SAMRAICellVariable<double>("F", 1);
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                ...
                for (SAMRAIPatchLevel::Iterator p(level); p; p++)
                {
                    ...
                    SAMRAIPointer<SAMRAICellData<double>> H_data = patch->getPatchData(phi_cloned_idx);
