...
class DummyCoarsen : public SAMRAICoarsenOperator
{
public:
    bool findCoarsenOperator(const SAMRAIPointer<SAMRAIVariable>&, const std::string& op_name) const
    {
    ...
    SAMRAIIntVector getStencilWidth() const
    {
        return 0;
    }

    void
    coarsen(SAMRAIPatch& coarse, const SAMRAIPatch&, int dst_idx, int, const SAMRAIBox&, const SAMRAIIntVector&) const
    {
        pout << "Coarsening\n";
        SAMRAIPointer<SAMRAICellData<double>> data = coarse.getPatchData(dst_idx);
        data->fillAll(0.0);
        return;
    }
...
class DummyRefine : public SAMRAIRefineOperator
{
public:
    bool findRefineOperator(const SAMRAIPointer<SAMRAIVariable>&, const std::string& op_name) const
    {
        return op_name == s_op_name;
    }
    ...
    void refine(SAMRAIPatch& fine, const SAMRAIPatch&, int dst_idx, int, const SAMRAIBox&, const SAMRAIIntVector&) const
    {
        pout << "Refining\n";
        SAMRAIPointer<SAMRAICellData<double>> data = fine.getPatchData(dst_idx);
        data->fillAll(0.0);
        return;
    }
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> U_var = new SAMRAICellVariable<double>("U", NDIM);
        ...
        SAMRAIPointer<SAMRAIFaceVariable<double>> u_adv_var = new SAMRAIFaceVariable<double>("u_adv");
        ...
        {
            SAMRAIPointer<SAMRAICellVariable<double>> F_var = new SAMRAICellVariable<double>("F", NDIM);
