...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<SAMRAISideVariable<double>> u_sc_var = new SAMRAISideVariable<double>("u_sc");
        Pointer<SAMRAISideVariable<double>> f_sc_var = new SAMRAISideVariable<double>("f_sc");
        Pointer<SAMRAISideVariable<double>> e_sc_var = new SAMRAISideVariable<double>("e_sc");
        Pointer<SAMRAISideVariable<double>> r_sc_var = new SAMRAISideVariable<double>("r_sc");
#if (NDIM == 2)
        Pointer<SAMRAINodeVariable<double>> mu_nc_var = new SAMRAINodeVariable<double>("mu_node");
        const int mu_nc_idx = var_db->registerVariableAndContext(mu_nc_var, ctx, SAMRAIIntVector(1));
#elif (NDIM == 3)
        Pointer<SAMRAIEdgeVariable<double>> mu_ec_var = new SAMRAIEdgeVariable<double>("mu_edge");
        const int mu_ec_idx = var_db->registerVariableAndContext(mu_ec_var, ctx, SAMRAIIntVector(1));
        Pointer<SAMRAICellVariable<double>> mu_cc_var = new SAMRAICellVariable<double>("mu_cc");
        ...
        Pointer<SAMRAICellVariable<double>> u_cc_var = new SAMRAICellVariable<double>("u_cc", NDIM);
        Pointer<SAMRAICellVariable<double>> f_cc_var = new SAMRAICellVariable<double>("f_cc", NDIM);
        Pointer<SAMRAICellVariable<double>> e_cc_var = new SAMRAICellVariable<double>("e_cc", NDIM);
        Pointer<SAMRAICellVariable<double>> r_cc_var = new SAMRAICellVariable<double>("r_cc", NDIM);
        ...
        transaction_comp[0] = InterpolationTransactionComponent(mu_nc_idx,
                                                                ...
                                                                /*mu_bc_coef*/ nullptr,
                                                                Pointer<SAMRAIVariableFillPattern>(nullptr));
#elif (NDIM == 3)
        ...
        std::vector<Pointer<SAMRAISAMRAIVectorReal<double>>> U_nul_vecs(NDIM);
            ...
            for (unsigned int k = 0; k < NDIM; ++k)
            {
                ...
                for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
                {
                    ...
                    for (SAMRAIPatchLevel::Iterator p(level); p; p++)
                    {
                        ...
                        Pointer<SAMRAISideData<double>> U_nul_data =
        ...
        if (has_velocity_nullspace)
        {
            ...
            for (unsigned int k = 0; k < NDIM; ++k)
            {
                ...
                for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
                {
                    ...
                    {
                        Pointer<SAMRAIPatch> patch = level->getPatch(p());
                        Pointer<SAMRAISideData<double>> r_data =
                            patch->getPatchData(r_vec.getComponentDescriptorIndex(0));
                        r_data->getArrayData(k).fillAll(1.0);
                    }
                }

                double fk_mean = f_vec.dot(Pointer<SAMRAISAMRAIVectorReal<double>>(&r_vec, false)) /
                                 r_vec.dot(Pointer<SAMRAISAMRAIVectorReal<double>>(&r_vec, false));
        ...
        if (has_velocity_nullspace)
        {
            for (unsigned int k = 0; k < NDIM; ++k)
            {
                ...
                const double uk_mean = u_vec.dot(Pointer<SAMRAISAMRAIVectorReal<double>>(&r_vec, false)) /
                                       r_vec.dot(Pointer<SAMRAISAMRAIVectorReal<double>>(&r_vec, false));
        ...
        e_vec.subtract(Pointer<SAMRAISAMRAIVectorReal<double>>(&e_vec, false),
                       Pointer<SAMRAISAMRAIVectorReal<double>>(&u_vec, false));
        ...
        r_vec.subtract(Pointer<SAMRAISAMRAIVectorReal<double>>(&f_vec, false),
                       Pointer<SAMRAISAMRAIVectorReal<double>>(&r_vec, false));
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAIEdgeData<double>> mu_ec_data = patch->getPatchData(mu_ec_idx);
                Pointer<SAMRAICellData<double>> mu_cc_data = patch->getPatchData(mu_cc_idx);
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber() - 1; ++ln)
        {
            ...
            Pointer<SAMRAIPatchLevel> next_finer_level = patch_hierarchy->getPatchLevel(ln + 1);
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAICellData<double>> e_cc_data = patch->getPatchData(e_cc_idx);
                Pointer<SAMRAICellData<double>> r_cc_data = patch->getPatchData(r_cc_idx);
