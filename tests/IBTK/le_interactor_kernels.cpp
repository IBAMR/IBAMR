...
void
check_patch(Pointer<Patch<NDIM>> patch,
            bool expect_error,
            const std::string& error_kernel,
            const std::string& error_operation)
{
    ...
    for (int component = 0; component < NDIM; ++component)
    {
        for (SideIterator<NDIM> i(field->getGhostBox(), component); i; i++)
        {
            ...
            for (int d = 0; d < NDIM; ++d)
            {
                value += std::sin((0.13 + 0.09 * d) * (i()(d) + 0.3 * component));
            }
            (*field)(i()) = value;
        }
    }
    ...
    if (expect_error)
    {
        ...
        if (error_operation == "masked_empty_interpolate")
        {
            string_value.clear();
            LEInteractor::interpolate(string_value, NDIM, X, NDIM, field, field, patch, box, error_kernel);
        }
        else if (error_operation == "masked_spread")
        {
            LEInteractor::spread(field, string_spread, force, NDIM, X, NDIM, patch, box, error_kernel);
        }
        else
        {
    ...
    LEInteractor::spread(mask, string_masked_spread, force, NDIM, X, NDIM, patch, box, "IB_4");
    for (int component = 0; component < NDIM; ++component)
    {
        for (SideIterator<NDIM> i(field->getGhostBox(), component); i; i++)
        {
