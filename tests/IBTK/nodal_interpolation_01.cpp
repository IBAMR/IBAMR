...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<hier::VariableNd> u_var, e_var;
        ...
        Pointer<FaceVariableNd<double> > u_fc_var = new FaceVariableNd<double>("u_fc");
        ...
        Pointer<NodeVariableNd<double> > u_nc_var =
        ...
        const int u_nc_idx = var_db->registerVariableAndContext(u_nc_var, ctx, IntVectorNd(interp_back ? 1 : 0));
        ...
        if (interp_back)
        {
            ...
            e_idx = var_db->registerVariableAndContext(e_cc_var, ctx, IntVectorNd(0));
            e_var = e_cc_var;
        }
        else
        {
            e_idx = var_db->registerVariableAndContext(e_nc_var, ctx, IntVectorNd(0));
            e_var = e_nc_var;
        }
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber() - 1; ++ln)
        {
            ...
            for (PatchLevelNd::Iterator p(level); p; p++)
            {
                ...
                Pointer<NodeDataNd<double> > e_nc_data = patch->getPatchData(e_idx);
                if (e_nc_data)
                {
        ...
        if (input_db->getBoolWithDefault("print_cf_plane", false))
        {
            ...
            for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
            {
                ...
                {
                    Pointer<PatchNd> patch = level->getPatch(p());
                    Pointer<NodeDataNd<double> > u_data = patch->getPatchData(u_nc_idx);
