...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<SAMRAIVariable> u_var, e_var;
        ...
        SAMRAIPointer<SAMRAIFaceVariable<double>> u_fc_var = new SAMRAIFaceVariable<double>("u_fc");
        ...
        SAMRAIPointer<SAMRAINodeVariable<double>> u_nc_var =
        ...
        for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber() - 1; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                SAMRAIPointer<SAMRAINodeData<double>> e_nc_data = patch->getPatchData(e_idx);
                if (e_nc_data)
                {
        ...
        if (input_db->getBoolWithDefault("print_cf_plane", false))
        {
            ...
            for (int ln = 0; ln <= patch_hierarchy->getFinestLevelNumber(); ++ln)
            {
                ...
                {
                    SAMRAIPointer<SAMRAIPatch> patch = level->getPatch(p());
                    SAMRAIPointer<SAMRAINodeData<double>> u_data = patch->getPatchData(u_nc_idx);
