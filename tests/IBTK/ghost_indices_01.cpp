...
main(int argc, char* argv[])
{
    ...
    TimerManager::createManager(nullptr);
    {
        ...
        auto u_var = use_cell ? Pointer<SAMRAIVariable>(new SAMRAICellVariable<double>("u", u_depth)) :
                                Pointer<SAMRAIVariable>(new SAMRAISideVariable<double>("u", u_depth));
        auto dof_var = use_cell ? Pointer<SAMRAIVariable>(new SAMRAICellVariable<int>("dof", u_depth)) :
                                  Pointer<SAMRAIVariable>(new SAMRAISideVariable<int>("dof", u_depth));
        ...
        Pointer<SAMRAIPatchLevel> patch_level = patch_hierarchy->getPatchLevel(level);
        for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
        {
            ...
            if (use_cell)
            {
                Pointer<SAMRAICellData<int>> dof_data = patch->getPatchData(dof_idx);
                dof_data->print(patch_box, out);
            }
            else
            {
                Pointer<SAMRAISideData<int>> dof_data = patch->getPatchData(dof_idx);
                dof_data->print(patch_box, out);
            }
        }
        IBTK_MPI::barrier();
