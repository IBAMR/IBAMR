...
main(int argc, char* argv[])
{
    ...
    TimerManager::createManager(nullptr);
    {
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> f_approx_cc_var = new SAMRAICellVariable<double>("f_approx_cc");
        ...
        if (test_copied_vector)
        {
            e_vec.subtract(SAMRAIPointer<SAMRAISAMRAIVectorReal<double>>(&f_vec, false), f_copied);
        }
        else if (test_duplicated_vector)
        {
            e_vec.subtract(SAMRAIPointer<SAMRAISAMRAIVectorReal<double>>(&f_vec, false), f_duplicated);
        }
        else
        {
            e_vec.subtract(SAMRAIPointer<SAMRAISAMRAIVectorReal<double>>(&f_vec, false),
                           SAMRAIPointer<SAMRAISAMRAIVectorReal<double>>(&f_standard, false));
        }
...

        {
            ...
            for (int ln = 0; ln <= finest_level; ++ln)
            {
                ...
                for (typename SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
                {
                    ...
                    SAMRAIPointer<SAMRAICartesianPatchGeometry> patch_geometry =
