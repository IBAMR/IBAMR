...
main(int argc, char* argv[])
{
    ...
    TimerManager::createManager(nullptr);
    {
        ...
        Pointer<SAMRAIFaceVariable<double>> f_var = new SAMRAIFaceVariable<double>("f_var");
          ...
          uniform_double_distribution& dis)
{
    ...
    for (int ln = 0; ln <= hierarchy->getFinestLevelNumber(); ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> c_data = patch->getPatchData(idx);
            Pointer<SAMRAINodeData<double>> n_data = patch->getPatchData(idx);
            Pointer<SAMRAIEdgeData<double>> e_data = patch->getPatchData(idx);
            Pointer<SAMRAISideData<double>> s_data = patch->getPatchData(idx);
            Pointer<SAMRAIFaceData<double>> f_data = patch->getPatchData(idx);
