...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> cc_var = new SAMRAICellVariable<double>("cc");
        SAMRAIPointer<SAMRAISideVariable<double>> sc_var = new SAMRAISideVariable<double>("sc");
        SAMRAIPointer<SAMRAINodeVariable<double>> nc_var = new SAMRAINodeVariable<double>("nc", NDIM);
        const int gcw = 4;
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                SAMRAIPointer<SAMRAINodeData<double>> nc_data = patch->getPatchData(nc_idx);
