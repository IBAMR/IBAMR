...
/*
 * This test constructs the RIDP restriction operator induced by the RT0
 * velocity prolongation and verifies the expected row and column sum
 * invariants on a simple periodic multilevel hierarchy.
 */
...
main(int argc, char* argv[])
{
    ...
    MatInfo prolong_info;
    MatInfo ridp_info;
    ierr = MatGetInfo(prolong_mat, MAT_GLOBAL_SUM, &prolong_info);
    IBTK_CHKERRQ(ierr);
    ierr = MatGetInfo(ridp_mat, MAT_GLOBAL_SUM, &ridp_info);
    IBTK_CHKERRQ(ierr);

    PetscReal restriction_scale_min = 0.0, restriction_scale_max = 0.0;
    ierr = VecMin(restriction_scale, nullptr, &restriction_scale_min);
    IBTK_CHKERRQ(ierr);
    ierr = VecMax(restriction_scale, nullptr, &restriction_scale_max);
    IBTK_CHKERRQ(ierr);
    ...
    plog << "Input database:\n";
    input_db->printClassData(plog);
    pout << "n_fine_dofs = " << n_fine << "\n";
    pout << "n_coarse_dofs = " << n_coarse << "\n";
    pout << "n_fine_velocity_dofs = " << fine_velocity_dofs.size() << "\n";
    pout << "n_fine_pressure_dofs = " << fine_pressure_dofs.size() << "\n";
    pout << "n_coarse_velocity_dofs = " << coarse_velocity_dofs.size() << "\n";
    pout << "n_coarse_pressure_dofs = " << coarse_pressure_dofs.size() << "\n";
    pout << "prolongation_nnz = " << static_cast<int>(prolong_info.nz_used) << "\n";
    pout << "ridp_nnz = " << static_cast<int>(ridp_info.nz_used) << "\n";
    pout << "restriction_scale_min = " << restriction_scale_min << "\n";
    pout << "restriction_scale_max = " << restriction_scale_max << "\n";
    pout << "row_bad_velocity_count = " << row_bad_vel_count << "\n";
    pout << "row_bad_pressure_count = " << row_bad_pres_count << "\n";
    pout << "col_bad_velocity_count = " << col_bad_vel_count << "\n";
    pout << "col_bad_pressure_count = " << col_bad_pres_count << "\n";
    pout << "row_velocity_max_error = " << row_vel_max_err << "\n";
    pout << "row_pressure_max_error = " << row_pres_max_err << "\n";
    pout << "col_velocity_max_error = " << col_vel_max_err << "\n";
    pout << "col_pressure_max_error = " << col_pres_max_err << "\n";
