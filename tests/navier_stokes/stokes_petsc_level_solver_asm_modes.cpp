...
compute_stats(const std::vector<std::set<int>>& overlap_sets, const std::vector<std::set<int>>& nonoverlap_sets)
{
    ...
    for (std::size_t k = 0; k < stats.n_subdomains; ++k)
    {
        ...
        overlap_hash = overlap_hash * 1315423911ULL + static_cast<std::uint64_t>((k + 1) * (n_overlap + 3));
        nonoverlap_hash = nonoverlap_hash * 1315423911ULL + static_cast<std::uint64_t>((k + 1) * (n_nonoverlap + 7));
    }
...
subdomain_dofs_to_sets(const std::vector<std::vector<int>>& subdomain_dofs)
{
    std::vector<std::set<int>> subdomain_sets(subdomain_dofs.size());
    for (std::size_t k = 0; k < subdomain_dofs.size(); ++k)
    {
        subdomain_sets[k].insert(subdomain_dofs[k].begin(), subdomain_dofs[k].end());
    }
    return subdomain_sets;
}
...
main(int argc, char* argv[])
{
...
#if (NDIM == 2)
    const std::string seed_traversal_order =
        test_db->getStringWithDefault("coupling_aware_asm_seed_traversal_order", "I_J");
#else
    const std::string seed_traversal_order =
        test_db->getStringWithDefault("coupling_aware_asm_seed_traversal_order", "I_J_K");
#endif
    ...
    const std::string compare_seed_traversal_order =
        test_db->getStringWithDefault("compare_coupling_aware_asm_seed_traversal_order", seed_traversal_order);
    ...
    auto run_mode = [&](const std::string& mode,
                        ...
                        const int mode_seed_stride,
                        const std::string& mode_seed_traversal_order)
        -> std::pair<std::vector<std::set<int>>, std::vector<std::set<int>>>
    {
        ...
        solver_db->putString("coupling_aware_asm_seed_traversal_order", mode_seed_traversal_order);
        ...
        const auto& subdomain_dofs = solver->getASMSubdomains();
        const auto& nonoverlap_dofs = solver->getASMNonoverlapSubdomains();
        const std::vector<std::set<int>> overlap_sets = subdomain_dofs_to_sets(subdomain_dofs);
        const std::vector<std::set<int>> nonoverlap_sets = subdomain_dofs_to_sets(nonoverlap_dofs);
    ...
    const auto run =
        run_mode(asm_mode, closure_policy, box_size, overlap_size, seed_axis, seed_stride, seed_traversal_order);
    ...
    pout << "coupling_aware_asm_seed_traversal_order = " << seed_traversal_order << "\n";
    ...
    if (run_comparison)
    {
        ...
        pout << "compare_coupling_aware_asm_seed_traversal_order = " << compare_seed_traversal_order << "\n";
