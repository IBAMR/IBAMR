...
main(int argc, char* argv[])
{
    ...
    if (test_db->getBoolWithDefault("transfer_parity", false))
    {
        ...
        // Form D P^T P, where P is the prolongation matrix and D the diagonal restriction scaling. On this
        // periodic fixture its velocity row sums and column sums are one, and its pressure row sums and
        // column sums are zero, because P has zero pressure rows and columns.
        Mat restriction_prolongation = nullptr;
        ierr = MatTransposeMatMult(
            prolongation, prolongation, MAT_INITIAL_MATRIX, PETSC_DEFAULT, &restriction_prolongation);
        IBTK_CHKERRQ(ierr);
        ierr = MatDiagonalScale(restriction_prolongation, scale, nullptr);
        IBTK_CHKERRQ(ierr);
        ...
        ierr = VecSet(x, 1.0);
        IBTK_CHKERRQ(ierr);
        ierr = MatMult(restriction_prolongation, x, coarse_result);
        IBTK_CHKERRQ(ierr);
        ierr = VecAXPY(coarse_result, -1.0, coarse_expected);
        IBTK_CHKERRQ(ierr);
        ...
        IBTK_CHKERRQ(ierr);
        ierr = MatMultTranspose(restriction_prolongation, x, coarse_result);
        IBTK_CHKERRQ(ierr);
        ierr = VecAXPY(coarse_result, -1.0, coarse_expected);
        IBTK_CHKERRQ(ierr);
        ...
        plog << "minimum transfer norm = " << minimum_transfer_norm << '\n'
             ...
             << "restriction-prolongation row sum error = " << row_error << '\n'
             << "restriction-prolongation column sum error = " << column_error << '\n';
        ...
        ierr = MatDestroy(&restriction_prolongation);
        IBTK_CHKERRQ(ierr);
    ...
    ierr = AODestroy(&ordering);
    IBTK_CHKERRQ(ierr);
    if (!(std::isfinite(reference_error) && std::isfinite(schedule_error) && result_norm > 0.0 &&
          reference_error < 1.0e-12 && schedule_error < 1.0e-12 && pressure_leak < 1.0e-12))
    {
        TBOX_ERROR("Side prolongation check failed: interpolation error = "
                   << reference_error << ", schedule error = " << schedule_error
                   << ", pressure leakage = " << pressure_leak << ", result norm = " << result_norm << ".\n");
    }
    return 0;
}
