...
double
compute_cell_coordinate(const CellIndex<NDIM>& i,
                        const int axis,
                        const Box<NDIM>& patch_box,
                        const double* x_lower,
                        const double* dx)
{
    const int il = i(axis) - patch_box.lower(axis);
    return x_lower[axis] + dx[axis] * (static_cast<double>(il) + 0.5);
}

double
compute_affine_cell_value(const CellIndex<NDIM>& i,
                          const Box<NDIM>& patch_box,
                          const double* x_lower,
                          const double* dx,
                          ...
                          const double constant)
{
    ...
    {
        val += coeffs[d] * compute_cell_coordinate(i, d, patch_box, x_lower, dx);
    }
    return val;
}

double
compute_nonlinear_cell_value(const CellIndex<NDIM>& i,
                             const Box<NDIM>& patch_box,
                             const double* x_lower,
                             const double* dx)
{
    ...
    for (int d = 0; d < NDIM; ++d)
    {
        const double xd = compute_cell_coordinate(i, d, patch_box, x_lower, dx);
        sum += (d + 1.0) * xd;
        prod *= (0.4 + xd);
    }
    return 0.35 * std::sin(pi * sum) + 0.2 * std::cos(2.0 * pi * sum) + 0.1 * prod;
}
...
set_pressure_profile(Pointer<CellData<NDIM, double>> p_data,
                     ...
                     const std::array<double, NDIM>& affine_coeffs,
                     const double affine_constant)
{
    ...
    for (Box<NDIM>::Iterator b(patch_box); b; b++)
    {
        ...
        {
            val = compute_affine_cell_value(i, patch_box, x_lower, dx, affine_coeffs, affine_constant);
        }
        else
        {
            val = compute_nonlinear_cell_value(i, patch_box, x_lower, dx);
        }
        (*p_data)(i) = val;
    }
}
...
void
zero_side_data(Pointer<SideData<NDIM, double>> u_data, Pointer<Patch<NDIM>> patch)
{
    const Box<NDIM>& patch_box = patch->getBox();
...
main(int argc, char* argv[])
{
    ...
    const double affine_constant = test_db->getDoubleWithDefault("affine_constant", 0.7);
    ...
    if (test_mode == "prolongation")
    {
        ...
        zero_side_data(u_coarse_data, coarse_patch);
        zero_side_data(u_fine_mat_data, fine_patch);
        zero_side_data(u_fine_mf_data, fine_patch);
        p_fine_mat_data->fillAll(0.0);
        p_fine_mf_data->fillAll(0.0);
        set_pressure_profile(p_coarse_data, coarse_patch, profile_type, affine_coeffs, affine_constant);
        ...
        Pointer<RefineOperator<NDIM>> p_refine_op = grid_geom->lookupRefineOperator(p_fine_mf_var, "CONSTANT_REFINE");
    ...
    else if (test_mode == "restriction")
    {
        ...
        zero_side_data(u_fine_data, fine_patch);
        zero_side_data(u_coarse_mat_data, coarse_patch);
        zero_side_data(u_coarse_mf_data, coarse_patch);
        p_coarse_mat_data->fillAll(0.0);
        p_coarse_mf_data->fillAll(0.0);
        set_pressure_profile(p_fine_data, fine_patch, profile_type, affine_coeffs, affine_constant);
        const double nontrivial_tol = 1.0e-14;
        const double norm_match_tol = 1.0e-12;
        ...
        Pointer<CartesianGridGeometry<NDIM>> grid_geom = patch_hierarchy->getGridGeometry();
        Pointer<CoarsenOperator<NDIM>> p_coarsen_op =
            grid_geom->lookupCoarsenOperator(p_coarse_mf_var, "CONSERVATIVE_COARSEN");
