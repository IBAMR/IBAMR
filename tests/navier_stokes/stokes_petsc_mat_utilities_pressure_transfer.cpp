...
transfer_profile(TransferFixture& fixture, const bool restriction, const ProfileKind kind, Mat transfer)
{
    ...
    // Constant profiles are reproduced everywhere; affine profiles are reproduced away from the boundary.
    const auto exact_error = [&]()
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(output_level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator b(patch->getBox()); b; b++)
            {
                ...
                if (kind == AFFINE_PROFILE && !is_interior(X, fixture.x_lower, fixture.x_upper, fixture.coarse_dx))
                {
                    continue;
                }
...
void
require(const bool ok, const char* description)
{
    if (!ok) TBOX_ERROR("Pressure transfer check failed: " << description << ". See the log for the errors.\n");
}
...
void
print_transfer(const std::string& name, const TransferResult& r)
{
...
main(int argc, char* argv[])
{
    ...
    print_transfer("prolongation", prolongation_test);
    ...
    require(transfer_ok(prolongation_test, true), "prolongation of the test profile");
    require(transfer_ok(prolongation_constant, false), "prolongation of a constant profile");
    require(transfer_ok(prolongation_affine, false), "prolongation of an affine profile");
    ...
    print_transfer("restriction", restriction_test);
    ...
    require(transfer_ok(restriction_test, true), "restriction of the test profile");
    require(transfer_ok(restriction_constant, false), "restriction of a constant profile");
    require(transfer_ok(restriction_affine, false), "restriction of an affine profile");
    require(std::isfinite(adjoint_error) && adjoint_error < 1.0e-12, "restriction volume adjoint");
    require(std::isfinite(write_range_error) && write_range_error < 1.0e-12, "restriction write range");
    ...
    if (IBTK_MPI::getNodes() > 1)
    {
        ...
        require(counts.first > 0 && counts.second > 0, "off-process pressure matrix entries on every rank");
    }
