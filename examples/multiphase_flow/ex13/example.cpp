...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            SAMRAIPointer<CellVariableNd<double> > p_var = navier_stokes_integrator->getPressureVariable();
            ...
            for (int i = 0; i < num_pressure_probes; ++i)
            {
                ...
                for (int ln = finest_ln; ln >= coarsest_ln; --ln)
                {
                    ...
                    const CellIndexNd cell_idx = IndexUtilities::getCellIndex(
                    ...
                    for (PatchLevelNd::Iterator p(level); p; p++)
                    {
                        ...
                        SAMRAIPointer<CellDataNd<double> > p_data = patch->getPatchData(p_idx);
