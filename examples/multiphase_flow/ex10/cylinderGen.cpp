...
{
    // mesh parameters:
    const double radius = 0.055;
    const double D = 2.0 * radius;
    const double GAS_LS_INIT = 6.0 * D;
    const double Lx = 20.0 * D;
    const double Nx = 440.0 * 2.0;
    const double dx = Lx / Nx;

    // disk parameters:
    const double h = dx;
    const double X_shift = Lx / 2.0;
    const double Y_shift = GAS_LS_INIT + D;
    ...
    for (std::size_t outer = 0; outer < xs.size(); ++outer) total_n_pts += xs[outer]->size();
    out << total_n_pts << '\n';
    for (std::size_t outer = 0; outer < xs.size(); ++outer)
    {
        ...
        for (std::size_t inner = 0; inner < x.size(); ++inner)
            out << std::setfill('0') << std::setw(8) << std::left << x[inner] + X_shift << '\t' << std::setfill('0')
                << std::setw(8) << std::left << y[inner] + Y_shift << '\n';
    }
}
