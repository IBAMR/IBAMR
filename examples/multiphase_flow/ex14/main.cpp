...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        const int reinit_interval = input_db->getInteger("LS_REINIT_INTERVAL");
        ...
        compute_variable_surface_tension_coef_ctx.sigma0 = 1.0e-1;
        compute_variable_surface_tension_coef_ctx.dsigma_dT0 = -1.0e-1;
        compute_variable_surface_tension_coef_ctx.T_ref = input_db->getDouble("REFERENCE_TEMPERATURE");
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            const double gamma = 0.13333333333;
            const double U_ref = -(2.0 * gamma * beta * radius) / (6.0 * mu_liquid + 9.0 * mu_gas);
            ...
            pout << "vol is\t" << vol << "\n";
            pout << "Rise velocity is\t" << v_integral / vol << "at time \t" << loop_time << "\t" << loop_time / t_ref
                 << "\n";
