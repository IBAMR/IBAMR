...
main(int argc, char* argv[])
{
    ...
    const LibMeshInit& init = ibtk_init.getLibMeshInit();
    {
        ...
        SAMRAIPointer<SAMRAIDatabase> input_db = new SAMRAIInputDatabase("input_db");
        SAMRAIInputManager::getManager()->parseInputFile(input_filename, input_db);

        // Retrieve "Main" section of the input database.
        SAMRAIPointer<SAMRAIDatabase> main_db = input_db->getDatabase("Main");
        ...
        SAMRAIPointer<SAMRAIVariableContext> current_ctx =
            var_db->getContext("INSStaggeredHierarchyIntegrator::CURRENT");
        SAMRAIPointer<SAMRAIVariableContext> scratch_ctx =
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> P_var =
            new SAMRAICellVariable<double>("INSStaggeredHierarchyIntegrator::P_extrap");
             ...
             coarse_iteration_num += coarse_hier_dump_interval, fine_iteration_num += fine_hier_dump_interval)
        {
            ...
            SAMRAIComponentSelector hier_data;
            hier_data.setFlag(U_idx);
            hier_data.setFlag(P_idx);

            SAMRAIPointer<SAMRAIHDFDatabase> coarse_hier_db = new SAMRAIHDFDatabase("coarse_hier_db");
            ...
            SAMRAIPointer<SAMRAIHDFDatabase> fine_hier_db = new SAMRAIHDFDatabase("fine_hier_db");
            ...
            for (int ln = 0; ln <= coarse_patch_hierarchy->getFinestLevelNumber(); ++ln)
            {
                ...
                SAMRAIComponentSelector data_indices;
