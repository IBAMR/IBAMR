...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        const Pointer<SAMRAIVariable> u_var = navier_stokes_integrator->getVelocityVariable();
        const Pointer<VariableContext> u_ctx = navier_stokes_integrator->getCurrentContext();
        const int u_idx = var_db->mapVariableAndContextToIndex(u_var, u_ctx);

        const Pointer<SAMRAIVariable> p_var = navier_stokes_integrator->getPressureVariable();
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            const Pointer<SAMRAICartesianGridGeometry> coarsest_grid_geom = coarsest_level->getGridGeometry();
