...
int main(int argc, char* argv[])
{
   ...
   Mesh cylinder_mesh_thin(init.comm(), NDIM-1);
    ...
    for (unsigned int j = 0; j <= NXi_elem; j++)
        ...
        tbox::Pointer<IBHierarchyIntegrator> time_integrator = new IBExplicitHierarchyIntegrator(
        ...
        tbox::Pointer<LoadBalancer<NDIM> > load_balancer = new LoadBalancer<NDIM>(
            "LoadBalancer", app_initializer->getComponentDatabase("LoadBalancer"));
        tbox::Pointer<GriddingAlgorithm<NDIM> > gridding_algorithm = new GriddingAlgorithm<NDIM>(
        ...
        tbox::Pointer<VisItDataWriter<NDIM> > visit_data_writer =
                ...
                if (SAMRAI_MPI::getRank() == 0)
        {
            ...
            flow_rate_stream.open("flow_rate_outflow_kappa_" + std::to_string(int(kappa_s_thin)) +".curve", ios_base::out | ios_base::trunc);
                              ...
                              const string& /*data_dump_dirname*/)
{
    ...
    for (int ln = finest_ln; ln >= coarsest_ln; --ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            const tbox::Pointer<CartesianPatchGeometry<NDIM> > patch_geom =
            ...
            if (ln < finest_ln)
            {
                ...
                tbox::Pointer<PatchLevel<NDIM> > next_finer_level =
                 ...
                 const string& /*data_dump_dirname*/)
{
    ...
    for (MeshBase::const_element_iterator el_it = el_begin; el_it != el_end; ++el_it)
    {
        ...
        for (unsigned int qp = 0; qp < n_qp; ++qp)
        {
            ...
            tether_body_force_function_thin(F_qp, n, N, FF_qp, x_qp, q_point[qp], elem, 0, 
											var_data, grad_var_data, loop_time, force_fcn_ctx);
