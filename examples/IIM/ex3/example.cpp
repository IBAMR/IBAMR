...
void calculateGeomQuantitiesOfStructure(double& vol,  // volume of the body
...
void UpdateSurfaceMeshCoordinates(Mesh& boundary_mesh, EquationSystems* airfoil_equation_systems, EquationSystems* solid_systems)
{
                      ...
                      void* /*ctx*/)
{
    ...
    for (unsigned int d = 0; d < NDIM; ++d)
    {
        F(d) = kappa_s * (x_solid[d] - x_bndry(d)) + eta_s * (u_solid[d] - u_bndry[d]); // (u_solid_n - u_bndry_n) * n(d);
...
void update_solid_vel_pos(VectorValue<double>& Y,
...
int main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        TensorValue<double> Rz(std::cos(theta), -std::sin(theta), 0.0, std::sin(theta), std::cos(theta), 0.0, 0.0, 0.0, 1.0);
        ...
        Pointer<IIMethod> ib_method_ops =
            new IIMethod("IIMethod", app_initializer->getComponentDatabase("IIMethod"), &bndry_mesh,
		...
		if (USE_DISCON_ELEMS)
			ib_method_ops->registerDisconElemFamilyForJumps();
        ...
        {
            position_stream.open("position_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
            velocity_stream.open("velocity_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
            CD_stream.open("CD_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
            CL_stream.open("CL_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            update_solid_vel_pos(Y, U, Y_n, U_n, sum_Y, x_com_current, vol, bndry_equation_systems, *solid_equation_systems,
								current_time, 0.5 * dt, static_cast<void*>(&airfoil_params));
            Y_n = Y;
			U_n = U;
			sum_Y +=Y;
            ...
            update_solid_vel_pos(Y, U, Y_n, U_n, sum_Y, x_com_current, vol, bndry_equation_systems, *solid_equation_systems, 
								current_time + 0.5 * dt, 0.5 * dt, static_cast<void*>(&airfoil_params));
            Y_n = Y;
			U_n = U;
			sum_Y +=Y;
