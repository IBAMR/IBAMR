...
FeedbackForcer::FeedbackForcer(const double center1,
                               ...
                               const Pointer<SAMRAIPatchHierarchy> patch_hierarchy)
    : d_XC1(center1), d_XC2(center2), d_D(diameter), d_fluid_solver(fluid_solver), d_patch_hierarchy(patch_hierarchy)
{
    // intentionally blank
    return;
} // FeedbackForcer
                               ...
                               Pointer<SAMRAIPatchLevel> /*patch_level*/)
{
    ...
    Pointer<SAMRAICartesianGridGeometry> grid_geometry = d_patch_hierarchy->getGridGeometry();
    ...
    const double L = 2.0 * dx_finest[axis];
