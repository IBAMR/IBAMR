...
void update_solid_vel_pos(const double* params,
...
int main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        {
            position_stream.open("Displacement_2DOF_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
            l_max_stream.open("Lmax_2DOF_dt_" + std::to_string(double(DT)) +"_kappa_" + std::to_string(int(kappa_s)) +".curve", ios_base::out | ios_base::trunc);
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            update_solid_vel_pos(params, Y, U, x_com_current, bndry_equation_systems, *solid_equation_systems, current_time, 0.5 * dt);

            time_integrator->advanceHierarchy(dt);
            loop_time += dt;

            // Update the body configuration.
            update_solid_vel_pos(params, Y, U, x_com_current, bndry_equation_systems, *solid_equation_systems, current_time + 0.5 * dt, 0.5 * dt);
