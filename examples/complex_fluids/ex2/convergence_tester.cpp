...
main(int argc, char* argv[])
{
    ...
    Pointer<SAMRAICartesianGridGeometry> grid_geom = new SAMRAICartesianGridGeometry(
    ...
    Pointer<SAMRAISideVariable<double>> U_var = new SAMRAISideVariable<double>("INSStaggeredHierarchyIntegrator::U");
    ...
    Pointer<SAMRAICellVariable<double>> IDX_var = new SAMRAICellVariable<double>("Indicator");
    const int I_idx = var_db->registerVariableAndContext(IDX_var, current_ctx);

    Pointer<SAMRAICellVariable<double>> P_var = new SAMRAICellVariable<double>("INSStaggeredHierarchyIntegrator::P");
    ...
    Pointer<SAMRAICellVariable<double>> S_var =
    ...
    Pointer<SAMRAICellVariable<double>> Sxx_var = new SAMRAICellVariable<double>("Sxx");
    Pointer<SAMRAICellVariable<double>> Syy_var = new SAMRAICellVariable<double>("Syy");
    Pointer<SAMRAICellVariable<double>> Sxy_var = new SAMRAICellVariable<double>("Sxy");
    Pointer<SAMRAICellVariable<double>> U_draw_var = new SAMRAICellVariable<double>("U", NDIM);
         ...
         coarse_iteration_num += coarse_hier_dump_interval, fine_iteration_num += fine_hier_dump_interval)
    {
        ...
        Pointer<SAMRAIPatchHierarchy> coarse_patch_hierarchy =
        ...
        Pointer<SAMRAIPatchHierarchy> fine_patch_hierarchy = new SAMRAIPatchHierarchy(
        ...
        Pointer<SAMRAIPatchHierarchy> coarsened_fine_patch_hierarchy =
        ...
        {
            Pointer<SAMRAIPatchLevel> level = coarse_patch_hierarchy->getPatchLevel(ln);
        ...
        {
            Pointer<SAMRAIPatchLevel> level = fine_patch_hierarchy->getPatchLevel(ln);
            level->allocatePatchData(U_interp_idx, loop_time);
            level->allocatePatchData(P_interp_idx, loop_time);
            level->allocatePatchData(S_interp_idx, loop_time);
            level->allocatePatchData(U_scratch_idx, loop_time);
        ...
        {
            Pointer<SAMRAIPatchLevel> level = coarsened_fine_patch_hierarchy->getPatchLevel(ln);
        ...
        {
            Pointer<SAMRAIPatchLevel> coarser_level = coarse_patch_hierarchy->getPatchLevel(ln - 1);
            Pointer<SAMRAIPatchLevel> finer_level = coarse_patch_hierarchy->getPatchLevel(ln);
        ...
        {
            Pointer<SAMRAIPatchLevel> coarser_level = fine_patch_hierarchy->getPatchLevel(ln - 1);
            Pointer<SAMRAIPatchLevel> finer_level = fine_patch_hierarchy->getPatchLevel(ln);
        ...
        for (int ln = 0; ln <= fine_patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            Pointer<SAMRAIPatchLevel> dst_level = coarsened_fine_patch_hierarchy->getPatchLevel(ln);
            Pointer<SAMRAIPatchLevel> src_level = fine_patch_hierarchy->getPatchLevel(ln);

            Pointer<SAMRAICoarsenOperator> coarsen_op;
            for (SAMRAIPatchLevel::Iterator p(dst_level); p; p++)
            {
                Pointer<SAMRAIPatch> dst_patch = dst_level->getPatch(p());
                Pointer<SAMRAIPatch> src_patch = src_level->getPatch(p());
        ...
        for (int ln = 0; ln <= coarse_patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            Pointer<SAMRAIPatchLevel> dst_level = coarse_patch_hierarchy->getPatchLevel(ln);
            Pointer<SAMRAIPatchLevel> src_level = coarsened_fine_patch_hierarchy->getPatchLevel(ln);
        ...
        for (int ln = 0; ln <= coarse_patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAICellData<double>> U_draw_data = patch->getPatchData(U_draw_idx);
                Pointer<SAMRAISideData<double>> U_data = patch->getPatchData(U_interp_idx);
        ...
        for (int ln = 0; ln <= coarse_patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            {
                Pointer<SAMRAIPatch> patch = level->getPatch(p());
                Pointer<SAMRAICellData<double>> S_data = patch->getPatchData(S_interp_idx);
                Pointer<SAMRAICellData<double>> Sxx_data = patch->getPatchData(Sxx_idx);
                Pointer<SAMRAICellData<double>> Syy_data = patch->getPatchData(Syy_idx);
                Pointer<SAMRAICellData<double>> Sxy_data = patch->getPatchData(Sxy_idx);
        ...
        for (int ln = 0; ln <= coarse_patch_hierarchy->getFinestLevelNumber(); ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAICellData<double>> wgt_cc_data = patch->getPatchData(wgt_cc_idx);
                Pointer<SAMRAICellData<double>> I_data = patch->getPatchData(I_idx);
                Pointer<SAMRAISideData<double>> wgt_sc_data = patch->getPatchData(wgt_sc_idx);
                I_data->fillAll(1.0);
                for (SAMRAICellIterator i(box); i; i++)
                {
                    SAMRAICellIndex idx = *i;
