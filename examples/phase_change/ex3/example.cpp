...
namespace
{
class MeltingPCM : public PhaseChangeExamples::CoupledApplication
{
public:
    explicit MeltingPCM(Pointer<AppInitializer> app_initializer) : CoupledApplication(app_initializer)
    {
        useEnthalpy();
        registerLevelSet();
...
private:
    void initializeDiagnostics(const double loop_time) override
    {
        ...
        std::string file_name = d_input_db->getString("MASS_CONSERVATION_FILE_NAME");

        if (SAMRAI_MPI::getRank() == 0)
        {
            PhaseChangeExamples::open_diagnostic_file(d_mass_file, file_name);
        }
    }
    void postprocessStep(const double loop_time) override
    {
        ...
        if (SAMRAI_MPI::getRank() == 0)
        {
            d_mass_file << loop_time << "\t" << mass << std::endl;
        }
    }
