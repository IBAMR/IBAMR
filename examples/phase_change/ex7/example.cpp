                           ...
                           void* ctx)
{
    ...
    const int lf_extrap_new_idx =
        var_db->mapVariableAndContextToIndex(mask_surface_tension_force_ctx->lf_extrap_var,
                                             mask_surface_tension_force_ctx->adv_diff_hier_integrator->getNewContext());
    const int lf_extrap_scratch_idx = var_db->mapVariableAndContextToIndex(
        mask_surface_tension_force_ctx->lf_extrap_var,
        mask_surface_tension_force_ctx->adv_diff_hier_integrator->getScratchContext());
    ...
    std::vector<InterpolationTransactionComponent> lf_transaction_comps(3);
    ...
    lf_transaction_comps[2] = InterpolationTransactionComponent(lf_extrap_scratch_idx,
                                                                lf_extrap_new_idx,
                                                                "CONSERVATIVE_LINEAR_REFINE",
                                                                false,
                                                                "CONSERVATIVE_COARSEN",
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            Pointer<CellData<NDIM, double> > lf_extrap_data = patch->getPatchData(lf_extrap_scratch_idx);
            ...
            for (unsigned int axis = 0; axis < NDIM; axis++)
            {
                for (Box<NDIM>::Iterator it(SideGeometry<NDIM>::toSideBox(patch_box, axis)); it; it++)
                {
                    ...
                    const double lf_extrap_sc_value =
                        0.5 * ((*lf_extrap_data)(si.toCell(0)) + (*lf_extrap_data)(si.toCell(1)));
                    ...
                    //         {
                    multiplier_term *= lf_extrap_sc_value;
                    // }
...
main(int argc, char* argv[])
{
	...
	if (input_db->getBoolWithDefault("FIX_MASS_LOSS", false))
        ...
        // set registerLiquidFractionVariableForExtrapolation.
        ...
        mask_surface_tension_force_ctx.lf_extrap_var = lf_extrap_var;
