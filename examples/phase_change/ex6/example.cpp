...
namespace
{
class AllenCahnMelting : public PhaseChangeExamples::CoupledApplication
{
public:
    explicit AllenCahnMelting(Pointer<AppInitializer> app_initializer) : CoupledApplication(app_initializer)
    {
        useAllenCahn();
        registerLevelSet();
...
{
    return std::make_unique<AllenCahnMelting>(app_initializer);
}
} // namespace
