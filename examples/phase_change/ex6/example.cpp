...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<AdvDiffHierarchyIntegrator> adv_diff_integrator = new AllenCahnHierarchyIntegrator(
        ...
        LSLocateInterface* ptr_LSLocateInterface = new LSLocateInterface(
            "LSLocateInterface", adv_diff_integrator, ls_var, initial_horizontal_interface_position, bubbles_position);
        ...
        Pointer<AllenCahnHierarchyIntegrator> ac_hier_integrator = adv_diff_integrator;
        ...
        ac_hier_integrator->setAdvectionVelocity(time_integrator->getAdvectionVelocityVariable());
        ...
        Pointer<CartGridFunction> ls_init =
            new LevelSetInitialCondition("ls_init", initial_horizontal_interface_position, bubbles_position);
        ...
        if (!(periodic_shift.min() > 0) && input_db->keyExists("DensityBcCoefs"))
        {
            ...
            ac_hier_integrator->registerMassDensityBoundaryConditions(rho_bc_coef);
        }
        ...
        ac_hier_integrator->registerResetDiffusionCoefficientFcn(&callSetLiquidSolidGasConductivityCallbackFunction,
                                                                 static_cast<void*>(ptr_SetFluidProperties));

        ac_hier_integrator->registerResetSpecificHeatFcn(&callSetLiquidSolidGasSpecificHeatCallbackFunction,
                                                         static_cast<void*>(ptr_SetFluidProperties));

        ac_hier_integrator->registerResetFluidDensityFcn(&callSetLiquidSolidGasDensityCallbackFunction,
                                                         static_cast<void*>(ptr_SetFluidProperties));
