...
/////////////////////////////// STATIC ///////////////////////////////////////
void
callSetLiquidSolidDensityCallbackFunction(int rho_idx,
...
void
callSetLiquidSolidSpecificHeatCallbackFunction(int Cp_idx,
...
void
callSetLiquidSolidConductivityCallbackFunction(int D_idx,
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (Box<NDIM>::Iterator it(patch_box); it; it++)
                {
                    ...
                    (*rho_data)(ci) = d_rho_solid + (d_rho_liquid - d_rho_solid) * liquid_fraction;
                }
            }
        }
    }
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (int axis = 0; axis < NDIM; ++axis)
                {
                    for (Box<NDIM>::Iterator it(SideGeometry<NDIM>::toSideBox(patch_box, axis)); it; it++)
                    {
                        ...
                        (*rho_data)(si) = d_rho_solid + (d_rho_liquid - d_rho_solid) * liquid_fraction;
                    }
                }
            }
        }
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator it(patch_box); it; it++)
            {
                ...
                const double liquid_fraction = (*lf_data)(ci);
                (*D_data)(ci) = d_kappa_solid + (d_kappa_liquid - d_kappa_solid) * liquid_fraction;
            }
        }
    }
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator it(patch_box); it; it++)
            {
                ...
                const double liquid_fraction = (*lf_data)(ci);
                (*Cp_data)(ci) = d_Cp_solid + (d_Cp_liquid - d_Cp_solid) * liquid_fraction;
            }
        }
    }
