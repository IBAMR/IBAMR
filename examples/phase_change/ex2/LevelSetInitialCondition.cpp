...
LevelSetInitialCondition::LevelSetInitialCondition(const std::string& object_name, const double bubble_radius)
    : d_object_name(object_name), d_radius(bubble_radius)
{
    // intentionally blank
    return;
                                         ...
                                         Pointer<PatchLevel<NDIM> > /*patch_level*/)
{
    ...
    if (initial_time)
    {
        ...
        for (Box<NDIM>::Iterator it(patch_box); it; it++)
        {
            ...
            const double distance = std::sqrt(std::pow(coord[0] - 0.5, 2.0) + std::pow(coord[1] - 0.5, 2.0));
            (*D_data)(ci) = d_radius - distance;
        }
    }
    return;
} // setDataOnPatch
