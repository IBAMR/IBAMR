...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
        const int rho_cc_idx =
            var_db->mapVariableAndContextToIndex(rho_cc_var, adv_diff_integrator->getCurrentContext());
        const int rho_sc_idx = time_integrator->getLinearOperatorRhoPatchDataIndex();
        ...
        SAMRAI::tbox::Pointer<SAMRAI::pdat::CellVariable<NDIM, double> > rho_cc_interp_var;
        rho_cc_interp_var = new CellVariable<NDIM, double>("rho_cc_interp", NDIM);
        int rho_cc_interp_idx =
            var_db->registerVariableAndContext(rho_cc_interp_var, var_db->getContext("rho_cc_interp"), 0);
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            if (!level->checkAllocated(rho_cc_interp_idx))
            {
                patch_hierarchy->getPatchLevel(ln)->allocatePatchData(rho_cc_interp_idx, loop_time);
            }
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            hier_math_ops.interp(
                rho_cc_interp_idx, rho_cc_interp_var, rho_sc_idx, rho_sc_var, nullptr, loop_time, true);
            double mass = 0;
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                Pointer<PatchLevel<NDIM> > level = patch_hierarchy->getPatchLevel(ln);
                for (PatchLevel<NDIM>::Iterator p(level); p; p++)
                {
                    ...
                    Pointer<CellData<NDIM, double> > rho_cc_interp_data = patch->getPatchData(rho_cc_interp_idx);
            ...
            hier_cc_data_ops->multiply(total_enthalpy_idx, rho_cc_idx, h_idx);
            const double total_enthalpy = hier_cc_data_ops->integral(total_enthalpy_idx, wgt_cc_idx);
            const double mass_sc = hier_sc_data_ops->integral(rho_sc_idx, wgt_sc_idx);
            ...
            if (SAMRAI_MPI::getRank() == 0)
            {
                ...
                vol_stream << loop_time << "\t" << H_integrals[0] << "\t" << H_integrals[1] << "\t"
                           << level_set_fixer.getLagrangeMultiplier() << "\t" << mass << "\t" << mass_sc << "\t"
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            patch_hierarchy->getPatchLevel(ln)->deallocatePatchData(rho_cc_interp_idx);
        ...
        var_db->removePatchDataIndex(rho_cc_interp_idx);
