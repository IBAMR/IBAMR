...
/////////////////////////////// STATIC ///////////////////////////////////////
void
callSetLiquidGasDensityCallbackFunction(int rho_idx,
...
void
callSetLiquidGasSpecificHeatCallbackFunction(int Cp_idx,
...
void
callSetLiquidGasConductivityCallbackFunction(int D_idx,
...
void
callSetLiquidGasViscosityCallbackFunction(int mu_idx,
                                        ...
                                        const double new_time)
{
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (Box<NDIM>::Iterator it(patch_box); it; it++)
                {
                    ...
                    (*rho_data)(ci) = d_rho_gas + (d_rho_liquid - d_rho_gas) * heaviside;
                }
            }
        }
    }
    ...
    if (rho_sc_var)
    {
        ...
        std::vector<InterpolationTransactionComponent> interpolation_transaction(1);
        interpolation_transaction[0] = InterpolationTransactionComponent(H_scratch_idx,
                                                                         H_idx,
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                for (int axis = 0; axis < NDIM; ++axis)
                {
                    for (Box<NDIM>::Iterator it(SideGeometry<NDIM>::toSideBox(patch_box, axis)); it; it++)
                    {
                        ...
                        (*rho_data)(si) = d_rho_gas + (d_rho_liquid - d_rho_gas) * heaviside;
                    }
                }
            }
        }
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator it(patch_box); it; it++)
            {
                ...
                const double heaviside = (*H_data)(ci);
                (*D_data)(ci) = d_kappa_gas + (d_kappa_liquid - d_kappa_gas) * heaviside;
            }
        }
    }
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator it(patch_box); it; it++)
            {
                ...
                const double heaviside = (*H_data)(ci);
                (*Cp_data)(ci) = d_Cp_gas + (d_Cp_liquid - d_Cp_gas) * heaviside;
            }
        }
    }
    ...
    for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
    {
        ...
        for (PatchLevel<NDIM>::Iterator p(level); p; p++)
        {
            ...
            for (Box<NDIM>::Iterator it(patch_box); it; it++)
            {
                ...
                const double heaviside = (*H_data)(ci);
                (*mu_data)(ci) = d_mu_gas + (d_mu_liquid - d_mu_gas) * heaviside;
            }
        }
    }
