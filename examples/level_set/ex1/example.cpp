    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<AdvectorPredictorCorrectorHyperbolicPatchOps> hyp_patch_ops =
            new AdvectorPredictorCorrectorHyperbolicPatchOps(
                ...
                explicit_predictor,
                grid_geometry);
        Pointer<SAMRAIHyperbolicLevelIntegrator> hyp_level_integrator =
        ...
        Pointer<SAMRAIGriddingAlgorithm> gridding_algorithm =
            new SAMRAIGriddingAlgorithm("GriddingAlgorithm",
                                        ...
                                        box_generator,
                                        load_balancer);
        Pointer<SAMRAITimeRefinementIntegrator> time_integrator =
        ...
        // Heaviside
        Pointer<SAMRAICellVariable<double>> H_var = new SAMRAICellVariable<double>("H");
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                ...
                Pointer<SAMRAICellData<double>> Q_scratch_data = patch->getPatchData(Q_scratch_idx);
        ...
        SAMRAIHierarchyCellDataOpsReal<double> cc_data_ops(patch_hierarchy, coarsest_ln, finest_ln);
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (SAMRAIPatchLevel::Iterator p(level); p; p++)
            {
                Pointer<SAMRAIPatch> patch = level->getPatch(p());
                const SAMRAIBox& patch_box = patch->getBox();
                Pointer<SAMRAICellData<double>> D_data = patch->getPatchData(Q_scratch_idx);
                Pointer<SAMRAICellData<double>> E_data = patch->getPatchData(E_idx);
                Pointer<SAMRAICellData<double>> W_data = patch->getPatchData(wgt_cc_idx);
                for (SAMRAIBox::Iterator it(patch_box); it; it++)
                {
                    SAMRAICellIndex ci(it());
