...
reset_solid_level_set_callback_fcn(double current_time, double new_time, int /*cycle_num*/, void* ctx)
{
    ...
    Pointer<SAMRAIPatchHierarchy> patch_hier = resetter->adv_diff_integrator->getPatchHierarchy();
    ...
    {
        Pointer<SAMRAIPatchLevel> patch_level = patch_hier->getPatchLevel(ln);
        for (SAMRAIPatchLevel::Iterator p(patch_level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> ls_solid_data = patch->getPatchData(ls_solid_idx);
...
void
calculate_distance_analytically(Pointer<SAMRAIPatchHierarchy> patch_hierarchy, int E_idx)
{
    ...
    for (int ln = coarsest_ln; ln <= hier_finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> E_data = patch->getPatchData(E_idx);
            for (SAMRAIBox::Iterator it(patch_box); it; it++)
            {
...
void
calculate_error_near_band(Pointer<SAMRAIPatchHierarchy> patch_hierarchy,
                          int E_idx,
                          int d_idx,
                          int W_idx,
                          double& E_interface,
...

{
    ...
    for (int ln = coarsest_ln; ln <= hier_finest_ln; ++ln)
    {
        ...
        for (SAMRAIPatchLevel::Iterator p(level); p; p++)
        {
            ...
            Pointer<SAMRAICellData<double>> E_data = patch->getPatchData(E_idx);
            Pointer<SAMRAICellData<double>> W_data = patch->getPatchData(W_idx);
            for (SAMRAIBox::Iterator it(patch_box); it; it++)
            {
                SAMRAICellIndex ci(it());
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        Pointer<SAMRAICellVariable<double>> phi_var_solid = new SAMRAICellVariable<double>(ls_name_solid);
        ...
        Pointer<SAMRAICellVariable<double>> phi_var_gas = new SAMRAICellVariable<double>(ls_name_gas);
        ...
        SAMRAIVariableDatabase* var_db = SAMRAIVariableDatabase::getDatabase();
        Pointer<SAMRAICellVariable<double>> n_var = new SAMRAICellVariable<double>("num_elements", 1);
        Pointer<SAMRAICellVariable<double>> d_var = new SAMRAICellVariable<double>("distance", 1);
        ...
        // Compute the error.
        Pointer<SAMRAICellVariable<double>> E_var = new SAMRAICellVariable<double>("E");
