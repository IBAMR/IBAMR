...
void
update_target_points(const Pointer<SAMRAIPatchHierarchy>& hierarchy,
...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        if (simulate_dye_concentration_field)
        {
            ...
            Pointer<SAMRAIFaceVariable<double>> u_adv_var = navier_stokes_integrator->getAdvectionVelocityVariable();
            adv_diff_integrator->setAdvectionVelocity(Q_var, u_adv_var);
            std::unique_ptr<SAMRAIRobinBcCoefStrategy> Q_bc_coefs = nullptr;
            if (periodic_shift.min() == 0)
            {
