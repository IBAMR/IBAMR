    ...
    if (rho_cc_var)
    {
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            ...
            for (PatchLevel<NDIM>::Iterator p(level); p; p++)
            {
                ...
                double alpha = d_num_solid_interface_cells * std::pow(vol_cell, 1.0 / (double)NDIM);
                double beta = d_num_gas_interface_cells * std::pow(vol_cell, 1.0 / (double)NDIM);
