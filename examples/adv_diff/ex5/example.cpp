...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<CellVariableNd<double> > phi_left_solid_var =
            make_samrai_shared<CellVariableNd<double> >("level_set_left_solid");
        ...
        SAMRAIPointer<CartGridFunction> phi_left_solid_init =
            make_samrai_shared<LevelSetInitialCondition>("ls_left_solid_init", grid_geometry, interface_left, true);
        ...
        SAMRAIPointer<CellVariableNd<double> > phi_right_solid_var =
        ...
        SAMRAIPointer<CartGridFunction> phi_right_solid_init =
            make_samrai_shared<LevelSetInitialCondition>("ls_right_solid_init", grid_geometry, interface_right);
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                ...
                for (PatchLevelNd::Iterator p(level); p; p++)
                {
                    ...
                    SAMRAIPointer<CellDataNd<double> > q_data = patch->getPatchData(q_idx);
                    SAMRAIPointer<CellDataNd<double> > q_ex_data = patch->getPatchData(q_ex_cloned_idx);
