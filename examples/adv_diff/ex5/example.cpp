...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> phi_left_solid_var =
            new SAMRAICellVariable<double>("level_set_left_solid");
        ...
        SAMRAIPointer<CartGridFunction> phi_left_solid_init =
        ...
        SAMRAIPointer<SAMRAICellVariable<double>> phi_right_solid_var = new SAMRAICellVariable<double>(ls_name);
        ...
        SAMRAIPointer<CartGridFunction> phi_right_solid_init =
        ...
        {
            SAMRAIPointer<SAMRAICellVariable<double>> F_var = new SAMRAICellVariable<double>("F");
        ...
        SAMRAIPointer<CartGridFunction> q_ex = new muParserCartGridFunction(
        ...
        while (!IBTK::rel_equal_eps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
            {
                ...
                for (SAMRAIPatchLevel::Iterator p(level); p; p++)
                {
                    ...
                    SAMRAIPointer<SAMRAICellData<double>> q_data = patch->getPatchData(q_idx);
                    SAMRAIPointer<SAMRAICellData<double>> q_ex_data = patch->getPatchData(q_ex_cloned_idx);

                    for (SAMRAIBox::Iterator it(patch_box); it; it++)
                    {
                        SAMRAICellIndex ci(it());
                        SAMRAICellIndex ci_1 = ci;
                        double chi = 0.0;
                        for (const auto& phi_solid_var : ls_vars)
                        {
                            const int phi_idx = var_db->mapVariableAndContextToIndex(
