...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        time_integrator->registerTransportedQuantity(T_var, true);
        time_integrator->setDiffusionCoefficient(T_var, input_db->getDouble("KAPPA"));
        ...
        if (input_db->keyExists("TemperatureInitialConditions"))
        {
            ...
            time_integrator->setInitialConditions(T_var, T_init);
        }
        ...
        if (!(periodic_shift.min() > 0) && input_db->keyExists("TemperatureBcCoefs"))
        {
            T_bc_coef = new muParserRobinBcCoefs(
                "Temperature_bc_coef", app_initializer->getComponentDatabase("TemperatureBcCoefs"), grid_geometry);
        }
        time_integrator->setPhysicalBcCoef(T_var, T_bc_coef);
        ...
        if (input_db->keyExists("TemperatureForcingFunction"))
        {
            Pointer<CellVariable<NDIM, double> > F_var = new CellVariable<NDIM, double>("F", 1);
            Pointer<CartGridFunction> temp_forcing_fcn = new muParserCartGridFunction(
                "temp_forcing_fcn", app_initializer->getComponentDatabase("TemperatureForcingFunction"), grid_geometry);
            time_integrator->registerSourceTerm(F_var, true);
            time_integrator->setSourceTermFunction(F_var, temp_forcing_fcn);
            time_integrator->setSourceTerm(T_var, F_var);
        }

        Pointer<CartGridFunction> T_ex = new muParserCartGridFunction(
            "T_ex", app_initializer->getComponentDatabase("TemperatureExactSolutions"), grid_geometry);
        ...
        // VariableDatabase<NDIM>* var_db = VariableDatabase<NDIM>::getDatabase();
        const int T_idx = var_db->mapVariableAndContextToIndex(T_var, time_integrator->getCurrentContext());
        const int T_ex_cloned_idx = var_db->registerClonedPatchDataIndex(T_var, T_idx);
        const int T_err_cloned_idx = var_db->registerClonedPatchDataIndex(T_var, T_idx);
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            patch_hierarchy->getPatchLevel(ln)->allocatePatchData(T_ex_cloned_idx, loop_time);
            patch_hierarchy->getPatchLevel(ln)->allocatePatchData(T_err_cloned_idx, loop_time);
        ...
        visit_data_writer->registerPlotQuantity("T_exact", "SCALAR", T_ex_cloned_idx);
        visit_data_writer->registerPlotQuantity("T_error", "SCALAR", T_err_cloned_idx);
        ...
        while (!MathUtilities<double>::equalEps(loop_time, loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            T_ex->setDataOnPatchHierarchy(T_ex_cloned_idx, T_var, patch_hierarchy, loop_time);
            hier_cc_data_ops.subtract(T_err_cloned_idx, T_idx, T_ex_cloned_idx);

            pout << "Error in T at time " << loop_time << ":\n"
                 << "  L1-norm:  " << std::setprecision(10) << hier_cc_data_ops.L1Norm(T_err_cloned_idx, phi_cloned_idx)
                 << "\n"
                 << "  L2-norm:  " << hier_cc_data_ops.L2Norm(T_err_cloned_idx, phi_cloned_idx) << "\n"
                 << "  max-norm: " << hier_cc_data_ops.maxNorm(T_err_cloned_idx, phi_cloned_idx) << "\n";

            // Masking the variables for visualization.
            hier_cc_data_ops.divide(phi_cloned_idx, phi_cloned_idx, wgt_cc_idx);
            hier_cc_data_ops.multiply(T_err_cloned_idx, phi_cloned_idx, T_err_cloned_idx);
            hier_cc_data_ops.multiply(T_ex_cloned_idx, phi_cloned_idx, T_ex_cloned_idx);
            hier_cc_data_ops.multiply(T_idx, phi_cloned_idx, T_idx);
        ...
        for (int ln = coarsest_ln; ln <= finest_ln; ++ln)
        {
            patch_hierarchy->getPatchLevel(ln)->deallocatePatchData(T_ex_cloned_idx);
            patch_hierarchy->getPatchLevel(ln)->deallocatePatchData(T_err_cloned_idx);
