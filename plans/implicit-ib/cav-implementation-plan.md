...
- **Active branch/worktrees:** the plan-only base remains frozen at `1d209b7f11be4c9ebd221c68cefbfcedb95ea3df` on `codex/implicit-ib-cav-0-recovery-plan` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; committed `cav-1a` is `c7940ad4211cb85c0958660f30e0e923a91e7ddb` in `/tmp/ibamr-cav-durable-1a`; committed `cav-1b` is `efab0787cbdfce8f6672cc1aca1b75ca57cdc08b` in `/tmp/ibamr-cav-durable-1b`; and the active durable unit is `codex/implicit-ib-cav-1c-matrix-live-kernel-consistency` in `/tmp/ibamr-cav-durable-1c`, stacked exactly on `efab0787c`. The disposable reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-1c` Debug build compiles both dimensional focused executables and all affected `tests-IBTK`, `tests-IB`, `tests-spread`, and `tests-interpolate` groups. The exact focused selector passes **2/2 twice** in approximately 57 s and 56 s; every one of the 13 supported choices detects formula and order controls, with maximum errors `1.30e-13` (2D) and `7.23e-14` (3D). The complete serial interpolation/spreading scope passes **126/126** in approximately 70 s. The serial-compatible broad `IB/` scope passes **14/14** in approximately 82 s. The exact broad `-R IB/` result is **14/18** in approximately 77 s, with all four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI TCP-listener restriction. These are implementation validations, not independent audit results.
- **Current work:** `cav-1c` contains the Fortran-authoritative matrix callback ports and their same-unit native tests. Production IB4 uses the live optimized recurrence; no `USER_DEFINED` or generic pointwise production alternative is present. The only live-kernel edit fixes the independently reproduced 3D IB5 stale `ic0` loop index. No full matrix, velocity block, transpose, or persistent subdomain-index representation is copied. The plan and expected fixtures now record the validated behavior; no auditor, audit coordinator, or synthesizer has been launched.
...
- **Exactly one next action:** create `cav-2-patch-construction-audit` from the committed `cav-1c` tip and recover only pressure-seeded `E_h` RELAXED/STRICT construction and order with same-unit tests.
