...
- **Active branch/worktrees:** the plan-only branch is `codex/implicit-ib-cav-0-recovery-plan` at `127dc0d144c4f6c31985eddef236cd76ccc7b99b` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; it already contains the production-IB4/no-`USER_DEFINED` decision and needs no backward rewrite. The durable stack tips through `cav-4` remain unchanged, with `cav-4` at `34987873ee6043149c8ed969a71b280a8bd26510`. The active branch is `codex/implicit-ib-cav-5-smoother-parity` in `/tmp/ibamr-cav-durable-5`; its feature commit is `a6c94488afb02ffa169470bace8d5a22a09163ae`, and its current plan/evidence tip is the commit containing this stopping-point update. The exact-SHA disposable exporter checkout is detached at that feature commit in `/tmp/ibamr-cav5-parity`; the disposable integration reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-5` Debug build compiles both dimensional libraries and the complete `tests-IB` target. The exact focused selector passes **3/3 twice in 2.14 s**; the live pressure-CAV case and all three pre-existing solver-component cases pass **4/4**. The serial broad IB scope passes **15/15**; the exact `-R IB/` scope reaches **15/19**, with only the four rank-4 cases blocked before IBAMR by the Open MPI/PRTE listener restriction. From the exact feature SHA, freshly generated live and clean-oracle exports at `K=1`, `10^2`, and `10^4` have exact mapped seed order, patch membership/order, and operator sparsity. Candidate/oracle common-arithmetic corrections and fresh residuals are bitwise equal on both independently exported operator sources; live-production replay diagnostics meet the fixed relative targets, and reverse traversal is detected. These are implementation validations, not independent audit results.
- **Current work:** M4 implementation validation is complete. Production pressure-seeded CAV constructs from the borrowed full-space FAC-owned `E_h`, applies one forward unrestricted multiplicative sweep through the full original Stokes-IB operator, recomputes the original residual through the sweep, and applies the zero-mean gauge only to the final pressure state. No full/block/transpose/update matrix copy, persistent subdomain numbering, reverse production option, flat additive-restrict mislabeling, `USER_DEFINED` kernel path, FAC recursion change, or RAS behavior is introduced. RAS has not started. No auditor, audit coordinator, or synthesizer has been launched, and no independent gate is complete.
- **Blockers:** no implementation blocker. Full review readiness still requires the four rank-4 cases to run outside this TCP-restricted environment, and the exact M4 candidate must later receive all six independent lane audits plus synthesis in separate clean tasks.
- **Exactly one next action:** create `cav-6a-fac-stage-observability` from the committed `cav-5` plan/evidence tip and recover only the narrow passive FAC/transfer/coarse-stage snapshots and lifecycle/regrid checks needed for first-mismatch localization, without changing the production V-cycle or starting RAS.
