...
2. Every semantic claim has a focused native test in the same review unit.
...
| Live operator/block/DOF/gauge snapshot API | Historical 8 `ad3dcdb13` and trimmed intent at `c9ce85d2f` | `cav-1a-live-operator-audit-schema` | Focused const live-snapshot/provenance test | Problem/operator | Proposed after M0 |
...
Reviewability means one coherent claim, focused same-unit tests, no hidden later dependency, refactor separate from behavior, construction separate from application, and no canonical-ledger regression.
...
- **Active branch/worktrees:** the authoritative plan remains on `codex/implicit-ib-cav-0-recovery-plan` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; the disposable implementation remains on `codex/implicit-ib-cav-integration-reconstruction` in `/tmp/ibamr-cav-integration-reconstruction`. Both derive from exact foundation `0c8447e3593113a8b62f94df9482c60d896f0d3b`. The clean oracle worktree remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** disposable schema `cav-common-arithmetic-multiplicative-v3` passes at `K=1`, `1e2`, and `1e4`. Candidate and oracle results are exactly equal in the common runtime for ordered patch structure, one multiplicative sweep correction/fresh residual, two-level V-cycle correction/fresh residual, FGMRES solution/fresh residual, and relative iteration history on both independently exported operator/RHS inputs. Separate live IBAMR runs converge in exactly 9, 8, and 9 iterations with original relative residuals `3.0556e-11`, `9.1779e-11`, and `1.2450e-11`; their live matrix-free-versus-`SAJ` IB-action relative defects are `2.9759e-15`, `3.9531e-15`, and `4.0806e-15`. The live scalar-summary errors against the oracle are `1.97e-16`, `2.12e-14`, and `2.30e-12`, and normalized iteration-history errors are `2.26e-17`, `4.41e-16`, and `1.14e-15`, all within the fixed table. Raw cross-runtime final-residual differences remain explicit diagnostics (`1.78e-14`, `1.34e-12`, `1.13e-10`) and do not replace either mandatory track. Fresh focused native validation is green for both all-kernel programs (`test_failures = 0`; maxima `1.30e-13` in 2D and `7.23e-14` in 3D) and for all 17 applicable serial implicit-IB/local-backend cases. The excluded two-rank expected-error fixture cannot reach its intended assertion because this sandbox rejects the MPI TCP listener. Harness SHA-256 is `9d6b1800b53e278786ed3430bbcfa2190de848fa5e503deab7302d82dbeeb49f`; report hashes for increasing `K` are `4912a3ad04045a77617468ce8e5c9af436040f700a69da216564af601841e2b7`, `826147134cf4100815fe0f47f4dd44b8c9cc3169df56707eed331df24c1b25f2`, and `7f6785c1b8a5f8af73223006ee3fd6bf7da1e636e9fc2146800e825278f12e4a`. Generated evidence remains under `/tmp`. These are implementation validations, not independent audit results.
- **Current work:** the disposable current-master multiplicative reconstruction now reproduces the pinned sandbox under the selected two-track numerical contract. Original Fortran interaction kernels govern every matrix callback, and the correct matrix-free implementations remain unchanged except for the independently reproduced 3D IB5 stale-index bug. Directly invoking full Fortran kernels to construct rows and introducing a shared Fortran weight-vector ABI were both rejected for this recovery because neither actually shares the optimized live path without extra scratch/copy/call overhead or hot-path refactoring. The implementation uses canonical global velocity-pressure numbering outside local factor boundaries, borrows the live operator, and retains only necessary local overwritten factors, pivots, patch references, and reusable work vectors; it adds no full/operator/update-matrix copies. The proposed small-review-unit decomposition is recorded in the recovery manifest. The user has now authorized local durable review-stack branches and commits, but not pushes, PRs, audits, or synthesis. RAS has not started, and no auditor, audit coordinator, or synthesizer has been launched.
- **Blockers:** none for the first durable review unit. The MPI-restricted two-rank expected-error fixture is not validated in this sandbox, but it does not block the serial observability/schema unit.
- **Exactly one next action:** freeze this authoritative `cav-0-recovery-plan` tip, then create a clean durable `cav-1a-live-operator-audit-schema` worktree from it and port only that review unit with its focused native tests.
