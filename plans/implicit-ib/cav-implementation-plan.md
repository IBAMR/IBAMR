...
- **Active branch/worktrees:** the plan-only base remains frozen at `1d209b7f11be4c9ebd221c68cefbfcedb95ea3df` on `codex/implicit-ib-cav-0-recovery-plan` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; committed durable tips are `c7940ad4211cb85c0958660f30e0e923a91e7ddb` (`cav-1a`), `efab0787cbdfce8f6672cc1aca1b75ca57cdc08b` (`cav-1b`), and `6b5f6054744df7cc348cc2f1efdea2663c7b236a` (`cav-1c`). The active branch is `codex/implicit-ib-cav-2-patch-construction-audit` in `/tmp/ibamr-cav-durable-2`; its feature commit is `ed1217120ab59b0dda7aca152403959f36096b23`, and the current plan/evidence tip is the commit containing this stopping-point update. The disposable reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-2` Debug build compiles the new 2D/3D construction cases and the reused live multilevel IB case. The exact focused selector passes **3/3 twice** in 3.68 s; the live `SAJ` case finds 1024 pressure patches with 240 enlarged. The related legacy/map/ASM selector passes **8/8** in 4.18 s. The serial-compatible broad `IB/` scope passes **14/14** in approximately 76 s. The exact broad `-R IB/` result is **14/18** in approximately 81 s, with all four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI TCP-listener restriction. These are implementation validations, not independent audit results.
- **Current work:** M2 is complete as implementation validation. `cav-2` supplies only pressure-cell-seeded RELAXED/STRICT patch construction and deterministic seed/patch order in the existing full global velocity-pressure DOF space. It borrows and scans the full-space `E_h` once and adds no matrix, velocity-block, transpose, local-matrix, ownership/restriction, solver/FAC-selection, or correction-application path. The historical velocity-component-seeded construction is unchanged and remains explicitly distinct. Production IB4 continues to use the optimized live Fortran recurrence; no `USER_DEFINED` comparison path exists. No auditor, audit coordinator, or synthesizer has been launched.
...
- **Exactly one next action:** create `cav-3a-blas-lapack-lu-backend` from the committed `cav-2` plan/evidence tip and recover only the minimal real-scalar serial LU backend, workspace/failure lifecycle, and same-unit native tests without changing patch construction or correction composition.
