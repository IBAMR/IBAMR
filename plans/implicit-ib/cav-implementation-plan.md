...
- **Current work:** the disposable current-master multiplicative reconstruction remains green under the selected two-track contract. Local durable branches/commits are authorized, but pushes, PRs, audits, and synthesis are not. The plan/common protocol/lane/synthesis instructions now make same-unit focused native tests, normal repository-root `attest` discovery/execution, CI budgets and measured runtime, executable/fixture reuse, meaningful sensitivity, narrow audit hooks, and IBAMR comment/style conformance binding implementation and audit criteria. `cav-1a` is implementing only a nonowning live operator-state view plus a focused case in the existing level-solver executable; no serializer, patch-construction change, backend, FAC observer, or solver behavior is included. RAS has not started, and no auditor, audit coordinator, or synthesizer has been launched.
...
- **Exactly one next action:** commit the forward-only `cav-0` test/style/audit-contract update, fast-forward the uncommitted `cav-1a` worktree to that tip, then build and run its `attest`-discoverable focused live-operator-state fixture before deciding whether the review unit is ready to commit.
