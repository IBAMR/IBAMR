...
- [ ] **M3 — Local backend recovery:** `cav-3a` now supplies the minimal serial real-scalar LU backend, focused lifecycle/failure tests, and selected live local-matrix/solve observation at exact feature commit `4195f0718`; `cav-3b` robust modes and the completed M3 evidence remain open.
...
- **Active branch/worktrees:** the plan-only branch is `codex/implicit-ib-cav-0-recovery-plan` at `127dc0d144c4f6c31985eddef236cd76ccc7b99b` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`. Committed durable tips are `c7940ad4211cb85c0958660f30e0e923a91e7ddb` (`cav-1a`), `efab0787cbdfce8f6672cc1aca1b75ca57cdc08b` (`cav-1b`), `6b5f6054744df7cc348cc2f1efdea2663c7b236a` (`cav-1c`), and `deb566b5e2d3e9a49bcd2449a61142bdbb503a9f` (`cav-2` plan/evidence). The active branch is `codex/implicit-ib-cav-3a-blas-lapack-lu-backend` in `/tmp/ibamr-cav-durable-3a`; its feature commit is `4195f0718476ae020f76d4dbb03dcb81f82b7341`, and the current plan/evidence tip is the commit containing this stopping-point update. The disposable reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-3a` Debug build compiles the complete navier-stokes test target and the backend in both 2D and 3D. The exact focused selector discovers the multiplicative observer/reinitialize, additive-restrict, and singular expected-failure fixtures and passes **3/3 twice** in 2.7 s and 1.6 s. The complete serial shell-semantics executable scope passes **16/16** in 10.2 s. A disposable residual-update-sign mutation makes the positive multiplicative fixture fail; the restored control passes. The serial-compatible broad `IB/` scope passes **14/14** in approximately 84 s. The exact broad `-R IB/` result is **14/18** in approximately 81 s, with all four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI/PRTE TCP-listener restriction. These are implementation validations, not independent audit results.
- **Current work:** the minimal `cav-3a` LU review unit is complete as implementation validation. It keeps all persistent algebraic state in the full global velocity-pressure numbering, borrows the live full Stokes operator, stores one overwritten local factor buffer plus pivots and reusable workspaces per patch, and uses the live matrix for multiplicative original-residual evolution. It retains no copied full, velocity-block, transpose, unfactored, or residual-update matrix. The observer is selected-only and transient; default application constructs no diagnostic matrix. Production IB4 remains the optimized live Fortran-authoritative path and no `USER_DEFINED` path exists. Robust BLAS/LAPACK modes remain `cav-3b`; global multiplicative semantics remain `cav-4`; RAS has not started. No auditor, audit coordinator, or synthesizer has been launched.
- **Blockers:** no implementation blocker. Full review readiness still requires the four rank-4 broad cases to be rerun outside this TCP-restricted sandbox; independent audit gates remain unlaunched and incomplete.
- **Exactly one next action:** create `cav-3b-blas-lapack-robust-modes` from the committed `cav-3a` plan/evidence tip and recover only the justified robust local-solver modes with same-unit native tests, without changing patch construction or correction traversal.
