...
- [ ] **M4 — Global multiplicative CAV:** durable `cav-4` now has exact 64-patch order/current-original-residual/local-RHS/correction/gauge semantics green for every production BLAS/LAPACK mode at feature commit `cc2b68af4`; `cav-5` one-sweep sandbox parity remains open, so the milestone is not complete and no independent gate has been launched.
...
- **Active branch/worktrees:** the plan-only branch is `codex/implicit-ib-cav-0-recovery-plan` at `127dc0d144c4f6c31985eddef236cd76ccc7b99b` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; it already contains the production-IB4/no-`USER_DEFINED` decision and needs no backward rewrite for this later implementation evidence. Committed durable tips are `c7940ad4211cb85c0958660f30e0e923a91e7ddb` (`cav-1a`), `efab0787cbdfce8f6672cc1aca1b75ca57cdc08b` (`cav-1b`), `6b5f6054744df7cc348cc2f1efdea2663c7b236a` (`cav-1c`), `deb566b5e2d3e9a49bcd2449a61142bdbb503a9f` (`cav-2` plan/evidence), `eaa3ec43d36b26a47d7d3bae30e30ac0dee2ec76` (`cav-3a` plan/evidence), and `e03e1097333cf4bb6cb83a42812bfa230c868c68` (`cav-3b` plan/evidence). The active branch is `codex/implicit-ib-cav-4-multiplicative-composition` in `/tmp/ibamr-cav-durable-4`; its feature commit is `cc2b68af4a64dba7078ceeac6cde342f562dfd95`, and the current plan/evidence tip is the commit containing this stopping-point update. The disposable reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-4` Debug build compiles both dimensional libraries and the normal navier-stokes and IB targets. The exact focused selector discovers one new shared-executable fixture and passes **1/1 twice** in approximately 1.0 s and 0.74 s. Across all 64 RELAXED patches and default SVD, SVD, LU, symmetric-indefinite, and QR modes, patch order is valid and the restricted local RHS, fresh original residual `b-Ax`, completed correction, and final pressure-gauge-normalized state agree within `1e-10`. The complete serial-compatible shell-semantics scope passes **21/21** in approximately 10.3 s. Independent stale-residual, reverse-order, and missing-gauge mutations are all detected; the restored control passes. The serial-compatible broad `IB/` scope passes **14/14** in approximately 78 s. The exact broad `-R IB/` result is **14/18** in approximately 73 s, with all four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI/PRTE TCP-listener restriction. These are implementation validations, not independent audit results.
- **Current work:** the durable stack is green through the complete native global multiplicative-composition contract. The sweep operates entirely in the existing full global velocity-pressure numbering, restricts the current original residual for each patch in declared order, embeds the patch correction on the configured update set, updates the residual with the borrowed live operator before the successor patch, and applies the zero-mean gauge only to the completed pressure state. The test reuses the existing passive diagnostic observer and executable; production code, matrix ownership, local backend modes, patch construction, FAC orchestration, and matrix-free kernels are unchanged. No copied full/block/transpose/update matrix, persistent index translation, or `USER_DEFINED` kernel path is introduced. `cav-5` one-sweep sandbox parity is still required before M4 completes. RAS has not started, and no auditor, audit coordinator, or synthesizer has been launched.
- **Blockers:** no implementation blocker. Full review readiness still requires the MPI cases to be rerun outside this TCP-restricted sandbox; independent audit gates remain unlaunched and incomplete.
- **Exactly one next action:** create `cav-5-smoother-parity` from the committed `cav-4` plan/evidence tip and reproduce the pinned sandbox's single global multiplicative sweep through the live export/two-source common-arithmetic path, including forward/reverse traversal and RELAXED/STRICT scope labels, without changing FAC orchestration or starting RAS.
