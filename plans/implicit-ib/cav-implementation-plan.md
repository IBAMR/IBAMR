...
### Durable `cav-6b` native implementation-validation evidence

The `cav-6b-fac-vcycle-fgmres-parity` feature commit is `76bde97b8c993e8a008606b1c6f83988ba20d48e`, stacked directly on the committed `cav-6a` plan/evidence tip `19cc63db5259101c49d336d0a36483c5b48c6b72` in `/tmp/ibamr-cav-durable-6b`, with Debug build `/tmp/ibamr-cav-durable-6b-dbg`. The first live two-level integrator attempt exposed that `IBImplicitStaggeredHierarchyIntegrator` attached the finest-level assembled `SAJ` matrix to the outer hierarchy Jacobian and then applied that single-level representation to a multilevel vector. The existing guard correctly aborted. The durable fix leaves the outer hierarchy Jacobian on the original matrix-free IB path; FAC alone owns and applies its assembled per-level coupling matrices. This removes the invalid global/local representation crossing without adding a matrix, copy, persistent index translation, alternate operator, or new production option.

Same-unit native coverage reuses two established executables. The pressure-CAV solver-components case now requires FGMRES, records its residual history, freshly recomputes `b-Ax` with the original matrix-free Jacobian, and reports total, momentum, divergence, and independently evaluated IB coupling-consistency residuals with the Section 9 denominator floors. A deforming divergence-free probe avoids the elastic rigid-translation nullspace. The live run converges in four iterations with `rho=1.63006e-11`; its momentum and divergence residual norms are `3.575e-10` and `3.29634e-9`, and `rho_IB=8.01966e-11`, all under the fixed `1e-10` total/IB criteria. Replacing FGMRES with GMRES makes the case fail on the explicit method check.

A second input/output fixture reuses `implicit_stokes_ib_01` for two full implicit timesteps on an `8x8 -> 16x16` hierarchy. The normal live regrid callback fires exactly once before timestep zero at finest level one; both post-regrid solves converge and the final velocity/pressure norms are finite. Reattaching finest-level `SAJ` to the outer hierarchy Jacobian as a disposable mutation reproduces the single-level-vector abort, while the restored matrix-free candidate passes. No new executable, production audit hook, external fixture, `USER_DEFINED` kernel path, or generated run artifact is committed.

The exact focused commands use the checkout-root runner and the explicit MPI/numdiff paths. The residual case passes **1/1 twice** in 1.92 s and 1.92 s; the regrid case passes **1/1 twice** in 10.71 s and 10.70 s; the combined related solver-components and integrator scope passes **6/6** in 15.64 s. The complete `tests-IB` target builds both dimensional libraries. The canonical broad command

```sh
/tmp/ibamr-cav-durable-6b/attest \
  --test-directory /tests/ \
  --mpi-executable /opt/homebrew/bin/mpiexec \
  --numdiff-executable /Users/boyceg/code/autoibamr/dbg/packages/numdiff-5.9.0/bin/numdiff \
  --verbose -R IB/
...
discovers and passes **20/20** cases, including all four rank-4 cases, in approximately 127 s, below the 30-minute broad Debug budget. These are implementation validations, not independent audit evidence. Exact-tip two-source N2--N4 replay and equivalence to the disposable integration reconstruction remain open; M5 is therefore not yet promoted, and no auditor, audit coordinator, or synthesizer has been launched.
...
- [ ] **M5 — Multiplicative production path:** durable `cav-6a` supplies the passive no-copy FAC stage seam at feature commit `1dd6bce29`. Durable `cav-6b` feature commit `76bde97b8` keeps the outer hierarchy Jacobian matrix-free, passes same-unit live FGMRES/original momentum-divergence-IB residual checks, and passes an actual two-level CAV regrid/cache-lifecycle case plus the complete broad **20/20** IB scope. Exact-tip N2--N4 two-source common-arithmetic transfer/FAC/V-cycle/FGMRES replay and explicit equivalence to the disposable integration reconstruction remain required before M5 implementation validation can be promoted; every independent gate remains unlaunched and incomplete.
...
| Multiplicative production V-cycle/FGMRES path | Cleaned FAC/transfers plus historical 8 parity/residual-sweep intent | `cav-6b-fac-vcycle-fgmres-parity` | Reused solver-components case for FGMRES history and fresh total/momentum/divergence/IB residuals; reused full integrator with two-level live regrid; wrong-Krylov and reattached-single-level-`SAJ` mutations; N2–N4 two-source common-arithmetic replay and integration-tip equivalence | V-cycle; FGMRES | Native implementation is green at feature commit `76bde97b8`: the outer hierarchy Jacobian remains matrix-free while FAC owns per-level matrices; focused residual and regrid cases pass twice in 1.92 s and 10.71 s, related scope passes 6/6, and broad IB passes 20/20 including rank-4 cases in about 127 s. The native residual case has 4 FGMRES iterations, `rho=1.63006e-11`, momentum/divergence norms `3.575e-10`/`3.29634e-9`, and `rho_IB=8.01966e-11`. The disposable reconstruction remains green at `K=1`, `1e2`, and `1e4`, but exact durable-tip N2–N4 replay/equivalence and all independent audits remain open. |
...
| 2026-08-20 | Keep the production outer Stokes-IB hierarchy Jacobian matrix-free and confine assembled coupling matrices to FAC's per-level CAV consumers. | The first durable two-level regrid run reproduced an invalid representation crossing: the integrator attached finest-level `SAJ` to a multilevel outer vector and hit the operator's single-level guard. Feature commit `76bde97b8` removes that attachment; it adds no replacement matrix, copy, index map, public option, or alternate kernel. Same-unit FGMRES physical-residual and real-regrid fixtures pass, and a disposable reattachment mutation reproduces the abort. The production IB4 matrix construction continues to follow the optimized Fortran recurrence; `USER_DEFINED` remains excluded unless a later concrete mismatch requires a disposable discriminator. Exact-tip external parity remains open, so M5 and every independent gate remain incomplete. |
...
- **Active branch/worktrees:** the plan-only `cav-0` branch remains untouched at `127dc0d144c4f6c31985eddef236cd76ccc7b99b` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`. The active durable branch is `codex/implicit-ib-cav-6b-fac-vcycle-fgmres-parity` in `/tmp/ibamr-cav-durable-6b`, stacked directly on committed `cav-6a` plan/evidence tip `19cc63db5259101c49d336d0a36483c5b48c6b72`, with feature commit `76bde97b8c993e8a008606b1c6f83988ba20d48e` and Debug build `/tmp/ibamr-cav-durable-6b-dbg`. The exact-SHA cav-5 exporter remains disposable in `/tmp/ibamr-cav5-parity`; the integration reconstruction remains disposable in `/tmp/ibamr-cav-integration-reconstruction`; and the clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the exact cav-6b feature commit builds the complete `tests-IB` target and both dimensional libraries. The FGMRES/original-residual case passes **1/1 twice** in 1.92 s and 1.92 s; the full two-level CAV/regrid case passes **1/1 twice** in 10.71 s and 10.70 s; their complete related scope passes **6/6** in 15.64 s. The wrong-Krylov mutation and the reattached-finest-`SAJ` multilevel mutation both fail, and the restored candidate passes. The canonical broad `-R IB/` scope passes **20/20**, including all four rank-4 cases, in approximately 127 s. These are implementation validations, not independent audit evidence.
- **Current work:** durable production now keeps the outer hierarchy Jacobian on the original matrix-free Stokes-IB path while FAC owns the assembled per-level coupling matrices used by pressure-seeded multiplicative CAV. Full global velocity-pressure numbering remains canonical; no matrix copy or persistent index translation was introduced. The native end-to-end path now records FGMRES history and freshly recomputed total/momentum/divergence/IB residuals, and an actual hierarchy regrid proves operator/subdomain/factor reconstruction. Optimized Fortran-authoritative IB4 remains the only production IB4 path and `USER_DEFINED` remains absent. Exact durable-tip N2--N4 common-arithmetic replay and integration-tip equivalence remain open. RAS has not started. No auditor, audit coordinator, or synthesizer has been launched, and no independent gate is complete.
- **Blockers:** none. M5 cannot be promoted until exact-tip two-source transfer/V-cycle/FGMRES parity and explicit equivalence to the disposable integration reconstruction are freshly rerun and recorded.
- **Exactly one next action:** create disposable export instrumentation from exact feature commit `76bde97b8c993e8a008606b1c6f83988ba20d48e`, regenerate the clean pinned-oracle `K=1`, `10^2`, and `10^4` N2--N4 bundles, and run the declared two-source common-arithmetic transfer/V-cycle/FGMRES and integration-tip-equivalence comparison without modifying production code or starting RAS.
