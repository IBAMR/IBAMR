...
- **Active branch/worktrees:** the plan-only base is frozen at `1d209b7f11be4c9ebd221c68cefbfcedb95ea3df` on `codex/implicit-ib-cav-0-recovery-plan` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`; committed `cav-1a` is `c7940ad4211cb85c0958660f30e0e923a91e7ddb` on `codex/implicit-ib-cav-1a-live-operator-audit-schema` in `/tmp/ibamr-cav-durable-1a`; the active durable unit is `codex/implicit-ib-cav-1b-raw-export-comparator-contract` in `/tmp/ibamr-cav-durable-1b`; and the disposable implementation remains on `codex/implicit-ib-cav-integration-reconstruction` in `/tmp/ibamr-cav-integration-reconstruction`. All derive from exact foundation `0c8447e3593113a8b62f94df9482c60d896f0d3b`. The clean oracle worktree remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the durable `cav-1b` unit builds through the normal `tests-navier_stokes` aggregate, including both 2D and 3D libraries. Its exact focused selector discovers two same-unit cases, which pass post-format deterministic reruns **2/2** in 1.19 s and 0.83 s; the wider serial selector for the reused executable and comparator passes **14/14** in 8.73 s. The live case round-trips its 48-DOF, 304-entry raw operator bundle exactly at 17 digits, proves the global velocity/pressure map, and hand-checks a four-entry IBAMR `-Div` pressure row; the comparator accepts only the declared permutation/pressure-row-sign/pressure-gauge controls and detects every assigned negative mutation. Disabling only the live export hook in a disposable fixture is rejected by `attest`. The exact broad `-R IB/` command completes in 83.72 s with all 14 serial cases passing and four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI TCP-listener restriction; the explicitly labeled serial subset reruns green **14/14** in 80.57 s. Generated raw bundles and mutations remain under `/tmp`. These are implementation validations, not independent audit results.
- **Current work:** `cav-1b` contains only test/evidence infrastructure layered on `cav-1a`: one shared raw-bundle/comparator implementation, one fixture in the existing live level-solver executable, and one no-hierarchy comparator-control executable. It streams the borrowed live matrix and existing full global velocity-pressure numbering, creates no production matrix/block copy or persistent subdomain translation, applies the convention difference only to pressure equation rows/RHS, and confines gauge alignment to a pressure-state constant. It does not construct CAV patches, change local solvers or correction composition, observe FAC stages, alter matrix-free kernels, or start RAS. No auditor, audit coordinator, or synthesizer has been launched.
- **Blockers:** no implementation blocker in `cav-1b`. Full review readiness still requires the four discovered rank-4 broad cases to be rerun outside this TCP-restricted sandbox; the focused cases and all 14 serial broad cases are green.
- **Exactly one next action:** create `cav-1c-matrix-live-kernel-consistency` from the exact committed `cav-1b` tip and recover only the all-kernel matrix/live consistency claim with its same-unit 2D/3D native tests.
