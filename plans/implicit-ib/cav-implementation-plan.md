...
- **Active branch/worktrees:** the plan-only branch is `codex/implicit-ib-cav-0-recovery-plan` at `127dc0d144c4f6c31985eddef236cd76ccc7b99b` in `/Users/boyceg/.codex/worktrees/0760/IBAMR`. Committed durable tips are `c7940ad4211cb85c0958660f30e0e923a91e7ddb` (`cav-1a`), `efab0787cbdfce8f6672cc1aca1b75ca57cdc08b` (`cav-1b`), `6b5f6054744df7cc348cc2f1efdea2663c7b236a` (`cav-1c`), `deb566b5e2d3e9a49bcd2449a61142bdbb503a9f` (`cav-2` plan/evidence), and `eaa3ec43d36b26a47d7d3bae30e30ac0dee2ec76` (`cav-3a` plan/evidence). The active branch is `codex/implicit-ib-cav-3b-blas-lapack-robust-modes` in `/tmp/ibamr-cav-durable-3b`; its feature commit is `4f5d45464a8a6bda1a4e149fc5dbbf1c7451c746`, and the current plan/evidence tip is the commit containing this stopping-point update. The disposable reconstruction remains in `/tmp/ibamr-cav-integration-reconstruction`. The clean oracle remains detached and unmodified at `5b77344db6746269f8c77695c99e9043907ba74b` in `/tmp/implicit-ib-sandbox-cav-oracle-5b77344`.
- **Last green validation:** the `cav-3b` Debug build compiles the backend in both 2D and 3D. The exact focused selector discovers seven same-unit/retained fixtures and passes **7/7 twice** in 4.07 s and 3.87 s. The complete serial shell-semantics executable scope passes **20/20** in 12.93 s. Independent disposable changes to ignore the SVD cutoff, disable the QR rank check, and use the wrong symmetric-indefinite triangle are all detected; the restored control passes. The serial-compatible broad `IB/` scope passes **14/14** in 91.88 s. The exact broad `-R IB/` result is **14/18** in 92.30 s, with all four rank-4 cases blocked before IBAMR by the managed sandbox's Open MPI/PRTE TCP-listener restriction. These are implementation validations, not independent audit results.
- **Current work:** M3 is complete as implementation validation. The generic backend defaults to SVD and also supports LU, symmetric-indefinite, and full-rank QR; the fixed-LU compatibility key remains available and Cholesky is rejected for the indefinite Stokes patches. All persistent algebraic state outside LAPACK uses full global velocity-pressure numbering. Each patch retains one matrix-sized factor or solve representation, plus only the necessary pivots and reusable vector workspaces; QR/SVD setup factors and all LAPACK work arrays are released before application. No copied full, velocity-block, transpose, unfactored, or residual-update matrix and no persistent index translation are introduced. Production IB4 remains the optimized live Fortran-authoritative path, and no `USER_DEFINED` path exists. Global multiplicative semantics remain `cav-4`; RAS has not started. No auditor, audit coordinator, or synthesizer has been launched.
...
- **Exactly one next action:** create `cav-4-multiplicative-composition` from the committed `cav-3b` plan/evidence tip and isolate the exact global multiplicative correction/residual-update semantics with same-unit stagewise tests, without changing patch construction, local-solver modes, FAC orchestration, or starting RAS.
