...
FORWARD_DECL_CAPTURE_RE = re.compile(
    r"^\s*(?:template\s*<[^;{}]+>\s*)?(?:class|struct)\s+([A-Za-z_][A-Za-z0-9_]*)\s*;\s*$"
)
USING_ALIAS_LINE_RE = re.compile(r"^\s*using\s+[A-Za-z_][A-Za-z0-9_]*\s*=\s*[^;]+;\s*$")
USING_ALIAS_LHS_RE = re.compile(r"\busing\s+([A-Za-z_][A-Za-z0-9_]*)\s*=")
...
def _build_base_to_alias(entries: Sequence[AliasEntry]) -> Dict[str, Tuple[str, Set[str], bool]]:
    ...
    return {base: (alias_name, include_keys, is_template_alias)
            for base, (alias_name, include_keys, is_template_alias, _) in best_for_base.items()}


def _rewrite_samrai_forward_decls(text: str, entries: Sequence[AliasEntry]) -> Tuple[str, bool, Set[str]]:
    base_to_alias = _build_base_to_alias(entries)
    alias_to_include = {entry.alias_name: entry.include_name for entry in entries}
    base_to_include: Dict[str, str] = {}
    for base, (alias_name, _, _) in base_to_alias.items():
        inc = alias_to_include.get(alias_name)
        if inc is not None:
            base_to_include[base] = inc

    changed = False
    needed_includes: Set[str] = set()
    ns_stack: List[str] = []
    out_lines: List[str] = []
    ...
    for line in text.splitlines(keepends=True):
        ...
        # Handle namespace closes before matching declarations on the line.
        close_count = stripped.count("}")
        for _ in range(close_count):
            if ns_stack:
                ns_stack.pop()

        m_decl = FORWARD_DECL_CAPTURE_RE.match(stripped)
        remove_line = False
        if m_decl:
            name = m_decl.group(1)
            in_samrai_scope = "SAMRAI" in ns_stack
            in_samrai_module_scope = bool(set(ns_stack) & set(SAMRAI_MODULES))

            include_name = None
            if name.startswith("SAMRAI") and name in alias_to_include:
                include_name = alias_to_include[name]
            elif (in_samrai_scope or in_samrai_module_scope) and name in base_to_include:
                include_name = base_to_include[name]

            if include_name is not None:
                needed_includes.add(include_name)
                remove_line = True
                changed = True
        ...
        # Handle namespace opens after declaration handling.
        for m_ns in re.finditer(r"\bnamespace\s+([A-Za-z_][A-Za-z0-9_:]*)\s*\{", line):
            ns_stack.extend([n for n in m_ns.group(1).split("::") if n])

    return "".join(out_lines), changed, needed_includes


def _find_samrai_forward_decl_lines(text: str, entries: Sequence[AliasEntry]) -> List[str]:
    base_to_alias = _build_base_to_alias(entries)
    alias_names = {entry.alias_name for entry in entries}
    base_names = set(base_to_alias.keys())
    ...
    for line in text.splitlines():
        ...
        if m_decl:
            ...
            in_samrai_scope = "SAMRAI" in ns_stack
            in_samrai_module_scope = bool(set(ns_stack) & set(SAMRAI_MODULES))
            if name in alias_names or ((in_samrai_scope or in_samrai_module_scope) and name in base_names):
                found.append(line)
...
def _extract_noninclude_string_literals(text: str) -> List[str]:
    text_wo_includes = "\n".join(
        line for line in text.splitlines() if not INCLUDE_RE.match(line)
    )
    return re.findall(r'"(?:\\.|[^"\\])*"', text_wo_includes)
...
def _replace_types(
    ...
    for base, (alias_name, include_keys, is_template_alias) in base_to_alias.items():
    ...
    for base, alias_name, include_keys, is_template_alias in work:
        ...
        use_unqualified = aggressive_unqualified
    ...
    # Normalize alias template arguments to a fixed point so nested aliases
    # are handled (e.g. SAMRAIIndexIterator<..., SAMRAICellGeometry<NDIM>>).
    while True:
...
def _mask_using_alias_lhs(text: str) -> Tuple[str, Dict[str, str]]:
    token_to_name: Dict[str, str] = {}
    out_lines: List[str] = []
    for idx, line in enumerate(text.splitlines(keepends=True)):
        line_no_nl = line[:-1] if line.endswith("\n") else line
        m = USING_ALIAS_LINE_RE.match(line_no_nl)
        if not m:
            out_lines.append(line)
            continue
        m_lhs = USING_ALIAS_LHS_RE.search(line_no_nl)
        if not m_lhs:
            out_lines.append(line)
            continue
        alias_name = m_lhs.group(1)
        token = f"__SAMRAI_REWRITE_USING_ALIAS_LHS_{idx}__"
        token_to_name[token] = alias_name
        replaced = line_no_nl.replace(alias_name, token, 1)
        out_lines.append(replaced + ("\n" if line.endswith("\n") else ""))
    return "".join(out_lines), token_to_name


def _unmask_using_alias_lhs(text: str, token_to_name: Dict[str, str]) -> str:
    out = text
...
def _is_dimension_template_arg(arg: str) -> bool:
    a = arg.strip()
    # Accept canonical dimension identifiers and literal dimensions.
    if a in {"NDIM", "DIM", "dim", "spacedim"}:
        return True
        ...
        if alias_name in alias_template_flags and j < n and text[j] == "<":
            ...
            if depth == 0 and k > j:
                ...
                if is_template_alias:
                    ...
                    # SAMRAI4 aliases drop a leading dimension argument only.
                    # Preserve non-dimension leading arguments (e.g. LSet<T>).
                    if len(args) > 1 and _is_dimension_template_arg(args[0]):
...
def _remove_redundant_dim_templates(text: str) -> Tuple[str, bool]:
    lines = text.splitlines(keepends=True)
    out: List[str] = []
    i = 0
    changed = False
    templ_re = re.compile(r"^\s*template\s*<\s*int\s+DIM\s*>\s*$")
    while i < len(lines):
        line = lines[i]
        if not templ_re.match(line.rstrip("\n")):
            out.append(line)
            i += 1
            continue

        # Collect the next declaration block and remove template<int DIM>
        # only when DIM is no longer referenced in that declaration.
        j = i + 1
        decl_lines: List[str] = []
        while j < len(lines):
        ...
        # Restrict this cleanup to function templates only; keep class/struct
        # forward declarations templated on DIM unchanged.
        if decl_text and "(" in decl_text and "DIM" not in decl_text:
            changed = True
            i += 1
            continue
        out.append(line)
...
def _ensure_names_header(text: str) -> Tuple[str, bool]:
    ...
    if names_include in text or '#include "ibtk/samrai_compatibility_names.h"' in text:
        return text, False
...
def _is_examples_or_tests_path(path: pathlib.Path) -> bool:
    p = path.as_posix()
    return (
        p.startswith("examples/")
        or p.startswith("ibtk/examples/")
        or p.startswith("tests/")
        or "/examples/" in p
        or "/ibtk/examples/" in p
        or "/tests/" in p
    )


def _is_samrai_compat_include(include_name: str) -> bool:
    if include_name == "ibtk/samrai_compatibility_names.h":
        return True
    base = pathlib.PurePosixPath(include_name).name
    return base.startswith("SAMRAI") and base.endswith(".h") and base != "SAMRAI_config.h"


def _samrai_heading_index(lines: List[str]) -> int | None:
    preferred = (
    ...
    for i, line in enumerate(lines):
        if line.strip() in preferred:
            return i
    fallback = re.compile(r"^\s*//\s*.*SAMRAI.*(?:INCLUDES|objects)\s*$")
    for i, line in enumerate(lines):
...
def _normalize_examples_tests_samrai_include_block(text: str, path: pathlib.Path) -> Tuple[str, bool]:
    if not _is_examples_or_tests_path(path):
        return text, False
    lines = text.splitlines()
    heading = _samrai_heading_index(lines)
    if heading is None:
        return text, False

    compat_includes: List[str] = []
    keep_lines: List[str] = []
    for line in lines:
        m = INCLUDE_RE.match(line)
        if m and _is_samrai_compat_include(m.group(3).strip()):
            compat_includes.append(m.group(3).strip())
            continue
    ...
    ordered = list(dict.fromkeys(compat_includes))

    heading = _samrai_heading_index(keep_lines)
    if heading is None:
        return text, False

    insert_at = heading + 1

    new_lines = keep_lines[:insert_at] + [f"#include <{inc}>" for inc in ordered] + keep_lines[insert_at:]
    new_text = "\n".join(new_lines) + ("\n" if text.endswith("\n") else "")
...
def transform_file(
    ...
    text_after_fwd, fwd_changed, fwd_alias_includes = _rewrite_samrai_forward_decls(text_after_includes, entries)
    masked_text, include_tokens = _mask_include_lines(text_after_fwd)
    masked_text, fwd_decl_tokens = _mask_forward_declarations(masked_text)
    masked_text, using_alias_lhs_tokens = _mask_using_alias_lhs(masked_text)
    ...
    text_after_types = _unmask_using_alias_lhs(text_after_types, using_alias_lhs_tokens)
    ...
    text_after_types, dim_template_changed = _remove_redundant_dim_templates(text_after_types)
    alias_names = {entry.alias_name for entry in entries}
    ...
    used_alias_includes.update(fwd_alias_includes)
    ...
    text_normalized, include_layout_changed = _normalize_examples_tests_samrai_include_block(text_with_names, path)

    if (
        includes_changed
        or fwd_changed
        or types_changed
        or dim_template_changed
        or dequalify_changed
        or alias_includes_changed
        or names_changed
        or include_layout_changed
    ):
        return old_text, text_normalized
...
def main(argv: Sequence[str]) -> int:
    ...
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="Suppress per-file output and print only final counters.",
    )
    parser.add_argument(
        "--progress-every",
        type=int,
        default=0,
        help="Print a compact progress line every N files (0 disables progress output).",
    )
    parser.add_argument(
        "--echo-file",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Fail if any rewritten file changes non-include string literals.",
    )
    parser.add_argument(
        "--fail-on-samrai-forward-decls",
        action="store_true",
        help="Fail if SAMRAI forward declarations remain after rewrite.",
    )
    ...
    for file_arg in args.file:
        ...
        if args.fail_on_string_change and old_text != new_text:
            old_strings = _extract_noninclude_string_literals(old_text)
            new_strings = _extract_noninclude_string_literals(new_text)
            if old_strings != new_strings:
                failed_files += 1
                print(f"error: string literals changed in {path}", file=sys.stderr)
                continue
        if args.fail_on_samrai_forward_decls:
            remaining_fwd = _find_samrai_forward_decl_lines(new_text, entries)
            if remaining_fwd:
                failed_files += 1
                print(f"error: SAMRAI forward declarations remain in {path}", file=sys.stderr)
                for line in remaining_fwd[:5]:
        ...
        if old_text == new_text:
            ...
            if args.progress_every > 0 and (total_files % args.progress_every) == 0:
                print(
                    f"progress: {total_files}/{len(args.file)} files "
                    f"(updated={changed_files}, unchanged={unchanged_files}, skipped={skipped_files})",
                    file=sys.stderr,
                )
            continue
        changed_files += 1
    ...
    if not args.write and changed_files == 0 and not args.summary_only:
        print("no changes")
    print(
        f"summary: files={len(args.file)} updated={changed_files} "
        f"unchanged={unchanged_files} skipped={skipped_files} failed={failed_files}",
        file=sys.stderr if args.summary_only else sys.stdout,
    )
