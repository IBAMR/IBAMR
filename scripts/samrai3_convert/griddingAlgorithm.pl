...
for my $file (@filesToProcess) {
    print "Updating GriddingAlgorithm for source file $file\n";
    ...
    while ( my $str = <FILE> ) {
        # Update constructor for GriddingAlgorithm. Needs PatchHierarchy.
        $str =~ s/new\ GriddingAlgorithm\(/new\ GriddingAlgorithm\(patch_hierarchy, /g;
        print TEMPFILE $str;
    }
